...
010/30 lm-SRI 的最后证明轮暴露出一个停止策略问题：默认最多 5 个 probe 候选时，queue 比例虽然从约 `4571` 降到 `71.6`，但仍明显不均衡，循环却因为 `maxCandidates=5` 停止。由此修改为：基础候选数仍保持原配置；当达到基础上限后，如果当前 score 仍超过 `bidirectionalMidpointProbeHighImbalanceRatio`（默认 10.0），且试探方向没有反转，则继续同方向试探，不再额外设置固定次数上限。一旦方向反转，仍按原逻辑在前后两个点之间额外试一次中点后停止；若 score 已降到阈值以下，则停止并从已有候选中选最优。停止保护来自 rank0、方向反转、score 达标、candidate 贴边/重复等自然条件。这个改动只影响 probe 的候选搜索长度，不改 label 扩展、join、SRI reduced-cost 或 completion-bound 语义。

### 2026-06-15 arc-memory rank-1/SRC 思路评估

阅读 `arc_memory_rank1_summary_clean.pdf` 后，核心思路可以概括为：cut 仍然先按 full-memory rank-1/SRI 的三元组 violation 来识别，也就是先确认某个 `(S,p)` 在当前 LP 正值列上确实有切割价值；但真正加入 master 时不保留 full memory，也不使用当前 node-memory 的“经过 memory job 才保留 state”，而是构造一个 memory arc set `A_M`。列系数扫描时按 route 的 arc 顺序推进：若当前 arc `(i,j)` 不在 `A_M`，则该 cut 的 state 清零；随后如果 head job `j` 属于 scope，则累加 multiplier；state 达到 1 时 coefficient 加 1 并扣回 state。这样 full cut 在当前正值 route 上的 violation 可以保持不变，但对其他 route 允许更细粒度遗忘，理论上会比 node-memory 更少破坏 labeling dominance。

构造 memory arc 的方式也比较直接。对每条当前 LP 中正值且 full coefficient 大于 0 的 route，沿 route 扫描 scope job。只要 state 已经大于 0，就说明有一段 partial state 正在向后传递，此时经过的 arc 暂存到 `A_aux`；一旦后续 scope job 让 state 达到 1，就把这一段 `A_aux` 加入该 cut 的 `A_M`，然后清空 `A_aux` 并扣回 state。直观上，真正需要记住的是两个触发 scope job 之间用于传递 half-state 的 arcs。进入第一个 scope job 的 arc 通常不需要记，因为此前 state 为 0，reset 不会损失信息。若已有相同 `(S,p)` 的 cut，文档建议合并 memory arcs，而不是再建一条新 cut，避免 active nonrobust cut 数量膨胀。

放到当前 TWET 代码里，master 侧改造不难，但 pricing 侧不是简单替换字段。`TWETCut` 目前只有 `memoryJobs`，需要增加 arc-memory 元数据，最好用 flattened arc key 或 `(n+2)*(n+2)` 的布尔预处理数组；`SubsetRowCutEvaluator` 需要增加 arc-memory coefficient 扫描，LP 建 cut 行和动态加列会自然走这个 evaluator，因此 master 口径比较容易统一。`SubsetRowCutGenerator` 则需要在现有 `buildLimitedMemorySet()` 之外新增 `buildLimitedMemoryArcSet()`，仍然沿用现有三元 SRI separation 和每轮 cut 数/appearance 限制。

真正要小心的是 exact ng-DSSR pricing。forward extension 现在只按新 job 判断 lm-SRI state，arc-memory 必须改为按扩展弧 `(current,next)` 判断是否 reset，再处理 head job `next`。backward prepend 也必须知道新弧 `(prev,current)`，不能继续只传一个 job；反向 label 的 residual state 应表示“当前 suffix 前端 memory-connected segment 的剩余 half-state”，当 `(prev,current)` 不在 memory arc 中时，旧 suffix 的 front residual 不能再和 `prev` 前面的 prefix 合并。join 阶段同理，不能简单用 `forwardState + backwardState >= 2`，还必须检查 crossing arc `(forwardLast, backwardFirst)` 是否属于该 cut 的 memory arc；不属于时 forward residual 会在 crossing arc 上被清零，只能保留 backward suffix 自己的 residual。

启发式 pricing 也要同步改。当前 sequence-based lm-SRI 已经有 prefix/suffix state profile，可用于 remove/add/exchange 的局部 penalty 计算；arc-memory 下这些 profile 的 state 转移要带上相邻 arc。前缀扫描需要知道 `(prev,job)`，后缀扫描需要知道 `(job,next)`，局部 move 的 `sequenceBasedPenalty(prefix + middle + suffix)` 还要按桥接 arc 判断 state 是否延续。这个仍然可以做成局部计算，不必每个候选完整重扫，但接口要从 `applyForwardExtension(states, job)` 改成类似 `applyForwardExtension(states, from, to)`。
...
### 2026-07-25 40-2 partial-list 对照与 SRI state 分区判断

本轮用 `wet040_001_2m` 做了同配置 root-only 对照。无 SRI 时，当前 source-aware graph 得到 `bound=22490, root=14.289s, exact=4.291s/16`；切回旧 partial-list 后得到相同 bound，`root=15.290s, exact=4.422s/17`。这个简单实例上 partial-list 只慢约 5.5%，不能仅凭无 cut 的 40-2 断言它一定严重退化。

真正的差异出现在 active SRI。40-2 partial-list + arc-memory SRI 得到 `root=121.209s, exact=38.271s/75, heuristic=76.763s/292`，共执行 16 轮 cut loop、加入 80 条 cut；cut separation 自身只有 0.072 秒。进一步按 active SRI 状态拆分同一 run：SRI 尚未激活、仍使用 source-aware graph 的 15 次 exact 平均 226.649ms，平均 kept labels 为 forward 214.3、backward 135.1；SRI 激活并切到 partial-list 后的 60 次 exact 平均 581.189ms，平均 kept labels 增至 forward 2289.0、backward 829.2，并产生 13,758,524 次 partial-list dominance 比较。因此这里的核心问题确实是 active SRI 迫使主线放弃新 dominance graph，而不是 cut separation 本身。总 root 时间的约 8 倍差距还包含额外 cut-loop 和 heuristic 调用，不能全部归因于 dominance backend；但 exact 单次约 2.56 倍、标签规模约 6 至 11 倍的差异已经足以确认该热点。

历史 ng-set 规模并没有接近任务数。40-2 的多组旧实验全局最大值通常为 6；setup 变化实验约为 3 至 10。50-3 W300 的困难证书轮次中，平均 ng-set 最终约 6.44 至 8.72，单 job 最大值通常为 14 至 15；采用只更新最短环时最大值约 10。当前 40-2 K4/top20 对照的最大值为 7。需要注意，较早记录还包含“job 自身放入 ng-set”的旧语义，和当前默认排除自身相比，数值解释可能相差约 1。总体上，真实 ng-memory 维度是中等的，但该统计不能代替 SRI residual state 数量统计。

基于当前 arc-memory SRI，可以把 source-aware graph 扩展为按 SRI residual state 分区：每个 terminal job 下，先按 packed residual bit vector 分区，每个分区内部继续使用现有 `reachableSet` dominance DAG。等价的复合身份是 `(terminalJob, sriResidualState, reachableSet)`。同一 residual state 下未来 SRI 转移一致，因此现有 envelope dominance 可以安全复用；不同 state 之间第一版不做 dominance，虽然弱于论文式跨 state 补偿，但避免了 partial-list 对全部 labels 的扫描。join 不能要求正反 state 相同，而应按 `(trueNgMemorySet, sriResidualState)` 分组；对于当前 arc-memory，给定正反 state 和 crossing arc 后，SRI join shift 是固定的，因此可恢复 group-envelope prefilter，再对 surviving group pair 恢复真实 label pair。

该设计不应直接推广到 full-SRI。full-SRI 的扩展系数还依赖 scope job 是否已访问，仅有 residual bit 不足以表示未来行为；若要支持，key 还必须包含与 active cut scope 有关的 visited mask。active cuts 在 cut round 之间也会变化，因此 state interner 和 dominance graph 必须按每次 exact pricing 重建，不能跨 cut 集复用。热路径上不应继续使用现有字符串形式 `sriStateKey`，而应使用 packed/interned immutable state。下一步实现前，先统计每个方向、每个 terminal job 的 distinct residual states、zero-state 占比、复合 key 相对 no-SRI key 的膨胀倍数和每个 state 的 label 数；理论上状态数可指数增长，但 arc-memory 在非 memory arc 上频繁清零，实际状态可能较稀疏。当前判断是：这条 state-partitioned source-aware graph 是让 ng-DSSR 真正利用 SRI 的最合理方向，第一版只支持 arc-memory，不做 full-SRI 和跨 residual dominance。

实现结构上不必为每个 SRI state 复制一棵 Hasse 图。更合适的做法是继续只用 `reachableSet` 构造每个 terminal job 的 source-aware DAG，在每个 dominance node 内维护 `StateId -> sourced envelope`。新 label 到达时，以它的 packed SRI residual 获取对应 state bucket，只查询和更新该 bucket；局部 envelope delta 也只沿同一 `StateId` 向 reachableSet successors 传播。label 沿任务弧扩展后先更新 residual，再以新 state 进入下一 terminal job 的图。这样共享 reachableSet 拓扑，同时保证只有未来 SRI 行为一致的 labels 才进行 envelope dominance。

join 时不能把正反 state 合成一个相同 key，也不能要求二者相等。forward/backward 分别按 `(trueNgMemorySet, StateId)` 聚合；每个 state-pair 先检查 ng-memory 兼容，再根据 crossing arc 计算 arc-memory residual 的固定 join shift。半路径 frontier 已包含各自内部产生的 SRI reduced-cost contribution，join 只补 crossing arc 把两侧 residual 连起来后新增的 cut coefficient。因为该 shift 对一个 state-pair 和 crossing arc 是常数，可以先拼 group envelopes 做安全预剪枝，只有 surviving group-pair 再恢复 source labels、生成 sequence，并用统一 evaluator 做最终系数复核。

### 当前 limited-SRI 的 partial-list dominance 口径
