...
集合规模进一步把差异表现得更直接。family 的 wide、zero、narrow 被选 CutSet 平均规模分别为 `9.71/9.02/10.52`，中位数为 `15/8/7.5`；random set03/set05 的平均规模为 `5.46/3.31`，中位数只有 `3/2.5`。random set05 的42次CutSet中有21次恰好是二元集合，30次不超过3个任务；set03的均值还被一个28任务的大集合明显抬高，13次中仍有7次不超过3个任务。由此可见，当前方法在family中往往一次约束一个较大的族内路线块，在random中则经常退化成二元或三元局部集合，实际没有聚合掉多少全局替代选择。规模本身不是充分条件，但它与“稳定块内替代”和“边界漂移的全局替代”机制一致。
