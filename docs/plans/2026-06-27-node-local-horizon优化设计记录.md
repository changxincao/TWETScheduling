...
同时暂停了 `Tree` 中每个 node 开头的 `diagnoseLocalHorizonAtNode(node)` 调用，只保留方法本体和注释。原因是分支和 pricingOnly 禁弧会让 node 的可行顺序集合更强，局部 Cmax 通常只会变差；即使用 CP/MIP 在某个 node 上求到可行 horizon，也不稳定改善后续 pricing。当前决定仍使用初始启发式收缩后的全局 `CmaxH`，node-local CP/MIP horizon 诊断暂不接入。
