...
> **文档状态：历史实验与方案演变。** 本文中的 `setupR25/R50/R75`、旧 `SetupRatioVariantGenerator` 和后续建议只服务早期A/B，不是当前正式数据。当前正式random/family setup均值、上限和构造方式以 `docs/plans/2026-08-23-正式实验当前统一方案.md` 第1.3节为准。
...
## 2026-07-01 setup-ratio 三档 strong / no-strong 补跑

按用户要求补齐 `setupR25/R50/R75` 三档在 strong branching 开关上的对照。算例仍为 `data/setup-variants/40-2/wet040_001_2m_setupR25/R50/R75.dat`，setup cost 系数为 `20`。time-indexed 组使用纯 `TimeIndexedGraphPricingEngine`，不启用旧启发式 pricing、SRI/rank-1 cut 或 completion-bound helper；ng-DSSR 组使用 nearestK8/top10、`BEST_UB`、启发式 pricing、ALNS seed、`allCycles` completion bound、subtree pricingOnly、midpoint probe/reuse、dual-bound pruning 和 post-node time-indexed helper。

time-indexed 强分支结果来自 `test-results/bpc/tmp-timegraph-40-2-setupR75-cost20-sb-alns-20260701.csv`：`R25 solve=214.180s,root=111.845s,nodes=6,pool=84998,obj=31893`；`R50 solve=334.596s,root=60.035s,nodes=40,pool=160765,obj=43625`；`R75 solve=261.224s,root=88.995s,nodes=23,pool=136927,obj=55007`。time-indexed 不开强分支结果来自 `test-results/bpc/tmp-timegraph-40-2-setupR-all-cost20-nostrong-alns-20260701.csv`：`R25 solve=218.453s,root=191.403s,nodes=6,pool=87879,obj=31893`；`R50 solve=385.109s,root=103.018s,nodes=92,pool=278276,obj=43625`；`R75 solve=292.818s,root=162.234s,nodes=68,pool=218911,obj=55007`。

ng-DSSR 强分支结果来自 `test-results/bpc/tmp-ngdssr-40-2-setupR75-cost20-sb-alns-20260701.csv`：`R25 ROOT_PROCESSED,solve=131.461s,root=131.459s,nodes=1,pool=11717,obj=31893`；`R50 solve=258.703s,root=81.499s,nodes=5,pool=22961,obj=43625`；`R75 solve=386.189s,root=92.692s,nodes=11,pool=43074,obj=55007`。ng-DSSR 不开强分支结果来自 `test-results/bpc/tmp-ngdssr-40-2-setupR-all-cost20-nostrong-alns-20260701.csv`：`R25 ROOT_PROCESSED,solve=132.152s,root=132.147s,nodes=1,pool=9334,obj=31893`；`R50 solve=180.958s,root=107.594s,nodes=9,pool=20279,obj=43625`；`R75 solve=264.473s,root=77.022s,nodes=30,pool=33319,obj=55007`。

这组结果说明 strong branching 对两种 pricing 的作用不同。time-indexed 下 strong branching 基本稳定有利：R50/R75 节点数和列池显著下降，总时间也下降，R25 总时间接近但 root 更短。ng-DSSR 下 strong branching 没有稳定收益：R50/R75 虽然节点数更少，但强分支试探和更重的 master LP/子节点准备成本没有被完全抵消，不开 strong 反而更快；R25 两者都在 root 闭合，差异很小。因此后续不能简单把 strong branching 设为“总是更好”，需要按 pricing 口径和实例时间尺度分开比较。当前最直接的观察是：time-indexed 更依赖 strong branching 控制 pseudo-schedule 列和分支树；ng-DSSR 的列本身更强，强分支带来的树缩减不一定覆盖试探成本。

进一步拆分 ng-DSSR 的 strong/no-strong 时间可以看到，strong 变慢的主因不是正式 pricing。R50 中，strong 版本正式 heuristic/exact pricing 为 `57.975s/21.409s`，no-strong 为 `82.156s/33.535s`，也就是说 strong 确实减少了正式 pricing；但 strong 版本 master LP 时间达到 `130.994s`，其中 `strong_branching_rmp=91.549s`、`strong_branching_after_column_filter=29.595s`、`strong_branching_phase2_initial=6.118s`，另有 `HeuristicPricing[strongBranching]=16.113s`。相比 no-strong 的 master LP `14.681s`，这部分额外试探成本直接吃掉了节点减少的收益。

R75 的结构更明显：strong 版本正式 heuristic/exact pricing 为 `72.887s/44.872s`，no-strong 为 `81.863s/94.490s`，正式 exact pricing 少了很多；但 strong 版本 master LP 为 `195.156s`，其中 `strong_branching_rmp=136.078s`、`strong_branching_after_column_filter=43.992s`、`strong_branching_phase2_initial=8.794s`，另有 `HeuristicPricing[strongBranching]=32.046s`。no-strong 的 master LP 只有 `25.587s`。因此 ng-DSSR 上 strong branching 的问题是：它用更少节点换来了大量 trial RMP/child LP，当前 R50/R75 的 trial LP 成本过高，不是 pricing 本身失控。

从调用次数看，耗时机制更明确。R50 有 `2` 次 strong branch，phase1 按 `20` 个候选、左右两支计算，正好形成 `80` 次 `strong_branching_rmp`，平均每次 `1.144s`；筛列后又有 `80` 次 `strong_branching_after_column_filter`，平均 `0.370s`；phase2 对 `4` 个候选左右两支，形成 `16` 次 `strong_branching_phase2_initial`，平均 `0.382s`。R75 有 `5` 次 strong branch，对应 `200` 次 phase1 RMP，平均 `0.680s`，以及 `199` 次筛列后 RMP，平均 `0.221s`、`40` 次 phase2 initial，平均 `0.220s`。因此这里不是某个单次 LP 异常慢，而是 strong branching 的候选侧 LP 数量太多，且每次 trial LP 已经达到几百毫秒到一秒量级。

代码层面确认，phase1 第一遍 trial RMP 使用的是父节点当前 `restrictedColumnIds` 作为 child seed；求解可行后再调用 `resetRestrictedColumnsByCurrentReducedCost(branchSeedColumnLimit=5000, branchSeedReducedCostAllowance=5000)` 筛列，并重解一次。因此现有日志虽未直接打印每个 trial 的列数，但可确认第一遍不是小模型，而是父节点当前 RMP 列集，第二遍通常被压到 5000 列左右。time-indexed strong branching 也有大量 phase1 LP，但单次 trial LP 明显更轻：R50 为 `strong_branching_rmp=101.144s/692 calls`，平均 `0.146s`，筛列后 `46.203s/665 calls`，平均 `0.069s`；R75 为 `85.757s/440 calls`，平均 `0.195s`，筛列后 `23.848s/423 calls`，平均 `0.056s`。也就是说 time-indexed 的 strong trial 总量也不小，但每个 RMP 很轻，节点减少后能抵消；ng-DSSR 的 trial RMP 单次更重，导致 strong branching 不划算。
