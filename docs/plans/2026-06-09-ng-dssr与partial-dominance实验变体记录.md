...
这里需要特别注意 ng-memory 的包含方向。若被删 label 为 `L3`，且 `M3 ⊆ M1`，这表示 `L3` 的 ng-memory 更小、join 限制更少，而不是更容易被 `L1` 替代。例如 `M3={5}`、`M1={5,9}`、backward memory `B={9}`，则 `M3∩B=∅`，但 `M1∩B={9}`；因此 `L3` 可以 join，数值更低的 `L1` 却会被集合检查拒绝。要按状态安全删除 `L3`，所需的充分方向应是存在数值不差的保留 label 满足 `M_keep ⊆ M3`，而当前 whole-node envelope 不保证这一点。另一方面，不同 father path、elementary sequence 或 non-elementary DSSR witness 本身只会改变加列批次和 DSSR 轮数；在无时间/轮数限制且 exact DSSR 真正闭合时，它们不应独立改变最终 bound，不能用作本次 bound 偏差的单独正确性解释。当前日志没有保存导致 `0.241785` bound 差异的具体 label-pair，因此确定证据是“来源清理改变了列族和闭合结果”，集合方向是代码层面的主要风险机制，但不能声称已追踪到某一条具体 `L3+B`。
