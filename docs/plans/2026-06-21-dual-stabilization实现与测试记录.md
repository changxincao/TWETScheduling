...
当前风险点是：这个剪枝要求 observed bound 来自已经覆盖完整列族的 certified exact pricing。当前主线用 ng-DSSR 定价，返回的通常是当前 DSSR/relaxation 轮中找到的一批负 elementary 列；当同轮还存在 non-elementary negative route，或 engine 只返回 top-K 候选时，返回列里的 best reduced cost 不等价于完整 elementary 列族的全局 `rc_min`。这时用它计算 `DB` 会把 lower bound 抬得过高，存在误剪风险。因此该开关保持默认关闭；后续若要正式启用，需要让 pricing result 显式携带“已证明的全局最小 reduced cost”证书，再由 PC 使用该证书剪枝。代码中已先保守处理：`lp.isColumnizedOutsourcing()` 时也不启用 dual-bound pruning，因为列化外包还必须同时证明内部列族和外包列族的 `rc_min`。
