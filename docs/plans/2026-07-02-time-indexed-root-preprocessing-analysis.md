...
`routeVisitedMask` 使用一个 `long` 保存 label 沿 father 链实际访问过的任务，任务 `j` 对应第 `j-1` 位。它与会遗忘历史任务的 `ngMemorySet` 不同，只用于 join 前识别 elementary/non-elementary：每个半路径同时维护“半路径内部是否已经重复”的 `routeElementary`；两侧各自 elementary 且两个 mask 不相交时，完整路径才可直接判为 elementary。任务编号超过 64 的 label 会关闭该快速口径并回退原有 sequence 检查，不影响正确性。
...
当前不直接用 `PackedBitSet` 全量替换 `routeVisitedMask`。虽然前者支持任意任务规模，而且已经有单 word 内联后端，但它仍要求每个 label 分配一个 `PackedBitSet` 对象，并在 child 扩展时复制；当前 `long` 则直接存放在 label 内，更新和求交均为零分配 O(1) 位运算。40/50/60-job 主实验都适用该快路径。若后续正式测试超过 64 个任务，更合理的实现是保留 `long` 快路径并为大规模实例增加多 word fallback，而不是让所有小实例承担对象分配成本。即使改用任意长度位集，`routeElementary` 仍不能删除，因为集合本身无法区分 `{3,7}` 与访问序列 `3,7,3`；可以去掉的只有表示 64 位口径是否可用的 fallback 标志。