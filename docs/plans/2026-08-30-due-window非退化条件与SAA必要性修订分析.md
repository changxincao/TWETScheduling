...
这一设定只能作为付费等待模型成立。若 $\eta_j=0$ 且没有位置成本、范围或 terminal cost，即使把每台机器首任务固定在最早时刻，仍可沿机器序列逐项插入免费等待并设置足够晚的点窗口，使 width 与 ET 同时归零，因而免费等待版本发生全局退化。
...
由于该模型也没有任务位置成本，$\lambda_j>\beta_j$ 或 $\lambda_j>\alpha_j$ 的最小宽度结论仍成立，overtime 不会改变该证明，因为压缩窗口时可以固定现有 timing。除此之外，它最主要的全局风险不是某个任务的系数，而是 terminal horizon 过松：若所有场景都能在 $T$ 内通过等待同步，overtime、ET 和 width 可能同时失去作用。数值实验既要避免上述 $\lambda_j$ 支配区间，也要单独校准 $T$ 的紧度。
