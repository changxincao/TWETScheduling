...
## 14. 2026-06-07 node3 指定 Tmid 的真实阶段诊断

为验证有限 probe 是否能代表真实阶段，本次增加了默认关闭的 `twet.bpc.midpointFullDiagnostic*` 诊断入口。命中指定 node 后，它对给定 Tmid 列表逐个重建 half-domain 状态，先跑真实 forward 阶段最多 `forwardSeconds`，再跑真实 backward 阶段最多 `backwardSeconds`，只打印 label/queue 统计，不向 RMP 加列。013 node3 本次使用整数 Tmid `551/496/446/402/362/325`，对应前面第一轮 probe 候选的整数化版本；forward 限时 180 秒，backward 限时 120 秒。

结果如下，括号内是方向化的 `forward/backward` 比例，不再取无方向倍数：

| Tmid | fw 耗时 | bw 耗时 | fwExh/bwExh | fwKept:bwKept | fwQ:bwQ | kept+queue ratio | queue-only ratio |
|---:|---:|---:|:---:|---:|---:|---:|---:|
| 551 | 180.001s | 2.128s | false/true | 330063:8488 | 128863:0 | 54.06 | 128864.00 |
| 496 | 180.003s | 3.155s | false/true | 160231:14747 | 17937:0 | 12.08 | 17938.00 |
| 446 | 41.794s | 3.793s | true/true | 69006:23002 | 0:0 | 3.00 | 1.00 |
| 402 | 11.385s | 4.997s | true/true | 30603:28827 | 0:0 | 1.06 | 1.00 |
| 362 | 2.758s | 5.715s | true/true | 14371:34628 | 0:0 | 0.42 | 1.00 |
| 325 | 0.814s | 6.946s | true/true | 7348:39090 | 0:0 | 0.19 | 1.00 |

这个结果把前面的判断收得更清楚。第一，`551` 和 `496` 在真实 forward 阶段 180 秒内都没有跑完，而 backward 几秒就结束，说明 completionBound reference 右侧候选确实是极端 forward-heavy，不只是浅层 probe 的错觉。第二，真实较均衡的点在 `402` 附近，`kept+queue` 方向比约 `1.06`；`446` 仍 forward 约 3 倍，`362/325` 已经转成 backward-heavy。第三，queue-only 只能用于“未耗尽时识别极端 frontier”。一旦两边都耗尽，`q=0:0`，queue-only 全部变成 `1.0`，无法区分 `446/402/362/325`，因此不能只用 remaining queue 做最终 score。

这也解释了有限 probe 为什么会选 `325-355`：在 pop=5000 的浅层截面里，`325-355` 的 queue 还没耗尽且 frontier 数量接近；但真实完整阶段显示 `325` 已经过左，backward kept 达 `39090`，而 forward kept 只有 `7348`。因此后续如果要更好设置 Tmid，不能只依赖浅层 queue 平衡。一个更稳的方向是两阶段：先用 queue-only 或 kept+queue 筛掉 `551/496` 这种极端偏右点，再对候选区间做少量更深或完整的阶段诊断，优先找 `kept+queue` 方向比接近 1 且两侧能在合理时间内耗尽的点。对这次 node3，真实诊断支持的候选是 `402`，不是浅层 probe 选出的 `325`。

关于候选迭代，当前 queue 模式的规则是：每个候选计算 `left=fwKept+fwQueue`、`right=bwKept+bwQueue`；若 `left>right`，认为 forward 压力大，下一候选乘 `0.9` 往左；否则乘 `1.1` 往右。最后选择时不是用最后一个候选，而是对所有候选排序：先看 reliability rank，再看 `abs(log((left+1)/(right+1)))`，再看总压力、pop 数、耗时和 Tmid。第二轮出现 `355.278≈1.02` 后又试到 `319.750≈2.02`，原因正是 `355.278` 处左侧仍略大于右侧，所以迭代规则继续往左乘 `0.9`，导致下一步过冲；最终选择仍会回到 score 更小的 `355.278`，不会选 `319.750`。
## 15. 2026-06-07 node3 第二轮候选的真实阶段诊断

继续按同一诊断入口测试第二轮 probe 候选。当前代码已经把 `Tmid` 尽量取整，因此小数候选 `541.5/487.35/438.615/394.7535/355.27815/319.750335` 实际诊断为 `542/487/439/395/355/320`。每个候选先跑真实 forward 阶段最多 180 秒，再跑真实 backward 阶段最多 120 秒；本诊断只统计标签和队列，不向 RMP 加列。

| Tmid | fw 耗时 | bw 耗时 | fwExh/bwExh | fwKept:bwKept | fwQ:bwQ | kept+queue F/B | queue-only F/B |
|---:|---:|---:|:---:|---:|---:|---:|---:|
| 542 | 180.005s | 1.360s | false/true | 308827:9464 | 101863:0 | 43.39 | 101864.00 |
| 487 | 160.999s | 2.568s | true/true | 141088:16255 | 0:0 | 8.68 | 1.00 |
| 439 | 33.620s | 3.846s | true/true | 61036:23868 | 0:0 | 2.56 | 1.00 |
| 395 | 7.940s | 5.363s | true/true | 26685:30071 | 0:0 | 0.89 | 1.00 |
| 355 | 2.274s | 6.323s | true/true | 12644:35772 | 0:0 | 0.35 | 1.00 |
| 320 | 0.757s | 7.653s | true/true | 6705:39778 | 0:0 | 0.17 | 1.00 |

这一轮结果和第一轮一致地说明了两个现象。第一，probe 的 queue-only 在浅层上会把 `355` 看成很好，因为它的剩余队列接近 `3899:3822`；但真实完整阶段中 `355` 已经明显偏左，backward kept 达到 `35772`，是 forward 的约 2.83 倍。第二，真实较均衡的位置不是 probe 选中的 `355`，而是在 `395` 附近；`395` 的 kept+queue 方向比为 `0.89`，无方向倍数约 `1.13`，两边都能在十秒级内跑完。`439` 仍 forward-heavy，`355/320` 已 backward-heavy。

因此当前更清楚的判断是：`0.9` 连续左移能快速排除 `542/487` 这种极端偏右候选，但如果某一步从 forward-heavy 变成 backward-heavy，继续简单乘法没有意义。后续更合理的候选策略应记录方向反转：例如 `439` 仍 forward-heavy、`395` 已轻微 backward-heavy，则 bracket 应落在这两个点之间；或者直接使用方向反转前后的较优点，而不是继续试到 `355/320`。如果只看 queue-only，耗尽后又会全部变成 `1.0`，因此真实阶段或更深二阶段 probe 必须保留 kept 口径。

进一步修正 queue-only 的解释口径。queue-only 不是“没用”，而是必须和队列耗尽状态一起看。若 probe 结束时两边队列都非空，queue-only 反映当前 frontier 的方向，通常比 kept+queue 对 split 变化更敏感；若一边队列为 0、另一边仍很多，说明一侧已耗尽而另一侧还有大量未处理 frontier，此时不能只看 queue-only，应结合 kept 或 kept+queue 判断总压力；若两边队列都为 0，说明该候选在 probe 预算内已经跑完，queue-only 退化为 `1.0`，这时真正应看 kept，且该候选的可靠性最高。013 node3 的问题不是 queue-only 本身错误，而是 rank=1 浅层 probe 的队列 frontier 不能估计后续子树规模。

关于 `439/395/355` 的时间和 label 数不单调，也需要区分“保留 label 数”和“实际计算工作量”。第二轮真实诊断不做 final join，只跑 forward/backward 扩展。`439` 的 forward constructed candidates 仍有约 119 万，`395` 降到约 49 万，`355` 进一步降到约 21 万；因此即使 `355` 的 backward kept 比 `395` 更多，forward 半域缩小带来的候选构造、函数操作和 dominance 检查减少更大，所以诊断时间反而更短。正式 exact pricing 还要做 final join，过多 backward label 会在 join 阶段重新变成压力，因此不能只用单侧扩展时间判断最终 Tmid。
## 16. 2026-06-07 probe 后续策略判断

综合两轮真实阶段诊断后，当前判断是：probe 在 013 node3 上是有用的。它未必能准确预测最终 full expansion 的正反向 kept label 比例，但能识别 completionBound reference 附近的极端偏右切分，避免 pricing 退化成近似单向 forward labeling。也就是说，probe 的主要价值不是精确预测最终比例，而是把 Tmid 从“明显会爆”的区域拉回到一个可求解的区间。

默认指标仍建议先用 `kept + queueRemaining`，即当前实现的 queue score 口径。这个指标比纯 kept 更能反映尚未处理的 frontier，比纯 queue-only 又能处理一边已经耗尽的情况。实际展示时可以直接输出 `F/B` 和无方向倍数，日志里的 `abs(log(F/B))` 只是排序时的等价变换，数值直观性较差。后续如果修改代码，可以保留内部用 `log` 或直接用倍数排序；两者在单调性上等价，关键是日志和诊断里应显示倍数。

候选迭代策略不宜继续简单固定跑满 6 个点。更合理的规则是：从 reference 出发，若当前 `F/B` 明显大于 1，则左移；若明显小于 1，则右移。若某一步方向反转，例如从 forward-heavy 变成 backward-heavy，则说明已经 bracket 到较优区间，最多在两个点之间再做一次中点试探，或直接选择两端中 score 更好的点。若某个候选的不均衡倍数已经低于阈值，例如 `1.05` 或 `1.10`，可以提前停止，尤其当该候选 rank=0 或两边 queue 都已明显被采样时更可信。阈值需要继续实验，013 node3 说明 `rank=1` 的低倍数仍可能低估后续 backward 子树，因此早停应保守。

节点内后续 pricing round 可以考虑复用上一轮选中的 Tmid 作为 reference。原因是同一 node 内分支约束、禁弧、任务结构和 horizon 不变，LP dual 会变但 Tmid 的合理量级通常不会剧烈跳变。一个稳妥做法是：每个 node 第一轮 exact pricing 正常 probe；后续轮次先用上一轮 selected Tmid 做一次很小预算的 probe 或直接检查其 `F/B`，如果倍数仍在阈值内就复用，否则再从该 Tmid 附近做局部 probe。这比每轮都从 completionBound reference 重新向左试探更省时间。
## 17. 2026-06-07 probe 停止规则与复用待测方案

后续实现时，`rank=0` 候选不需要再设置倍数阈值。`rank=0` 表示该 Tmid 在 probe 预算内两边队列都耗尽，probe 已经等价于该候选下的完整 forward/backward 扩展；此时可以直接停止继续试探，并在已测试候选中按 kept+queue/kept 倍数选择。阈值主要用于 `rank=1`，即两边都被采样但还没有耗尽的情况。

`rank=2` 在 balanced pop 之后正常情况下应很少出现，但代码层面仍可能出现：例如某一侧初始队列为空、Tmid 贴边、强时间窗或 bound 导致一侧在 pop 前就没有可扩展 label。因此不宜贸然删除概念；可以把它降级为异常/低可信诊断，正常求解中如果出现就不早停，并在日志里标记。若后续确认所有正式配置下不再出现，再考虑简化。

阈值需要实验，而不能只凭直觉固定。现有两个真实诊断表说明：单独的 forward/backward 阶段时间不一定随最终 kept 均衡单调变化，`1.5` 左右的 probe 比值也可能对应较快阶段；但正式 exact pricing 还包含 final join，因此判断阈值应以完整 exact pricing 的总时间、join 规模和是否能稳定返回为准。建议测试 `1.05/1.10/1.25/1.50/2.00` 几档 `rank=1` 早停阈值，记录 probe 时间、exact 非 probe 时间、full fw/bw kept、join pairs 和新增列数。初始保守候选可取 `1.25` 或 `1.50`，因为当前目标主要是避免极端单向 labeling，而不是精确最均衡。

probe 候选数建议上限先设为 5。若没有早停且方向未反转，就最多按单向移动试 5 个点；若方向反转，例如从 `F/B>1` 变到 `F/B<1`，说明已经 bracket 到较优区间，最多在前后两个点之间再做一次中点试探，然后停止。这样能避免像 `355 -> 320` 这种继续过头的试探，同时控制额外 dry-run 成本。
