...
2026-06-03 继续收紧 arc fixing 扫描。第一，时间域无交集不再等到 `shiftX/add` 后才发现，而是在扫描时直接判断 `prefix.head.start + delta > suffix.tail.end` 或 `prefix.tail.end + delta < suffix.head.start`，满足任一条件即说明 shifted prefix 与 suffix 没有时间交集。第二，给 `forwardFByJob/backwardBByJob` 各自预先计算函数最小值，先用 `min(forwardF_i)+min(backwardB_j)+setupCost(i,j)-arcDual(i,j)` 做弱 scalar 下界；若这个下界已经不小于 cutoff，则直接本地固定 arc，否则再回到完整函数拼接。
