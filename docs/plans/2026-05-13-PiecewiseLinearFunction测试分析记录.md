...
进一步讨论 backward pricing 时，当前倾向是 label 自身的函数左端不要提前裁剪，保持从 0 开始；这和 forward 中右端保持到 `T` 是对称的“锚点”思想。原因和启发式一致：backward 函数往前扩展时会做负向 `shiftX`，如果 label 函数提前带着某个“后段第一个任务最早完成时间”的左端限制，这个限制会被平移成“前段最后一个任务完成时间”的限制，从而误剪可行域。因此，backward label 函数本身不应通过物理 `setDomain` 裁左端；真正需要表达 profitable completion window 或任务自身可行窗口时，应只限制本次加入的 job 成本函数，把窗口外写成 `big_M`，再与 label 函数相加并调用 `minimizeSuffixInPlace()`。这样左侧 `big_M` 区间会通过 suffix-min 被右侧可行 completion 的最小值闭包覆盖，表示“如果当前时间下界很早，可以选择稍晚的真实完成时间”。这个语义和 forward 中右侧 `big_M` 经过 prefix-min 被左侧可行最优值闭包覆盖是对应的。
