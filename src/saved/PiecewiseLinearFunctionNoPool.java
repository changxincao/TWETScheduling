...
//假设各段之间区间连续的
public class PiecewiseLinearFunctionNoPool {
	...
	public PiecewiseLinearFunctionNoPool() {
		head = tail = null;
	}

	public PiecewiseLinearFunctionNoPool(double domainStart, double domainEnd) {
	...
	public PiecewiseLinearFunctionNoPool copy() {
		PiecewiseLinearFunctionNoPool res = new PiecewiseLinearFunctionNoPool(domainStart, domainEnd);
		if (this.head == null)
			return res;
	...
	// 设置定义域
	public PiecewiseLinearFunctionNoPool setDomain(double domainStart, double domainEnd) {
		//不会改变当前函数，会复制一个产生新的，不需要copy()以后 
		TimerManager.start("分段线性函数设置定义域");
		PiecewiseLinearFunctionNoPool res = this.copy();
...
//			}
	...
	public PiecewiseLinearFunctionNoPool shift(double delta) {
		TimerManager.start("分段线性函数shift");
		PiecewiseLinearFunctionNoPool res = copy();
		Segment p = res.head;
		while (p != null) {
			p.start += delta;
			p.end += delta;
	...
	public PiecewiseLinearFunctionNoPool add(PiecewiseLinearFunctionNoPool g) {
		PiecewiseLinearFunctionNoPool res = new PiecewiseLinearFunctionNoPool(domainStart, domainEnd);
...
//					Segment curSeg=new Segment(seg.start, tCross, a, b);
	 ...
	 */
	public boolean dominates(PiecewiseLinearFunctionNoPool g) {
		if (this.head == null)
			return false;
		if (g.head == null)
			return true;
	 ...
	 * 替换为前面段的最低值（保持非增性）。
	 */
	public void updateDominatedIntervals(PiecewiseLinearFunctionNoPool g) {
		if (this.head == null) {
...
//	            	            if (q == null) break;//可写可不写
	 ...
	 */
	public void mergeMinimum(PiecewiseLinearFunctionNoPool g) {
...
//	                replaceSegment(prev, p, cur, nxt, q.slope, q.intercept);
	...
	public void mergeMinimum2(PiecewiseLinearFunctionNoPool g) {
		// 1) 空函数处理
		if (this.head == null) {
			this.head = g.head;
			this.tail = g.tail;
