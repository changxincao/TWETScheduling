...
$rows=foreach($d in Get-ChildItem $WorkDir -Directory){
 ...
 $probe=0.;$rounds=0;$initialCount=0;$reuse=0;$calls=0;$fullF=0.;$fullB=0.;$post=0.;$filtered=0;$fallback=0
 foreach($l in [IO.File]::ReadLines((Join-Path $d.FullName live.log))){
  ...
  if($l -match 'midpointByDssrRound=r1/[^,]+?seedSource=firstRoundMean:filtered'){$filtered++;$reuse++}
  if($l -match 'midpointByDssrRound=r1/[^,]+?seedSource=firstRoundMean:fallback'){$fallback++;$reuse++}
