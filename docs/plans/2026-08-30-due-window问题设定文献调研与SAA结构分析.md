...
**修订说明。** 关于“范围约束是否推出 $[U_j,U_j]$”“initial idle 是否必须计费”“位置成本与宽度成本的系数区间”以及“什么条件下仍需联合 SAA”，以[决策型 due window 的非退化条件与 SAA 必要性：修订分析](2026-08-30-due-window非退化条件与SAA必要性修订分析.md)为准。该文把隔离任务的局部边界结论与完整机器序列区分开，并覆盖本文早期版本中偏强的概括。

关于含 waiting cost 的完整 SAA 模型、窗口端点分位数、消去 due-window 后的等价模型以及窗口 LP 对偶，统一以[随机 due-window 并行机调度：分位数投影与完整模型推导](2026-08-30-due-window分位数投影与完整模型推导.md)为准。本文后面的单任务公式和数值构造只保留为退化机制或边际方向说明，不作为完整模型的求解表达式。

当前主要结论是：**机器问题完全可以不设 overtime，而且当前所有候选模型都允许场景内等待；需要比较的是等待免费还是付费。** 宽度必须由正宽度成本、固定宽度或最大宽度控制。免费等待时，必须另用正位置成本或真实承诺范围锚定窗口位置，否则有限 SAA 可沿序列同步到足够晚的目标时刻并消掉 ET；付费等待在首任务时刻固定或 initial idle 同样计价时，也能提供位置斜率。当前推荐主模型为“正位置成本 + 正宽度成本 + 期望 ET + 二阶段付费等待”，每台启用机器的首任务固定在最早可行时刻；免费等待版本保留相同的位置和宽度成本，作为结构基准。另做删除位置成本的付费等待消融，以检验 waiting cost 单独锚定位置的效果；不再把 no-voluntary-idle 作为当前问题的候选模型。
...
因此，在当前始终允许等待的研究边界下，该设定应解释为 **位置成本锚定的免费等待基准**：等待仍然允许且单价为 0，但正位置成本阻止窗口无限后移。该模型是有界的，却会使单任务 early 系数退出局部值函数，并使窗口端点偏向点窗口或从 0 开始的宽窗口；它更适合当作结构对照，而不是最有表现力的主模型。付费等待版本则以正等待单价恢复早到一侧的有效斜率。
...
2. 若有正宽度成本但二阶段等待免费，在隔离任务、无位置成本且不影响下游的条件下，$[U_j,U_j]$ 是一个边界最优选择；完整机器序列中等待会推迟后续任务，因此这只是局部倾向，不是全局结论；
3. 若等待有正成本，最优窗口才更可能位于范围内部，并由样本完成时间分布与等待单价共同决定。
...
该式清楚说明等待成本的作用。$\eta=0$ 时，早到一侧的有效斜率为 0，快速场景可以免费等到目标；$0<\eta<\alpha$ 时，模型愿意等待来替代一部分 earliness；$\eta\ge\alpha$ 时，仅为避免这个隔离任务早到而等待没有收益，但等待仍然允许。真实序列中，一次等待还会移动所有后续任务，因此不能逐任务完全分离，但连续 LP 结构不变。
...
| 位置成本 | 正宽度成本 | 免费 | 有界但 early 局部失效；端点常为点窗口或 $[0,b]$，适合作为免费等待基准 |
| 无或有位置成本 | 正宽度成本 | 正等待成本，且 initial idle 计费或首任务时刻固定 | 非平凡；等待、ET 与窗口精度共同权衡 |
| 真实范围 | 固定/有成本宽度 | 正等待成本 | 非平凡且业务解释最强 |
...
### 5.1 文献对照：固定完成时刻时，标量函数变成场景向量函数

以下结构用于解释随机 DDA 文献，不属于当前候选模型。采用 earliest-start 或 no-voluntary-idle 后，给定排程 $x$，每个场景的完成时刻 $C_{j\omega}(x)$ 已确定。对任务 $j$ 投影窗口得到
...
### 5.2 自适应等待不破坏条件窗口投影，但会破坏逐任务等待投影

这里需要区分两个不同的消元顺序。令 $\mathbf C_j=(C_{j1},\ldots,C_{jN})$ 表示所有等待已经计入以后，任务 $j$ 的**实际**场景完成时间向量。给定 $\mathbf C_j$ 后，只要各任务使用彼此独立的任务特定窗口，窗口变量不进入机器可行性约束，而且位置、宽度与 ET 成本按任务相加，就仍可对每个任务分别定义
...
其中 $\mathcal T(x)$ 是由机器分配、顺序、随机加工时间和累计等待共同定义的 timing 可行域。这个变换对所有任务同时成立；窗口投影本身并没有被自适应等待破坏。各任务之间仍然通过 $\mathcal T(x)$ 耦合，因为一次机器空闲会改变同一机器上所有后续任务的完成时间。

真正不能全局使用的是另一种局部推导：先把某个任务从序列中隔离，再单独优化它自己的等待量，由此把 early 单价替换为 $\min\{\alpha_j,\eta_j\}$。该结论要求这次等待只改变当前任务而不改变任何后续任务。在当前机器 idle 会向后传播的语义下，各任务等待不能分别消元，所以不能在完整模型中统一把 $\alpha_j$ 替换成 $\min\{\alpha_j,\eta_j\}$。完整模型做窗口投影时仍使用原始 $\alpha_j$；等待单价 $\eta_j$ 通过外层 timing 优化改变最终的 $\mathbf C_j$。

### 5.3 分位数是最优性刻画，不是必须显式加入的非线性约束

固定 $\mathbf C_j$ 后，$a_j^*$ 和 $b_j^*$ 在内部情形下是加权经验分位数。这一结论说明最优窗口落在哪些 order statistics 上，但它不是说完整目标只等于某个分位数。投影值 $\Phi_j(\mathbf C_j)$ 还包含窗口位置、宽度和分位点两侧的全部偏差成本。

如果直接写约束 $a_j=Q_{\tau_a}(\mathbf C_j)$、$b_j=Q_{\tau_b}(\mathbf C_j)$，由于样本次序会随排程和等待改变，分位数映射是非光滑且可能集合值的，通常反而需要排序或选择逻辑。没有必要这样建模。保留 $a_j,b_j,E_{j\omega},T_{j\omega}$ 后，原模型就是线性扩展式，求解器会自动把端点放到相应分位点；若将这些变量投影掉，$\Phi_j$ 是关于 $\mathbf C_j$ 的凸分段线性函数，可以写成若干仿射函数的上包络，并用 LP 对偶割逐步表示。

“due window 必须先给定，场景 timing 才能执行”是正确的业务时序，但不要求确定性等价模型按这个顺序逐块求解。对固定排程 $x$，严格的两阶段写法是

$$
Q(x)=\min_{a,b}
\left\{
g(a,b)+\frac1N\sum_\omega q_\omega(x,a,b)
\right\},
$$

其中 $q_\omega$ 在共同窗口给定后优化场景 $\omega$ 的等待与完成时间。由于内外层都是最小化，这与在一个联合 LP 中同时优化共同的 $a,b$ 和全部场景 timing 完全等价。共同窗口没有场景下标，因此并未偷看某个单独场景。2025 风格将 $a,b$ 留在 master 后逐场景求 $q_\omega$；2026 风格则对固定 $x$ 把共同窗口和所有场景 timing 一起投影成 $Q(x)$。两者是不同分解边界，不是不同的随机决策时序。
...
其中 $\rho_j>0,\lambda_j>0,\eta_j>0$，不设 overtime。为与 FW 做干净比较，两个版本使用相同的 $\rho_j$ 和 $\lambda_j$。initial 和任务间主动 idle 均允许；主模型先统一计入 $I$，再单独做 initial-idle 计价敏感性。正位置成本和正等待成本都能提供位置斜率，正宽度成本控制窗口扩张，连续 recourse 保持不变。若有可信的 $[L_j,U_j]$，可再作为业务约束加入，而不是把它当成唯一防退化工具。

该版本相对现有随机 DWA 文献的区别也很清楚：Yue and Zhou (2021) 强制无 idle；本模型允许根据场景主动等待，但显式计算其机会成本。这比添加没有生产语义的 overtime 更自然，也能形成明确的方法贡献。

### 6.2 免费等待基准：位置成本 + 宽度成本 + 场景自适应等待
...
并允许所有场景自适应等待，但令 $\eta_j=0$。正位置成本 $\rho_ja_j$ 用来替代机器语境中缺失的 overtime，防止窗口无限后移。该版本完整保留 wait-and-see timing，适合检验免费等待究竟会把窗口推向何种极端；但其局部 recourse 不再支付 earliness，端点常在 $[0,b]$ 与 $[b,b]$ 之间选择，因此应作为结构基准而不是默认主模型。

### 6.3 范围模型：只有存在业务承诺区间时作为主设定

第三个模型可用 $L_j\le a_j\le b_j\le U_j$ 代替位置成本。为避免免费等待版本总取边界，仍应优先采用正 waiting cost，并配合固定宽度或正宽度成本。范围可从客户可接受 lead time、历史交付承诺或排程周期生成；若只是按 makespan 人为截取，应做多档敏感性分析，并报告多少窗口端点落在边界，否则模型结果可能主要反映边界设置。

### 6.4 不建议作为主模型的版本

不建议使用“自由位置 + 自由宽度 + 仅 waiting cost”，因为窗口会无限放宽；“范围 + 正宽度成本 + 免费等待”可以作为有真实承诺边界的模型，但不能仅凭隔离任务的边界倾向就断言完整序列会取 $[U_j,U_j]$。机器没有明确班次时，不建议为了模仿 TWATSP 人为增加 overtime。

综合而言，最有价值的模型比较不是简单比较三种随意成本，而是比较两种 timing 语义：

1. **FW 模型**：位置成本 + 宽度成本 + SAA-ET，允许免费场景等待；
2. **PW 模型**：相同窗口成本 + SAA-ET + 二阶段主动等待成本，允许付费场景等待。

两者使用相同的一阶段排程、窗口位置成本和宽度成本，唯一核心差异是 $\eta=0$ 与 $\eta>0$，因此能干净识别等待定价的作用。PW 模型中还可做一个删除位置成本的消融，检验正等待成本能否单独锚定位置；但不应让 FW 与 PW 同时改变位置成本，否则无法判断差异来自等待还是窗口成本。若再加入范围模型，应把它作为有业务数据的 B 版本。

## 7. 对 Benders、SAA 与后续 DRO 的影响

上述 FW、PW、B 三类模型只要使用线性 ET、线性位置/宽度/等待成本，固定离散排程后的 timing recourse 都是连续 LP。位置成本只改变第一阶段目标；端点范围只增加简单边界；waiting cost 只改变 recourse 目标系数。它们都不妨碍 ordinary Benders、deepest Benders cuts 或 2025/2026 两种分解边界。

FW 与 PW 都允许等待，且等待会影响下游完成时间，所以完整 timing 必须联合优化。给定实际完成时间向量后，任务特定窗口可以逐任务投影成 $\Phi_j(\mathbf C_j)$；不能逐任务投影的是会向后传播的机器等待。把 early 单价替换为 $\min\{\alpha,\eta\}$ 只作隔离任务诊断，完整序列仍保留原始 ET 系数和 waiting cost。范围约束还能消除连续时间平移方向，通常有利于数值稳定和 cut 强度。
...
| Yue and Zhou (2021), EJOR 290, 453–468 | 随机加工时间、任务特定 DIF 窗口；期望 ET、位置、宽度成本；no idle | 提供分位数投影的文献对照，但 no-idle 不属于当前候选模型 |
...
本轮没有在 Excel 及其代表性 DWA 全文中确认到“任务特定窗口位置和宽度均完全自由、没有任何窗口成本/边界，仅靠二阶段 waiting cost”的同型研究。因而 PW 模型的合理性来自机制推导和给定窗口 timing 文献，而不是某篇完全相同模型。这个证据边界应在后续论文中如实表述。

## 9. 当前建议

现阶段不需要给机器增加 overtime。先把 PW 模型作为主要的两阶段 SAA 问题：任务分配、顺序和 due window 为第一阶段，场景下 timing 与主动等待为第二阶段；保留正位置成本和正宽度成本，并对所有能够主动后移时间线的 idle 收费。再把相同模型中的 $\eta$ 设为 0，形成 FW 免费等待基准。若认为 initial idle 没有等待成本语义，FW 必须继续依靠位置成本或真实范围锚定；PW 若想用 waiting cost 单独锚定位置，则 initial idle 也必须计入。若能获得有含义的任务承诺范围，再加入 B 模型，而不是先随意规定边界。

这样既保留 due window 必须决策的核心，也不会引入当前问题不存在的 no-wait 语义。理论上可以研究 $\eta=0$ 时 early 侧斜率消失、$\eta>0$ 时斜率恢复的值函数变化；计算上可在完全相同的样本、窗口成本和 OOS 场景下比较“免费等”与“付费等”，把差异明确归因于等待定价。
...
这个结论同样有条件。若没有宽度成本，模型反而倾向选择最宽的允许窗口，例如 $[L_j,U_j]$；若有位置成本、正 waiting cost，或等待会显著损害后续任务，最优端点可能位于区间内部。因此文档使用“经常取 $[U_j,U_j]$”，而不是无条件定理。
...
2. 实际 TWA 中等待也是决策，$C_{i\omega}$ 是内生变量；可以在给定实际 $C$ 后投影窗口，但不能提前用 earliest completion 替代最终的 $C$；
...
### 11.3 在始终允许等待时，真正起作用的是 waiting 单价

当前问题不存在禁止等待，所以只比较免费等待与付费等待。仍取三个等概率的最早完成时刻 $4,7,10$，点窗口为 $[d,d]$，令 early 单价 $\alpha=10$、tardy 单价 $\beta=1$。若等待单价为 $\eta$，每个较早场景会在“直接支付 early”与“等待到 $d$”之间选择较便宜者，因此早到一侧的有效单价为
...
| 等待单价 | 有效 early 单价 | $d=4$ | $d=7$ | $d=10$ | 最优点 |
|---:|---:|---:|---:|---:|---:|
| $\eta=0$ | 0 | 3 | 1 | 0 | 10 |
| $\eta=0.1$ | 0.1 | 3 | 1.1 | 0.3 | 10 |
| $\eta=1$ | 1 | 3 | 2 | 3 | 7 |
| $\eta=5$ | 5 | 3 | 6 | 15 | 4 |

这里 $\alpha$ 始终固定为 10，最优窗口却随着 $\eta$ 从 0、0.1、1 增加到 5，依次从最迟样本 10 移到中间样本 7，再移到最早样本 4。因而，在当前问题中不能说“early 成本大，所以取最迟样本”。准确说法是：当 $\alpha>\eta$ 时，模型宁可等待而不支付 earliness，继续增大 $\alpha$ 已经没有作用；若 $\eta$ 很小，晚窗口只产生很便宜的等待，因此可能靠近最迟样本。推动窗口靠后的是等待便宜，而不是 early 单价大。

若 $0<\eta<\alpha$，earliness 可能仍然在局部最优解中为 0，但模型不会退化成零成本，因为等待量按 $\eta$ 计费。若 $\eta\ge\alpha$，仅为了本任务避免 early 而等待没有收益，但等待仍被允许，并可能因为下游任务的窗口而发生。完整机器序列中，一次等待会同时改变所有后续完成时刻，上表只用于解释成本方向，实际 recourse 仍需联合优化。
...
这也解释了经验分位数结构：免费等待时 $\bar\alpha=0$，局部 early 侧完全消失；付费等待时只需把原 early 系数替换为 $\bar\alpha=\min\{\alpha,\eta\}$。当窗口宽度成本为 $\lambda$ 且两个端点不碰撞时，局部左、右端点分别对应经验分位水平 $\lambda/\bar\alpha$ 与 $1-\lambda/\beta$；若两者交叉，窗口收缩为点，其分位水平为 $\beta/(\bar\alpha+\beta)$。这些闭式结果是局部诊断，完整序列仍由联合 timing LP 决定。

### 11.4 宽度成本只能防止窗口变宽，不能阻止窗口整体后移

仍用样本 $4,7,10$。若没有宽度成本，无论等待免费还是付费，窗口都可取 $[4,10]$，三个样本的 ET 和等待均为 0；还可以取更宽的 $[0,100]$。此时窗口宽度没有优化意义。因此，只要宽度是决策，就必须用正宽度成本、固定宽度、最大宽度、服务水平约束或第二目标中的至少一种来控制。
...
这说明在“免费等待 + 线性位置和宽度成本”的局部模型中，两个端点常落到极端结构，而不是都形成丰富的内部决策。若研究目标是让 early、tardy、waiting 和两个窗口端点都真正参与权衡，对主动等待收费更自然。
...
要让范围设定有实际意义，$[L_j,U_j]$ 应来自客户承诺期、产品 lead-time 或真实计划周期，并且最好与正 waiting cost 或固定宽度配合。若 $U_j$ 只是人为给出的一个很大数，它主要起数值截断作用，不会自动产生好的经济权衡。
...
这句话不是说 due window 不再是决策，而是说可以把它作为内部最优化变量投影掉。对 SAA 中任意一组**实际**场景完成时间，一个任务的共同窗口必须服务向量

$$
(C_{j1},C_{j2},\ldots,C_{jN}),
$$

所以窗口投影后得到 $\Phi_j(C_{j1},\ldots,C_{jN})$。其最优端点由场景完成时间的 order statistics 决定，函数值则是位置、宽度和两侧 ET 偏差的最小总成本。它是凸分段线性函数，而不是一个需要显式书写的分位数等式。

若窗口彼此独立且只进入软 ET 成本，给定全部实际完成时间后，$\Phi_j$ 可以对所有任务分别计算。因此完整模型确实可写成“排程与场景完成时间的函数”加 timing 约束。困难不在窗口投影，而在这些完成时间不是给定排程的固定参数：场景等待会沿机器序列传播，使所有 $C_{j\omega}$ 仍需联合优化。由此只能得到

$$
\min_{x,C,I\in\mathcal T(x)}
\left\{c(x)+\frac1N\sum_{\omega,j}\eta_jI_{j\omega}
+\sum_j\Phi_j(\mathbf C_j)\right\},
$$

不能把它进一步错误地化成若干互不相关的单任务 earliest-completion 函数。若再把 $C,I$ 一起投影，才得到只关于离散排程 $x$ 的联合 LP 值函数 $Q(x)$。这正是 projected Benders 使用的对象。

这里还要修正局部等待推导的使用范围：$\min\{\alpha_j,\eta_j\}$ 只适用于等待不向后传播的隔离任务。完整机器序列做窗口投影时使用原始 $\alpha_j$，而 $\eta_j$ 留在联合 timing 目标中。这个局部公式可以解释成本方向，但不能作为所有任务的精确代数替换。

### 11.10 对当前并行机问题最稳妥的两种设定

若目标是先形成一个不退化、便于解释且 due window 确实需要决策的问题，当前建议保留两种有明确差异的模型。

第一种是 FW 免费等待基准。所有 initial 和任务间等待均允许且不收费，目标含正位置成本、正宽度成本和 SAA-ET。位置成本是该版本不可缺少的平移锚；若删除它，就必须改用真实承诺范围。该模型用于展示免费等待怎样削弱 early 并使窗口端点极端化，不应假设它会自然产生充分的双侧 ET。

第二种是 PW 付费等待主模型。所有等待仍然允许，但 initial 和任务间主动 idle 都按 $\eta>0$ 计费；目标使用与 FW 相同的位置成本、宽度成本和 SAA-ET，只增加 waiting cost。局部有效 early 单价为 $\min\{\alpha,\eta\}$；序列上，等待还会影响下游 ET。保持其他成本完全一致，才能把 FW/PW 的差异解释为等待定价效应。

建议以 $\eta/\alpha\in\{0,0.1,0.5,1,2\}$ 做敏感性分析。$\eta=0$ 是 FW；$0<\eta<\alpha$ 时通常以等待替代自身 earliness，但仍支付正成本；$\eta\ge\alpha$ 时不会仅为本任务避免 early 而等待。PW 可额外测试删除位置成本的消融，以确认正等待成本单独锚定位置；但主比较中不应只在一个版本删除位置成本。外生范围只在有真实业务数据时加入。

### 11.11 对 2025/2026 TWA 的逐页核查结论

这次复核的重点不是“它们是不是也有 due window”，而是确认它们到底把哪几个成本放进了模型、时间从哪里开始、waiting 是怎么进入传播的，以及 \(T\) 在 2026 里到底起什么作用。核查结果可以概括成三点。

第一，两篇原模型都包含四类代价：路线成本、窗口宽度成本、场景下的 earliness/lateness、以及 overtime。2025 的记法是 \(\sigma\) 乘窗口宽度、\(\phi\) 乘 \(E+L\)、\(\psi\) 乘 overtime；2026 的记法是 \(\alpha\) 乘窗口宽度、\(\beta\) 乘 overtime、\(\delta\) 乘 \(E+L\)。两篇都没有给窗口开始时间单独加一个位置成本，也都没有把窗口位置限制在某个外生区间里。

第二，时间锚都来自 depot 起点和终端班次长度。2025 的约束 (12) 把 depot departure time 固定在给定时刻 \(t_0\)，2026 的约束 (1h) 明确写为 \(w_{0\omega}=0\)；随后 departure time 按路径向后递推，最终返回约束用 \(T\) 或 \(T+o^\omega\) 收尾。也就是说，waiting 不是独立目标项，但它会进入递推，并最终影响是否触发 overtime。

第三，2026 第 6.2.3 节明确说明了 \(T\) 过大的退化。原文的意思不是“作者随便挑了个 \(T\)”，而是说：如果 \(T\) 足够大，完全可以在每个客户前插入足够等待，让最坏场景也准时，从而把 earliness 和 lateness 都压成 0，最后只剩 routing cost。作者因此要求 \(T\) 必须谨慎设定，否则实例会变得不现实，也不会再体现时间窗与迟到风险的 trade-off。

从当前机器调度问题看，这个结论对应两个彼此独立的退化方向。其一是在没有位置成本、真实范围、付费等待或有效 terminal cost 时，免费 waiting 可以配合自由窗口位置沿序列向后同步。其二是在窗口宽度完全不受控时，模型可以直接扩宽窗口消掉 ET，而不需要把执行时间向后推。要让 SAA 中的 due-window assignment 真正有内容，必须分别保留一种位置锚和一种宽度控制；若还希望 observed earliness 不被 waiting 系统性替代，则应进一步比较 waiting 单价与 early 单价。

更准确地说，position cost 和 width cost 控制的是两个不同方向。前者控制窗口整体平移，后者控制窗口张口大小。把它们写成 \(\rho a+\lambda(b-a)\) 之后，平移 \(a,b\) 同时增加 \(\delta\) 时，直接起作用的是 \(\rho\)，不是 \(\rho-\lambda\)；\(\rho-\lambda\) 只是在“改左端点而不改右端点”这种局部操作里才出现。这个差别很重要，因为它决定了“位置成本比宽度成本小”并不等于位置成本没用，只是说明两者在不同方向上起作用。

为了把这件事和前面讨论接起来，可以把当前问题的非退化条件写成一条很短的检查表：窗口宽度必须由正成本、固定值或上界中的至少一种控制；窗口位置必须由正位置成本、真实范围、有效 terminal cost，或在首任务时刻固定条件下的正 waiting cost 中至少一种锚定。不是任意两个成本项凑在一起都可以，因为 width 与 location 控制的是不同方向。

![](../figures/due-window-nontriviality-framework.png)

这张图可以作为本节的概念摘要：左侧是位置锚，中间是宽度锚，右侧是终端锚；任意一条锚线松掉，模型都会朝着不同方向退化。
