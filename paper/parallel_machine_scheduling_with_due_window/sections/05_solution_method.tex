...
$\mathrm{SC\text{-}M}$ keeps the outsourcing decisions in the master, whereas
$\mathrm{SC\text{-}C}$ invokes a second pricing problem for outsourcing columns.
...
current set of internal columns. For $\mathrm{SC\text{-}C}$, let
...
For $\mathrm{SC\text{-}C}$, exact outsourcing pricing is then performed with the
...
internal column, and, for $\mathrm{SC\text{-}C}$, let
...
selected in $\mathrm{SC\text{-}C}$. The bound is used only when the relevant pricing
certificates are complete. In particular, a certificate for internal columns alone
cannot close a node of $\mathrm{SC\text{-}C}$.
...
continuous completion times directly and therefore does not scale with the numeric
length of the scheduling horizon. Second, dominance compares the complete
time--cost trade-off of two partial sequences rather than a single scalar cost
evaluated at an arbitrarily selected time.
...
Labels are organized by terminal job and a reachable-set key. The complement of
this key combines the jobs retained by ng-memory with jobs that are currently
unreachable under the label's time domain. Keys form a partial order: a predecessor key
represents a state that can dominate labels stored at a successor key. For each
key, the algorithm maintains a sourced lower envelope. Every envelope segment
records the local label that contributes it, or records that it was inherited from
a predecessor key.

When a new label arrives, the existing envelope is first used for a read-only
dominance test. A surviving label is merged into the local envelope. Only the
intervals on which the envelope decreases are propagated to successor keys.
Because segment sources are retained, a local label is removed as soon as its last
contributing interval disappears; the algorithm does not rescan all historical
labels at that key. Under partial dominance, the retained intervals are applied
lazily before a label is expanded or joined. Thus, the normal and partial modes
share the same incremental dominance graph, and partial dominance adds only
functional-domain trimming.

This source-aware implementation is computationally important. A conventional
dominance graph repeatedly rebuilds predecessor envelopes and scans all labels at
affected nodes. The incremental graph instead propagates only geometric
improvements and identifies dominated labels from their disappearing sources.
...
This pricing problem is required only by $\mathrm{SC\text{-}C}$. For any total
...
$\max_q\{V(q)-G(q)\}$. The implementation constructs a Pareto frontier of states
$(q,v)$ by processing jobs one at a time. From each state it creates an exclusion
transition, which keeps $(q,v)$, and an inclusion transition, which produces
$(q+b_j,v+\widetilde\pi_j)$. States with no better dual reward at the same or greater
baseline amount are discarded. Branching decisions that require or forbid
outsourcing membership are enforced during these transitions.

The best negative-reduced-cost sets are recovered from predecessor pointers,
deduplicated, and added to the outsourcing pool. The same dynamic program supplies
the exact outsourcing reduced-cost certificate used in
\eqref{eq:node-dual-bound}.
...
$\mathrm{SC\text{-}M}$, the algorithm first branches on the active tariff segment,
then on the number of selected internal-machine columns, and finally on internal
adjacency or directed-arc decisions. For $\mathrm{SC\text{-}C}$, tariff branching
...
Closure requires the exact internal pricing certificate and, for
$\mathrm{SC\text{-}C}$, the exact outsourcing certificate. The relaxed ng-route
