...
同时在 `normalizeForward()/normalizeBackward()` 的 prefix/suffix-min 后增加一次精确相邻段压缩。这里只合并 slope/intercept 完全相等、且端点连续的相邻段，并清理零长度段，不做近似压缩，也不抬高函数值。因此它只是减少表示碎片，不改变 PWLF 表示的函数。

测试结果明显好于上一版 hull delta。30 任务三角化 `tmp-wet030_from040_010_2m` root-only smoke 中，结果仍为 `obj=bound=14318, valid=true`，耗时从上一轮默认路径约 `solve=12.542s, exact=4.243s` 降到 `solve=9.584s, exact=2.471s`。40 任务 `wet040_001_2m` zero-setup root-only 中，结果仍为 `incumbent=17887, bound=17866.666667, valid=true`，耗时从旧路径约 `solve=188.930s, exact=161.717s` 降到 `solve=136.439s, exact=101.692s`。对比前一版 hull delta 的 `solve=213.237s, exact=172.704s`，说明真正有效的是减少无效 merge 和段表示复杂度，而不是粗粒度 hull 传播。

从日志看，40 zero-setup 第一轮 completion-bound build 约 `12.555s`，内部 merge 次数仍为 `63796`，但大量 no-change merge 在 `mergeMinimum()` 内部提前返回，避免了 copy/normalize 的重成本。后续多轮 build 多数在 `8-13s` 范围内，root exact 总时间从 `160s` 量级降到 `100s` 量级。当前应保留这组优化作为默认路径；多区间 delta 只有在进一步压缩 `cropToInterval()` 和 delta 子函数构造成本后再考虑，否则容易重复上一版 hull 的问题。
