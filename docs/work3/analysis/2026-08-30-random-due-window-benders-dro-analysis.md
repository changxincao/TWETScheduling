...
2. 两篇 TWATSP-ST 论文都没有客户特定的外生窗口位置区间；它们用固定出发时刻、班次长度 T 和正的 overtime 成本锚定自由窗口。机器没有实际加班语义时可以删除 overtime，但若仍保留场景自适应等待，就需用正的主动等待成本、任务可承诺区间或非预见等待替代该锚点；若采用 earliest-start，则窗口位置可以自由，并可按完成时间经验分位数投影。无论采用哪种位置机制，窗口宽度还需单独由正成本、固定值或最大值控制。
