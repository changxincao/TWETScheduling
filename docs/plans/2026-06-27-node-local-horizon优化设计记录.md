...
进一步确认当前主求解是否使用该 local horizon 改进。旧配置名曾写成 `diagnosticLocalHorizonAtNode` / `twet.bpc.fullDomainCompare.localHorizonDiag`，这个命名不准确；该模块本意是尝试用 node-local Cmax 改进 `pricingHorizon`。60-2 已跑日志的 `command.txt` 没有设置相关开关，即使打开，旧实现也只构造 `LocalHorizonCmaxSolver.Problem` 并打印 CPLEX/CP 结果，没有写回 `Data.CmaxH`，也没有修改 pricing engine 内部的 `pricingHorizon`。因此此前 60-2 主求解没有使用 CP/CPLEX local horizon 收缩；它使用的是数据加载后的 `Data.CmaxH`、pricing 内部的 dynamic profitable window，以及 midpoint/probe 等已有机制。
...
已按上述判断修正 Tanaka loader。现在 `loadTanakaMultiMachine()` 在目标算例读完、zero setup 处理完，并设置 `computeSafeHorizon` 后，重新调用 `data.setImprovedCmax()`，之后再构造外包函数、hard window 和 penalty functions。这样目标算例自己的 `SchedulerForReleaseNoWait` 启发式会真正参与主求解 horizon 初始化。反射检查 `wet060_001_2m` 的 loader 输出为 `n=60,m=2,CmaxE=3300.0,CmaxH=3630.0000000000005`，不再是 `5727`。

同时暂停了 `Tree` 中每个 node 开头的 node-local horizon 改进调用，只保留方法本体和注释。原因是分支和 pricingOnly 禁弧会让 node 的可行顺序集合更强，局部 Cmax 通常只会变差；即使用 CP/MIP 在某个 node 上求到可行 horizon，也不稳定改善后续 pricing。当前决定仍使用初始启发式收缩后的全局 `CmaxH`，node-local CP/MIP horizon 改进实验暂不接入。

为避免后续误读，代码命名也已改成改进口径：配置字段改为 `enableNodeLocalHorizonImprovement` 等，runner 属性改为 `twet.bpc.fullDomainCompare.nodeLocalHorizonImprove` 等，Tree 中保留的方法改为 `tryImproveLocalHorizonAtNode()`，日志 tag 改为 `[LocalHorizonImprove]`。当前该调用仍被注释，表示保留方案但不参与主线求解。
