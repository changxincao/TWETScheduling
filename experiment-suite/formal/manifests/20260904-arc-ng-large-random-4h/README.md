...
15:57检查时6个solver和scheduler均正常运行，尚无完成项。当前节点为`n80-set01/set03/set05=9/38/49`、`n100-set01/set03/set05=29/17/24`；认证全树gap分别为`2.0670%/2.0115%/1.8965%`和`4.4214%/2.4172%/4.7395%`。累计exact平均耗时仍稳定在`9.990/1.047/1.245/2.421/1.731/2.062秒`，平均DSSR轮数`2.279/1.707/1.896/1.638/1.871/1.877`。与15:47相比节点推进很快而gap仅小幅下降，当前主要耗时已由family中的单次pricing爆炸转为Arc分支树的逐节点收缩。

针对random上“pricing快但Arc树收缩慢”的后续分支候选，当前优先考虑任务后继集合分支。对任务`i`按LP outgoing flow将正流量后继划分为两组`A`和补集，使`sum_{j in A} x_ij`接近`0.5`，左右支分别取`<=0`与`>=1`；前驱集合可作为独立后续变体。它一次切断一组可替代Arc，同时仍是整数有向arc-flow行，可直接复用`AggregateArcBranchConstraint`并由现有arc-dual pricing处理，不增加ng-DSSR标签资源。候选应以少量准入名额进入现有strong testing并与Arc按同一score竞争，不采用严格类型优先。Ryan-Foster/同机分支需要额外共现状态，完成时间分支改变pricing资源语义，现阶段不作为低侵入首选。
