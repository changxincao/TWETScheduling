...
2026-09-06在途n40配对快照：当前只有set01的27个random物理场景（m2/m3/m4、base/medium/high、zero/narrow/wide）三算法完整配齐，81条结果均为最优、目标一致且验证通过。NG-DSSR/TI/TI+SRI平均总时间为50.090/82.923/105.765秒，中位数为43.165/50.825/52.982秒，逐场景最快次数为21/2/4；平均节点数为14.11/19.52/2.85，平均根gap为0.4035%/0.6970%/0.1095%。按时间尺度，三者从base到high的平均时间分别由20.690增至88.681秒、36.908增至156.796秒、26.113增至224.933秒；按窗口，wide下分别为69.829/136.792/212.830秒，明显高于zero的40.572/57.226/51.445秒。最难配对点n040-set01-m2-random-high-n20-wwide为169.311/641.672/1039.150秒，节点16/44/4，说明SRI显著压缩树和根gap，但rank-1 pricing成本超过树缩减收益。family尚不能做配对均值：m2/base/zero的NG-DSSR和TI+SRI分别在根节点597.683/657.457秒闭合；m3/base/zero的NG-DSSR用1702.651秒、239节点闭合，其余对应TI运行仍在进行。该段只用于在途机制判断，不是n40最终跨set汇总。

n40 random的时间尺度与窗口交互：以下每格为m2/m3/m4三个场景的平均总时间，列顺序为zero/narrow/wide。NG-DSSR在base为25.864/14.733/21.472秒、medium为32.401/43.685/46.611秒、high为63.449/61.188/141.405秒；普通TI分别为46.998/31.876/31.851秒、42.413/61.414/61.367秒、82.267/70.962/317.158秒；TI+SRI分别为40.857/17.183/20.299秒、38.465/67.861/92.419秒、75.012/74.013/525.773秒。base下窗口效应被小树的离散波动掩盖；medium开始出现wide惩罚；high+wide形成明确交互，wide相对同尺度zero分别放大NG-DSSR/TI/TI+SRI约2.23/3.86/7.01倍。对应平均根gap由zero的0.2335%/0.3977%/0.0352%增加到wide的0.7308%/1.1910%/0.2419%。因此时间放大主要增加一次pricing的状态域，窗口变宽同时削弱松弛并扩大树；二者同时出现时，TI图规模与SRI residual状态代价叠加，不能把两项影响看成简单相加。

上述总时间不能解释为TI对时间尺度不敏感。正式medium/high不是同一base实例的统一10/20倍复制，而是processing与center分别使用独立逐任务倍率；同时总求解时间还混合了pricing调用数、树规模和RMP固定开销。组件统计显示，普通TI每次pricing在base/medium/high三个尺度上平均约为9.2/74.3/150.6毫秒，实际已近似放大8.1/16.3倍；平均pricing总时间约为3.65/25.41/94.63秒，而总时间为36.91/55.07/156.80秒。TI+SRI每次pricing约为19.7/138.4/634.3毫秒，高尺度均值受high+wide的1339.6毫秒长尾影响。m2-zero首次TI日志的compact horizon为3663/34952/70420，也直接显示约9.5/19.2倍放大。因此不成比例的是端到端总时间，不是time-indexed单次计算；base时总时间主要由树和master等固定工作构成，medium/high后pricing占比才迅速上升。
