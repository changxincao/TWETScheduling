...
2026-08-27又完成了第二个matched task set和两组完整BPC。set02 random的no-cut time-indexed在41.487秒、22个节点完成，family则在900秒时仍有4.088% gap、173个待处理节点和1796776条列，其中1796664条非基本；但family root本身只用3.696秒，快于random的11.652秒。它进一步确认family的主要问题是pseudo-schedule set-covering松弛变弱，而不是time-indexed DAG单次求解变慢。单点due date也不是elementarity保证：重复job的列系数和dual收益都会按visit count累计，分数列可以利用系数2取得非法的覆盖效率；random setup通常使这种重复不划算，family低setup块则系统性降低其边际代价。完整机制和ng-DSSR对照见前沿专题第36.27节。
