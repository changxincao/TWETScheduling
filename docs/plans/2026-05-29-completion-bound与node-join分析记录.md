...
本轮把 `HeuristicPricingEngine` 的 profile 构造改为可读取当前 node 继承的 time-indexed compact window。启发式搜索内部现在使用的窗口为：基础 hard window、由 time-indexed reduced-cost arc fixing 继承下来的 node compact window 二者的交集；root dual profitable window 不进入启发式窗口上下文，因为它依赖当前 dual，只适合 exact pricing 的 reduced-cost 剪枝，不应持久化或影响全局列成本。启发式使用该窗口后，会同步缩小单任务 penalty profile、首任务定义域和本次局部搜索 horizon，从而减少 tabu move 中的无效函数拼接和候选搜索空间。

返回列时仍然按原始 TWET objective 重新计算一次 sequence 成本，并用当前真实 dual 重新判定 reduced cost。这样做是为了保持全局 `Pool` 的不变量：`TWETColumn.cost` 必须是 sequence 的真实列成本，而不能带有某个 node 的局部 hard window 或 effective window 口径。理论上，time-indexed compact window 是对子树安全的硬剪枝信息，窗口外完工时间不应出现在优于 incumbent 的解中；但由于当前列池只按 `SequenceSignature` 管理列，不保存“该 sequence 在哪个窗口下优化”的上下文，保守做法仍是搜索阶段用窗口剪枝、入池阶段刷新真实成本。另一种“只使用非 dual window 的硬窗且不刷新成本”的做法暂不采用。
