...
第二，时间锚都来自 depot 起点和终端班次长度。2025 直接把 depot departure time 固定为 0，2026 也是把 \(w_{0\omega}=0\) 作为约束之一；随后 departure time 按路径向后递推，最终返回约束用 \(T\) 或 \(T+o^\omega\) 收尾。也就是说，waiting 不是独立目标项，但它会进入递推，并最终影响是否触发 overtime。
...
从当前机器调度问题看，这个结论直接对应两类退化风险。其一是没有 overtime 或者 \(T\) 太松时，waiting 可以把早到全抹掉，窗口只剩路由成本的陪衬。其二是窗口宽度免费时，窗口会被直接拉得很宽，甚至可以把位置整体后移。要让 SAA 真的有意思，至少需要一个位置锚和一个宽度锚；如果再想避免“早到项消失”，还要给 waiting 或终端超时留出真实代价。
...
为了把这件事和前面讨论接起来，可以把当前问题的非退化条件写成一条很短的检查表：窗口位置不能完全自由，窗口宽度不能完全免费，waiting 不能完全无代价，终端时间也不能完全松。四者里只要至少有两类锚同时存在，模型通常就不会退化成“只剩路由”；如果四类都放开，SAA 很容易只剩一个形式上的窗口层。
