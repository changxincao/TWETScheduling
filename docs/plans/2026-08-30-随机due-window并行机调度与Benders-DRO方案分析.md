...
### 1.2 当前模型：付费等待把 early 系数变为 $\min\{\alpha,\eta\}$

当前始终允许场景自适应等待。为看清等待单价的作用，先隔离一个任务，并用 $\underline C_j^\omega$ 表示该任务在场景 $\omega$ 中不额外等待时的最早完成时刻。给定窗口 $[a_j,b_j]$ 后，该任务的局部 recourse 为
...
免费等待 $\eta_j=0$ 会令 $\bar\alpha_j=0$，局部 earliness 完全失效；付费等待 $\eta_j>0$ 则恢复正的早到侧斜率。当 $\eta_j<\alpha_j$ 时，模型选择等待而不是支付 early，但等待本身仍产生 $\eta_jI_{j\omega}$，所以不会再免费同步。
...
这说明当前问题仍有 order-statistic 结构，但必须使用有效系数 $\bar\alpha_j$。机器序列中，等待任务 $j$ 会移动所有后续任务，所以上式只用于单任务诊断；完整模型必须联合优化所有任务和所有场景的 timing。宽度仍需正成本、固定值或真实上界，否则可用覆盖所有完成时刻的宽窗口同时消掉 ET 与等待。
