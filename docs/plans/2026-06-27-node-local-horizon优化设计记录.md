...
后续复核发现，当前 `Data` 构造函数在读取文件头后调用 `debug_set()`，其中会强制设置 `n=60,m=3,scale=3`。因此凡是走 `new Data(path,...)` 的 BPC 主求解，即使文件名和文件头是 `60-2 / 60 2`，实际也会按 3 台机器继续求解；而 `LocalHorizonCmaxSolver` 的轻量 loader 没走 `Data.debug_set()`，仍按文件头 2 台机器求解。这意味着前面对“60-2 BPC 求解”和“local horizon 2 台机器模型”的直接对比口径不一致。后续分析 60 规模结果前，应先移除或开关化 `debug_set()`，否则历史 `60-2` 求解很可能实际是 `60-3` 口径。
