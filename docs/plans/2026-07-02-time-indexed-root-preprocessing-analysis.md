...
但端到端净收益暂不成立：无诊断基线 exact 为 `33.116s`，BitSet prefilter 为 `33.400s`；总求解分别约 `81.254/89.220s`，后者还包含明显的外层运行波动。也就是说，group-group 过滤确实省掉了约一秒 join，但当前 join 只占较小部分，收益被 initialization/forward/backward 波动覆盖。因此该开关继续默认关闭，先保留为 join 更重算例的实验选项。由于 group-group 已剪掉约 98% group-pair，继续加 group-label envelope 的理论空间只剩约 84 万次直接 funcEval，当前不值得再增加一层构造和查询。
