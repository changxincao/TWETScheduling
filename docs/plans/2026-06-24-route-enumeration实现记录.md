...
这里不再把 `G(x)` 或 `G(x+b_j)` 返回 BigM 作为“保守不剪”的正常情况处理。外包 tariff 的定义域应覆盖 `B=0` 和所有可外包任务的 `sum b_j`，默认构造函数也按这个上界生成最后定义域。因此 suffix bound 中出现 BigM 说明数据或 tariff 构造不满足基本约束，应直接暴露为异常，而不是静默保留 label 把问题藏到后续求解里。
