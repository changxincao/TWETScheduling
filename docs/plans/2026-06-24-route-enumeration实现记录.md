...
2026-06-25 复查时补齐了外包 membership 分支在 finite master 中的重建。显式外包变量模式下，`OUTSOURCE_REQUIRED/FORBIDDEN` 分别转成 `y_j=1/0`；外包列模式下，转成包含该 job 的外包列 `omega` 之和等于 `1/0`。这和内部列上的 arc/adjacency 分支一样，都是根据当前 `Node` 状态重新建到临时 MIP 中，而不是复用当前 RMP 的约束对象。
