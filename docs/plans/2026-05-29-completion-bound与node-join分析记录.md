...
半域实现还放大了这个问题。branch node 下 `piWindow=disabled`，`pricingHorizon` 回到完整 `CmaxH`，backward 第一层真实 job label 又被 completion-bound reduced-cost cutoff 全部剪掉，crossing join 基本消失；但是 forward label 仍可以通过 `forward->sink` 直接生成完整列。这样一来，问题从“少边后的双向拼接”退化为“在剩余图上枚举大量能直接接 sink 的 forward-only 负列”。这解释了为什么 subtree 后 completion-bound DP 构造更快、arc 更少，但 final exact pricing 反而从 `45` 条负列变成 `69952` 条负列。
...
### 41.37 2026-06-05 回到经典 PaperDominanceGraph 路径

用户复核后认为 indexed backend 改动较多但端到端收益不稳定，因此当前不继续使用这批新增结构。代码层面将 `PaperDominanceGraphs` 统一入口改回只创建原 `PaperDominanceGraph`，统计也只读取原 DFS backend；即使外部仍传入 `-Dtwet.bpc.paperGraphBackend=indexed`，也不会切到 `IndexedPaperDominanceGraph`。复制版实现暂时保留在仓库中，作为已验证过但未采用的实验记录，后续若要继续做对照可以再恢复开关。

这个回退后的求解路径等价于经典方案：`GC/GCBidirectional/GCNGBBStyleBidirectional` 等 pricing 代码仍通过 factory 创建 dominance store，但 factory 内部固定返回旧 DFS graph。这样避免重新大面积改动多个 pricing 类，同时消除了误开 indexed backend 的风险。当前结论也相应调整为：集合索引和 set-trie 方向已尝试，未证明有稳定端到端收益；后续主要优化对象应回到 label 数量、dual 结构、completion bound 和 subtree 对 pricing 状态的影响，而不是先把 indexed backend 作为默认加速手段。

### 41.38 2026-06-05 ng-set 与后续 dominance graph 优化关系

当前不继续纠结 PaperDominanceGraph 的集合查询结构。前面的实验说明，单纯给 full reachable-set graph 加通用 superset/subset 索引不一定稳定，因为当前 DFS 已利用 inclusion DAG 的 roots 和后继单调性做了较强剪枝；真正让它爆炸的是 label 数量和 reachable-set cardinality 变大后，graph node 本身进入组合爆炸区。

因此后续更自然的方向是 ng-set。若 label 的状态集合从完整 visited/reachable set 改为受 ng-neighborhood 控制的较小集合，同一个 terminal job 上的 dominance graph 会天然变小：reachable key 的 cardinality 降低、key 种类减少、subset/superset 关系更容易命中，PaperDominanceGraph 的 DFS 查询压力也会随之下降。当前结论是，集合查询数据结构先不作为主线；后续应优先看 ng-set 对 label 数、dominance graph node 数和 exact pricing 时间的整体影响。

### 41.39 2026-06-05 completion bound/subtree 接入范围复核

本轮先复核当前几个双向 pricing 路径对 completion bound 和 subtree 的接入情况。当前正式主线仍是 half-domain 的 `GCNGBBStyleBidirectional`，它已经支持 `completionBound=allCycles/twoCycle`、completion-bound arc fixing、以及最后一轮 hard-window bounds 给 subtree arc elimination 复用。`GCBBStyleBidirectionalFullDomain` 和 `GCBBStyleBidirectionalFullDomainNodeJoin` 原本内部已经有 completion bound 剪枝，但 pricing engine 没有把最后一轮 bounds 暴露给 `Tree/PC` 的 subtree 复用通道；本轮补上 `reusableSubtreeArcEliminationBounds()` 和 engine 级缓存，使 full-domain arc-join、full-domain node-join 与 half-domain 主路径在 subtree 复用口径上保持一致。

这次没有把旧的单向 `GC`、旧 `GCBidirectional` 或 `GCBBAsymmetricBidirectional` 继续补齐。原因是这些不是当前 completion-bound/subtree 评估的主要路径，其中部分代码已经带有“未维护/实验旧路径”的语义。把 subtree 通道继续扩到这些路径只会扩大维护面，当前没有必要。后续如果重新启用某条旧路径，再按同样接口补 `PreparedBounds` 即可。

### 41.40 2026-06-05 subtree 对照口径修正和 012 复核

用户指出，012 这类样例若本来就会跑到时间限制，就不能拿“求到时间限制”或“没有收敛”做 subtree 优劣比较；若要比较局部节点负担，应统一 `maxNodes`，若要比较完整搜索能力，应统一时间上限。这一点是对的。此前把 `012` 的 `maxNodes=2` 结果和 10 分钟完整搜索口径混在一起并不严谨，后续记录分开看。

在 `maxNodes=2`、half-domain、`completionBound=allCycles` 的局部节点口径下，012 四组均能返回且 `valid=true`。off 为 `solve=22.984s, exact=7.820s, incumbent=13453, cols=14445`；pricingOnly 为 `28.428s, exact=9.205s, incumbent=13453, cols=14445`；formal 为 `22.346s, exact=7.172s, incumbent=14122, cols=9191`；formalSkipFilter 为 `24.878s, exact=9.273s, incumbent=14122, cols=13930`。这个口径只能说明前两个节点的局部负担：formal 降低了列数和 exact 时间，但短节点预算下上界变差；pricingOnly 没有改善 012 的前两个节点耗时。

随后检查旧记录，没有找到 012 的完整收敛或 10 分钟口径 CSV，只有上述 `maxNodes=2` 结果。因此按要求补跑 012 的 pricingOnly 和 formal/on 两组，统一 600 秒外层时间上限，completion bound 打开、`maxNodes=20000`。两组都在 600 秒内没有写出 CSV：pricingOnly 在 `601.573s` 被外层杀掉，formal/on 在 `600.928s` 被外层杀掉。由于 `Start-Process` 杀掉的是启动进程，JDK 子进程没有自动退出，本轮随后手动清理了两个残留 Java 子进程；这不改变判断，即两组在 10 分钟内都没有到达测试入口写 summary/CSV 的阶段。

因此 012 当前不能作为“subtree 可以快速求到收敛”的正例。它能说明两件事：第一，`maxNodes=2` 和完整搜索是两个不同问题，前两个节点能在 20 多秒返回，不代表后续节点能持续推进；第二，在 012 上 pricingOnly 和 formal/on 10 分钟都没有写 CSV，当前没有证据支持某个 subtree 方案能稳定解决完整搜索。后续若要继续分析 012，应打开 node progress summary 或阶段 heartbeat，用固定时间内处理到的 node、当前节点阶段和 pricing 组件耗时来判断卡点，而不是继续盲等收敛。

结合 004/005/010/011/013 的已有 `maxNodes=2` 结果，当前策略判断保持不变：`pricingOnly` 仍是最稳的默认使用方式，因为它不改变 RMP 分支行和 child 列过滤；formal/on 可以在 005、部分 013 这类样例中降低 label 和 exact 时间，但在 004、010、011、012 上可能表现为上界变差、RMIH 变重或完整搜索没有推进证据。`formalSkipFilter` 仍只适合作诊断，不适合作为默认策略。

### 41.41 2026-06-05 012 的 10 分钟口径更正和正式对照

上面 41.40 中“pricingOnly 和 formal/on 两组都 600 秒没有写 CSV”的判断需要更正。那次运行使用 `Start-Process` 包装 Java，杀掉外层进程后 JDK 子进程没有同步退出，stdout/stderr 也没有可靠落盘，因此不能作为 10 分钟口径的有效结果。随后改用 `D:\软件\Java\jdk_22\bin\java.exe` 直接启动 Java 进程，并由同一个 PowerShell 包装器等待 600 秒；超时则按实际 Java PID 清理，避免残留进程干扰后续计时。

新的 `pricingOnly` 结果为有效结果：`tmp-subtree-10min-20260605b-tmp-wet030_from040_012-pricingOnly`，half-domain、`completionBound=allCycles`、formal subtree 关闭、pricing-only subtree 打开、`maxNodes=20000`。该配置没有触发 600 秒上限，而是在 `260.227s` 正常结束，状态 `FINISHED`，`incumbent=bound=13360`，`valid=true`，处理 `10` 个节点，生成列/列池为 `17380/17380`。耗时大头是 exact pricing：`222.973s/27 calls`；heuristic pricing 为 `22.180s/102 calls`，RMIH 为 `8.270s/8 calls`，subtree elimination `5 calls` 共固定 `1247` 条 pricing-only arc，耗时约 `0.989s`。因此 012 不是 pricingOnly 下 10 分钟仍完全不动；它可以在当前口径下收敛，但收敛时间主要被后续节点 exact pricing 消耗。

pricingOnly 的慢节点集中在 node 7 和 node 9。node 7 用时 `111.912s`，其中 exact pricing `108.969s/2 calls`，最后一轮 exact 有 `fw kept/dominated=332142/36318`、`forwardExtend candidates=5,554,199`、`completionBound fwPruned=1,349,670`。node 9 用时 `106.055s`，其中 exact `101.749s/4 calls`，最后一轮约为 `fw kept/dominated=219589/19401`、`forwardExtend candidates=3,723,151`、`completionBound fwPruned=899,721`。这说明 pricingOnly 的主要问题不是 subtree 构造成本，也不是 RMIH，而是某些后续节点的 forward label/dominance graph 规模仍然很大；不过它最终能耗尽队列并关闭节点。

新的 formal/on 结果仍是不稳定的负例：`tmp-subtree-10min-20260605b-tmp-wet030_from040_012-formal`，formal subtree 打开、pricing-only subtree 关闭。该配置在 600 秒等待期内没有返回，后续因当前 .NET 版本不支持 `Kill(true)` 又手动杀掉 Java PID，包装器记录总 elapsed 为 `679.173s`，没有写出 CSV。它前 7 个节点只用 `38.536s`，随后卡在 node 8。node 8 的诊断摘要为 `depth=4, seed=2278, arcForbid=721, pricingOnlyArc=0, adjReq=1, adjForbid=3`，初始正式列过滤从 `2278` 降到 `1102`，heuristic pricing 补到 `1809` 后进入 exact pricing。被杀前仍停在 `forward.progress`，最后记录约为 `fwQueue=1,002,083`、`fwPops=1,067,454`、`fwKept=2,076,963`、`fwDom=1,210,261`、`forwardExtend candidates=20,850,701`、`constructed=4,717,002`、`boundSurvivors=3,287,223`、`cbFPruned=1,429,779`，尚未进入 backward/join。这个节点不是简单“慢一点”，而是 formal permanent arc forbid 改变子节点列池和 dual 后，把 forward exact pricing 推入百万级 label 队列。

由此当前 012 的结论应改为：在当前代码和同一 600 秒外层口径下，`pricingOnly` 是可收敛且有效的，`formal/on` 在 node 8 出现明显爆炸并超时。`maxNodes=2` 口径仍只说明前两个节点局部行为，不能代表完整搜索；但若以 012 作为 subtree 使用策略证据，当前它反而支持“默认使用 pricingOnly，不默认 formal permanent arc elimination”。formal 可以在少数实例早期节点减少列数或 exact 时间，但一旦它通过 `arcForbid` 进入 child column filter 和 RMP 兼容性语义，就可能把原有列池打散，触发 residual graph 冷启动和更坏的 dual/pricing 结构。

### 41.42 2026-06-05 pricingOnly/forbidden 分支在 heuristic pricing 中的口径统一

本轮复核分支和 subtree 禁弧在 heuristic/exact pricing 中的使用口径。此前主 exact `GCNGBBStyleBidirectional` 已经通过 `isPricingArcForbidden()` 同时检查正式禁弧、pricing-only subtree 禁弧和本轮 completion-bound local arc fixing；但 `HeuristicPricingEngine` 只检查 `node.isArcForbidden()`，因此 pricingOnly subtree 禁掉的 arc 仍可能被 heuristic tabu 搜索用于生成候选列。这个口径不一致会带来两个问题：第一，pricingOnly subtree 已经希望当前 pricing 图避开这些 arc，heuristic 却可能先把这类列加入 RMP；第二，后续 exact pricing 和 completion bound 都在较小图上工作，但 heuristic 仍在较大图上做局部搜索，诊断时容易把 subtree 效果混淆。

代码上已把 heuristic 的序列兼容性、remove/add/exchange move 兼容性统一改为 `node.isArcForbidden(from,to) || node.isPricingOnlyArcForbidden(from,to)`。因此 pricingOnly subtree arc 现在也会被 heuristic pricing 避开。这里没有把 completion-bound local arc fixing 接进 heuristic，因为 local fixing 是 exact pricing 某一轮内部构造 completion bound 后得到的临时状态，不写入 `Node`，heuristic 在 exact 前执行时本来就没有这份状态。

关于 forbidden adjacency，用户前面对“heuristic 没有硬编码 adjacency”的担心需要修正：当前 `Node.isArcForbidden(from,to)` 已经包含 `isAdjacencyPairForbiddenArc(from,to)`，也就是说 forbidden adjacency pair 的两个方向都会被当成 forbidden arc。因此只要 heuristic/exact 调用了 `isArcForbidden()`，就已经硬避开 forbidden adjacency。required arc 和 required adjacency 不应简单硬编码成“每条 pricing 列都必须包含”，因为它们是 master 里的覆盖型分支行，语义是最终若干列组合满足该分支；单条新列不包含 required arc 仍可能有用。当前 required 信息通过 LP 分支行 dual 进入 `LP.getArcDual()`，再进入 reduced cost，最终能否取正仍由 RMP 的 required 行控制，这个口径是合理的。

为了后续复盘 012 formal/on 的 node8 爆炸，给 `GCNGBBStyleBidirectional` 增加了默认关闭的 pricing snapshot。使用方式是设置 `twet.bpc.pricingSnapshotNodeId=<nodeId>`，可选 `twet.bpc.pricingSnapshotDir=<dir>`。触发后会在 exact pricing 初始化时写出四个文件：`summary.txt` 记录节点摘要、机器 dual、job-job 定价禁弧/允许弧数量；`job-duals.tsv` 记录每个 job dual；`arcs.tsv` 记录 source/job/sink arc 的正式禁弧、pricingOnly 禁弧、综合 pricing 禁弧和 arc dual；`columns.tsv` 记录当前 restricted columns 的成本、长度、序列以及是否包含正式/pricingOnly/pricing 禁弧。默认不开启，因此不影响正常实验。

用该快照复抓 012 formal/on 的 node8，文件位于 `test-results/bpc/pricing-snapshots-012-formal/pricing-node-8-1780675676346-*`。快照显示 node8 为 `arcForbid=721, adjReq=1, adjForbid=3, restrictedColumns=1809, machineDual=-11585.875`，job-job 定价边为 `forbidden=727, allowed=143`，其中 pricingOnly 为 0。也就是说“720 多条禁弧后只剩一百多条 job-job arc”这个观察是对的，但它不能推出 label 数应该很少。label 不是 arc，而是“路径前缀 + visited/reachable 集合 + 分段函数 envelope”的状态；剩余 143 条边里仍有若干出入度较高的核心 job，例如允许出度最高的 job 为 `5:10, 14:9, 22/28/25/2:7`，允许入度最高的 job 为 `18/22/5/17:10`。在这种子图上，简单路径和 reachable-set 组合仍然可以指数增长。

快照里的 dual 也解释了为什么删弧后仍会产生大量 forward label：machine dual 为很大的负数，定价 reduced cost 中 `cost - machineDual - jobDual - arcDual` 等价于给每条内部列一个很大的初始补偿；同时 job dual 仍有多个高值，例如 `job1=2707.58, job12=2068.625, job30=1950, job2=1824.17`。因此很多不同 forward prefix 在当前 dual 下都可能保持负 reduced-cost 潜力，completion bound 虽然剪掉大量扩展，但剩余状态仍能保留并进入 dominance graph。这和旧日志中 node8 的 `fwKept` 超过 200 万、`forwardExtend candidates` 超过 2000 万是一致的；不是 bound 构造死循环，也不是“禁弧少算了”，而是 formal permanent arc forbid 改变 child RMP/列池/dual 后诱发的 pricing 状态爆炸。

### 41.43 2026-06-06 heuristic pricing 禁弧处理复核

本轮进一步复核 heuristic pricing 对禁弧的具体处理方式。`HeuristicPricingEngine` 不是在搜索后再统一删掉非法列，而是在三个入口提前判断并跳过非法状态：第一，tabu 搜索开始时，seed sequence 会调用 `isSequenceCompatible()`，若 source->first、内部相邻 job->job、last->sink 任一 arc 被正式禁弧或 pricingOnly 禁弧，则该 seed 直接不搜索；第二，remove move 删除某个 job 后会产生一条 bridge arc `prev->next`，若这条 bridge arc 被禁，则该 remove move 返回 invalid，不会计算 merge2 成本，也不会进入候选 move 比较；第三，add/exchange move 会产生 `prev->job` 和 `job->next` 两条新 arc，任一被禁则该 move 返回 invalid，不会计算 merge3 成本。这样做的结果是 heuristic 搜索过程中始终保持当前序列在 pricing 图上可行，而不是生成后再修。

因此，接入 pricingOnly 后会改变 heuristic 能找到的列集合，这是预期行为。以前 heuristic 可能生成使用 pricingOnly subtree arc 的负 reduced-cost 列；现在这些列会被跳过。这个变化和 pricingOnly subtree 的定义一致：pricingOnly arc 虽然不进入 master 分支行，也不用于历史列兼容过滤，但当前 pricing 图应该避开它。若因此某轮 heuristic 少生成列，exact pricing 仍会在同一 pricing 图上补列或证明无负列，不会破坏定价正确性。

当前 exact pricing 中所谓“最终序列兼容性检查”容易误解。主 exact 的 forward/backward 扩展和 crossing join 本身已经通过 `isPricingArcForbidden()` 检查正式禁弧、pricingOnly 禁弧和本轮 completion-bound local arc fixing；因此正常情况下非法 arc 不会进入 label。`tryGenerateColumn()` 里的 `isSequenceCompatible()` 只在 `debugBPCPricingColumnCheck=true` 时做最终兜底，用来在调试复核时避免不兼容序列进入候选，不是正常路径的主要过滤器。上次记录中“最终序列兼容性检查也同步用了 pricing 禁弧口径”的准确含义是：这个 debug 兜底现在和扩展阶段使用同一套 pricing 禁弧判断，避免诊断口径不一致。
...
随后按用户建议把过滤位置前移到 seed 选择阶段。现在 `collectSeedColumns()` 遍历 active restricted columns 时，先用当前 pricing 图检查整条序列；若列含正式禁弧、forbidden adjacency 或 pricingOnly 禁弧，就不计算 reduced cost，也不进入 seed top-K 堆。这样最终选择的是“满足 forbidden arc 口径的 active columns 中 reduced cost 最小的 K 条”，而不是先选全局 top-K 再让不兼容 seed 在 `tabuSearch()` 入口被跳过。`tabuSearch()` 入口的兼容性检查仍保留为防御兜底。

### 41.44 2026-06-06 seed 入池前禁弧过滤的短测影响

按用户要求，用 `tmp-wet030_from040_012_2m` 做一次局部 A/B 测试，口径为 half-domain、`completionBound=allCycles`、formal subtree 关闭、pricingOnly subtree 打开、`maxNodes=2`。对照组不切换工作区，而是把提交 `44a5f94` 的 `HeuristicPricingEngine` 单独编译到覆盖 classpath 中；当前组使用 HEAD，即 seed 入池前先过滤 pricing 图不兼容的 active column。
...
### 41.45 2026-06-06 seed 排序前缀过滤优化

继续按用户建议优化 seed 选择。原来的全量预过滤做法是：遍历所有 restricted columns，先对每条列做完整 `isSequenceCompatible()`，再用 size=K 的 `PriorityQueue` 保留 reduced cost 最小的 K 条兼容 seed。这个语义干净，但在 node2 restricted 已有一万多列时，每次 heuristic pricing 都要额外扫大量序列禁弧。

本轮改成：先对所有 restricted columns 计算 reduced cost 并排序，排序主键为 reduced cost，次键为列长度，最后用 column id 做稳定 tie-break；随后从排序后的前缀开始检查 pricing 兼容性，找到 K 条兼容 seed 即停止。这样仍然不会让不兼容 seed 进入最终 top-K，但兼容性检查只发生在 reduced-cost 排名前缀上，而不是对所有列全量检查。主问题列正常不应为空，因此 seed 收集主流程不再单独做 `isEmpty()` 判断；底层 `isSequenceCompatible()` 仍对空序列返回 false，作为函数自身语义。

中间还试过“当前 LP 正值列优先”。这个策略不适合作为默认：在 `tmp-wet030_from040_012_2m`、half-domain、`completionBound=allCycles`、pricingOnly subtree、`maxNodes=2` 下，正值列优先会把前两个节点的 incumbent 拉到 `13771`，明显差于原来的 `13453`。原因是正值列会挤掉 reduced cost 更低的 seed，导致 heuristic 走到另一条列生成路径。因此最终没有保留正值优先，只保留 reduced cost 主排序。

最终版本同口径测试结果为 `NODE_LIMIT, incumbent=13444, bound=13259.557377, solve=14.875s, valid=true`。作为对照，旧覆盖版为 `solve=20.581s, incumbent=13453`，全量预过滤版为 `solve=24.169s, incumbent=13453`。最终版本 node1/node2 的 heuristic pricing 分别为 `3.381s/19/add4891` 和 `1.593s/11/add236`，exact pricing 分别为 `2.250s/3/add94` 和 `1.733s/3/add697`。当前结论是：排序前缀过滤明显降低了 seed 兼容性检查开销，并且本例短测没有损害上界，反而略好；但由于 seed tie 顺序改变，列池路径也会改变，后续若做更大批量测试，需要按完整搜索或统一 node/time limit 再确认稳定性。

### 41.46 2026-06-06 seed 排序前缀过滤的多样例复核

继续用 `010/011/013/014/015` 做同口径复核，均为 half-domain、`completionBound=allCycles`、pricingOnly subtree、`maxNodes=2`。比较三版：`old44` 为 `44a5f94` 覆盖 classpath，`prefilter90` 为 `90fd482` 的全量预过滤版，`sorted` 为当前 reduced-cost 全排序后扫描兼容前缀版。结果表明，`old44` 和 `prefilter90` 在这些样例上基本走同一列生成路径，差别主要是运行波动；`sorted` 会明显改变 seed 集合和列池路径。

具体结果为：`010` 上 old/prefilter 都为 `incumbent=16444`，sorted 为 `16517`，虽然 solve 从 `27.100s` 降到 `24.593s`，但上界变差。`011` 上 old/prefilter 都为 `14258`，sorted 为 `13935`，solve 也从约 `20.6s` 降到 `17.746s`，这是正例。`012` 上 sorted 为 `13444`，好于 old/prefilter 的 `13453`，且 solve 明显更低。`013` 上 old/prefilter 都为 `14433`，sorted 为 `14474`，上界变差且 solve 略慢。`014/015` 根节点已经整数，三版都能得到相同最终目标；`014` sorted 更慢，`015` sorted 略快。

因此，不能把 012 的改善解释为“新策略理论上比原始策略好”。更准确的解释是：当前 heuristic pricing 存在大量 reduced-cost 接近或相等的 seed，seed tie 顺序会影响 tabu 搜索起点，进而影响补列集合、RMIH 上界和 exact pricing 后续负列。sorted-prefix 在 012/011 上选到的 seed 路径更好，所以表现更好；但在 010/013 上同样的路径变化会损害上界。它的稳定收益只在“少做全量 compatibility scan”这一点上成立，解质量不稳定。

当前建议是谨慎使用 sorted-prefix 作为默认策略。若目标是保持原先上界行为，`prefilter90` 更稳；若目标是降低 heuristic seed 选择开销并接受启发式路径变化，sorted-prefix 可以继续作为实验策略。后续更稳妥的工程做法是把 seed selection 做成显式策略开关，例如 `prefilterTopK` 与 `sortedPrefix`，默认先保留旧行为，再用批量实验决定是否切换默认。

### 41.47 2026-06-06 012 formal/on 卡点和 node8 pricing 图复核

继续复核 `tmp-wet030_from040_012_2m` 在 formal/on subtree 下“求解不动”的位置。已有 600 秒正式日志显示，前 7 个 node 都能较快处理，node7 总时间约 `38.536s`。真正卡住的是 node8：`node=8, depth=4, arcForbid=721, adjReq=1, adjForbid=3, restricted` 从初始 `2278` 过滤到 `1102`，heuristic pricing 补到 `1809` 后进入 exact pricing。node8 之后没有 node summary，说明卡在该节点 exact pricing 内部。

node8 的 heartbeat 显示卡点非常明确：exact pricing 停在 `forward.progress`，尚未进入 backward/join。最后一条记录约为 `fwQueue=1002083, fwPops=1067454, fwKept=2076963, fwDom=1210261, fCand=20850701, fBuilt=4717002, fBoundSurvivors=3287223, cbFPruned=1429779`。这说明不是 LP、RMIH 或 subtree 构造慢，也不是 join 爆炸，而是 forward label 扩展和 dominance 插入阶段已经进入百万级状态。

pricing snapshot 已经能保存这个节点的输入，路径为 `test-results/bpc/pricing-snapshots-012-formal/pricing-node-8-1780675676346-*`。其中 `summary.txt` 记录节点、机器 dual 和允许/禁止弧数量，`arcs.tsv` 记录所有 source/job/sink arc 的正式禁弧、pricingOnly 禁弧、综合 pricing 禁弧和 arc dual，`job-duals.tsv` 记录 job dual，`columns.tsv` 记录 restricted columns 的成本、长度、序列和是否含禁弧。当前 node8 snapshot 显示 `machineDual=-11585.875`，job-job 允许弧只有 `143`，禁止弧 `727`，pricingOnly 禁弧为 `0`。
...
补充一点：用户指出“理论上不应该更加均衡吗”，这个质疑是合理的。如果按经典 bidirectional labeling 的目标理解，两侧应围绕某个资源分界面共同承担状态空间，避免一侧把大量长路径都枚举完。但当前实现的 half-domain 不是这种强制均衡的 meet-in-the-middle。它允许 forward label 在左半域内直接接 sink 生成完整列，同时 backward 只作为跨 `Tmid` 的后缀 join 组件；因此一旦负列或 near-zero 列大多可以在 `Tmid` 左侧完成，算法会自然退化成 forward-dominant。node7/node9 的 `fw kept` 二三十万而 `bw kept` 个位数，正是在说明这种退化已经发生。

所以更准确的表述应是：这不是代码没有处理 backward，但也不是一个理想的双向均衡状态。当前 half-domain 的“均衡”只来自时间窗口切分，不能保证 label 数量或 dominance graph 工作量均衡。若要让它真正接近双向均衡，需要改变策略，例如动态调整 `Tmid`、限制 forward direct-sink 的深度/时间区域、或强制一部分完整列只能通过 crossing join 证明；但这些会改变列生成路径和正确性证明边界，必须单独做实验，而不能直接把 backward 少当成局部 bug 修。

### 41.51 2026-06-06 012 pricingOnly 调小 Tmid 的对照测试

继续按用户要求在 `tmp-wet030_from040_012_2m` 上测试更小的 `Tmid`。测试口径保持为 direct Java、half-domain、`completionBound=allCycles`、pricingOnly subtree、`maxNodes=10`、node progress summary 打开，只通过 `twet.bpc.fullDomainCompare.midpointRatio` 改变 `Tmid`。旧有效基线没有显式设 ratio，node7/node9 中 `pricingHorizon=3231.0, tMid=1620.5`，约等于 `0.5015`。

完整跑完的两个结果为：`ratio=0.35` 正常收敛，`solve=181.654s`，处理 `9` 个节点，exact pricing `137.602s/27 calls`，`valid=true`；`ratio=0.30` 正常收敛，`solve=35.367s`，处理 `7` 个节点，exact pricing `12.865s/26 calls`，`valid=true`。旧基线为 `solve=260.227s`，处理 `10` 个节点，exact pricing `222.973s/27 calls`。因此在 012 pricingOnly 上，`Tmid` 从约 `0.50` 调到 `0.30` 是当前最强正例，既消除了旧 node7/node9 的百秒级 pricing，又减少了搜索节点。

慢节点明细更能说明原因。旧基线 node7 为 `nodeTime=111.912s, exact=108.969s/2, fw kept/dominated=332142/36318, bw=2/0, forwardSink=331905/0`；node9 为 `nodeTime=106.055s, exact=101.749s/4, fw kept/dominated=219589/19401, bw=8/12, forwardSink=219503/0`。`ratio=0.35` 后 node7 降为 `4.266s`，但 node8 和 node9 仍分别为 `62.358s`、`60.914s`，最后一轮 forward label 仍有 `113232` 和 `195496`。`ratio=0.30` 则在 node7 已收敛，node7 只有 `2.632s`，最后一轮为 `fw kept/dominated=2256/212, bw=26/100, forwardSink=2242/0`，没有进入旧的几十万 label 证明状态。

中断测试也提供了边界信息。`ratio=0.45` 前 7 个节点可返回，node7 从旧基线的 `111.912s` 降到 `5.301s`，但后续节点超过 150 秒没有返回，手动停止；`ratio=0.40` 前 6 个节点返回后在 node7 附近超过 90 秒没有返回，手动停止；`ratio=0.25` 前 7 个节点返回但总时间已到 `68.611s`，随后后续节点超过 60 秒未返回，手动停止。由于 runner 只有正常 solve 返回后才写 summary log，这三组只作为控制台部分运行证据，不作为完整收敛结果。

由此当前判断为：调小 `Tmid` 确实能解决 012 pricingOnly 上 forward-dominant 的旧 node7/node9 慢点，因为它缩短了 forward 可直接接 sink 的左半域，减少 source 出发的中长前缀证明。但 `Tmid` 不是越小越好。`0.35` 仍会在后续节点出现 60 秒级 forward 证明；`0.25` 则明显把根节点和早期节点的 backward/bound 评估变重，例如根节点 `bw kept=2044, completionBound eval=43279`，比 `0.30` 更慢且树更大。当前 012 上最合适的试验值是 `0.30`，但它改变了列生成路径和分支树，不能直接推广为全局默认；后续应在 `010/011/013/014/015` 等 30 任务样例上用同样 direct Java 口径做小批量复核。

### 41.52 2026-06-06 初始 CmaxH 的计算链路

继续梳理 `Tmid` 的上游 horizon。`Data` 构造函数中，`CmaxH` 的初始链路为：先 `setCmax()` 给一个解析上界，再 `setImprovedCmax()` 用一个无等待 release-time 启发式尝试改进，随后 `setPreprocessedHardWindows()` 和 `tightenCmaxByPreprocessedHardWindows()` 可能根据 job 级粗硬窗进一步收紧，最后基于该 `CmaxH` 构造 penalty functions 和 preprocessing forbidden arcs。

在不考虑外包、即 `outsourcingCost[j]=big_M` 的口径下，粗硬窗不会因为外包成本而明显收紧。此时 `setPreprocessedHardWindows()` 对每个 job 得到的基本是 `hardWindowStart[j]=0, hardWindowEnd[j]=CmaxH`，因此 `tightenCmaxByPreprocessedHardWindows()` 不会把 `CmaxH` 再降下来。真正决定初始 horizon 的主要就是 `setCmax()` 和 `setImprovedCmax()`。

`setCmax()` 先对每个 job 取 `max_sj=max_i s[i][j]`，把 `p[j]+max_sj` 当作 job 的保守加工占用。定义 `sumP=sum_j(p[j]+max_sj)`，`maxP=max_j(p[j]+max_sj)`，`maxD=max_j(d_e[j]-(p[j]+max_sj))`，然后设置：

`CmaxH = sumP/m + (1-1/m)*maxP + maxD + 1`

这个公式可以理解为一个 release/no-wait 风格的保守 horizon：`sumP/m` 是平均负载，`(1-1/m)*maxP` 给并行机列表调度类 makespan 上界留出最大任务补偿，`maxD` 把“不会继续等待的最晚有效开始区域”平移到 due-window 位置附近，最后 `+1` 是离散/整数时间上的余量。它不是严格来自当前 TWET 目标的最优性条件，而是给搜索和分段函数一个相对安全但不至于无限大的时间域。

`setImprovedCmax()` 进一步把每个 job 的 release time 临时设为 `d_l[j]-p[j]`，并允许 setup 在 release 前发生，用 `SchedulerForReleaseNoWait` 做启发式调度。如果得到的 `improvedCmax < CmaxH`，则令 `CmaxE=improvedCmax`，并把启发式用的 `CmaxH` 设为 `1.1*CmaxE`。这里 `CmaxE` 更像较紧的调度上界，`CmaxH` 则保留 10% slack，避免启发式和 pricing 的时间域过窄。

因此，在不允许外包的情况下，当前 BPC/pricing 使用的初始 `CmaxH` 大致是：先用解析公式得到一个保守值，再用 release/no-wait 启发式尝试改成 `1.1 * heuristicMakespan`；外包相关 hard-window 收紧基本不起作用。后续 `pricingHorizon` 通常等于这个 `CmaxH`，除非局部动态窗口启用并把 horizon 进一步设为 `min(CmaxH, localHorizon)`。

### 41.53 2026-06-06 release/no-wait 启发式和 1.1 slack 的语义

继续追问 `setImprovedCmax()` 中为什么把启发式 makespan 乘以 `1.1`。当前代码里，`SchedulerForReleaseNoWait.solve()` 返回的是一个构造性 ECT 列表调度的 makespan，不是精确最优 makespan，也不是正式 TWET 主问题的严格最优时间域证明。`CmaxH` 后续会作为 pricing horizon、penalty function domain 和预处理硬窗的基础。如果直接把 horizon 设成构造启发式值，时间域可能过紧：启发式本身可能因为贪婪排序错过更合理的列组合，正式 pricing/RMP 也可能需要略晚的任务完成时间来换取更低 TWET 成本或更好的列组合。因此代码保留 `CmaxE=improvedCmax` 作为较紧的启发式上界，但把用于搜索的 `CmaxH` 设为 `1.1*CmaxE`，给 10% 余量。这个 `1.1` 是经验折中：太小容易把可用时间域压死，太大又会扩大 DP/pricing 状态空间。

`SchedulerForReleaseNoWait` 的实际过程是：每个 job 转成 0-based id，释放时间设为 `r_j=d_l[j]-p[j]`，加工时间为 `p[j]`；初始 setup 用 `s[0][j]`，机器内部转移 setup 用 `s[i][j]`。构造启发式维护每台机器当前完成时间和最后一个 job。每轮枚举所有未排 job 和所有机器，计算 `start=max(r_j, machineAvailable+setup)`，`completion=start+p_j`，选择 completion 最小的 `(job,machine)`，平局时先选 start 更早，再选机器编号更小，然后把 job 追加到该机器尾部。当前 `solve()` 里的 local search 代码被注释掉，所以返回的就是这个构造性启发式结果。

因此这里的 release 约束只限制“加工开始时间”不早于 `d_l[j]-p[j]`。setup 不要求发生在 release 之后；公式 `start=max(r_j, previousFinish+setup)` 允许 setup 被安排在 release 之前的空闲时间里。例如前序完成为 0、setup 为 10、`r_j=100` 时，代码得到 `start=100`，可以理解为机器在 90 到 100 做 setup，任务从 100 开始加工。若前序完成为 95、setup 为 10，则 `start=105`，因为 setup 不能早于前序任务完成。也就是说，它不是“先等到 release 再做 setup”，而是“release 约束加工开始；setup 可利用 release 前的机器空闲，但必须在前序任务之后完成”。

### 41.54 2026-06-06 Tmid 策略实现口径

继续按当前讨论把 `Tmid` 的几种策略落到 `GCNGBBStyleBidirectional`。这次没有继续做“根据当前节点禁弧重算 pricingHorizon”的 nowait 插入启发式，因为禁弧后要快速构造保证可行的整机调度并不简单，粗糙 fallback 容易掩盖错误。当前只保留解析得到的 `pricingHorizon`，然后在该 horizon 内调整 `Tmid`。这样改动集中在半域分界点，不改 pricing 图、列可行性或主问题语义。

现在支持的策略为：`default` 保持原逻辑，取 `left=(dynamicMinHStart, earliestSourceCompletion)` 中较大值，然后 `Tmid=(left+pricingHorizon)/2`；若显式设置 `midpointRatio`，仍优先用比例值。`completionBound` 先用默认 `Tmid` 建基础 half-domain cache，再构造 completion bound；随后扫描 forward/backward 四类 bound 函数，对所有最小值小于 0 的函数取其 argmin time 的最大值作为参考时间，若没有负值函数则退回所有函数的 argmin time 最大值，最后用 `(left+reference)/2` 作为 `Tmid`。这个策略利用的是当前 pricing 松弛中“有希望的状态”实际落在哪个时间附近。

列策略不再使用简化 no-wait/release timing。`TWETColumnEvaluator.evaluateTiming()` 会临时构造单机 `Solution`，按原 `TWETColumnEvaluator` 成本口径计算列成本，再基于 `Solution` 的 forward prefix functions 求整条列最优完工时间。具体做法是先在最后一个 prefix 函数上取左侧最小点作为整列最优末完工时间，再从后往前反推：对第 `k` 个任务，在 `C_{k+1}-s_{j_k,j_{k+1}}-p_{j_{k+1}}` 之前，从第 `k` 个 prefix 函数上取最小点作为该任务完工时间。这样得到的是完整 PWLF 口径下的列内 timing，不是 no-wait 近似。

`columnLastAvg` 会先按当前 restricted columns 的 reduced cost 排序，只评价前 `K=bidirectionalMidpointColumnLimit` 条兼容 pricing 图的列，统计这些列的最优末完工时间平均值 `lastAvg`，然后取 `Tmid=(left+lastAvg)/2`。`columnHalfAvg` 用同一批列，但取每条列中间位置任务的反推最优完工时间 `halfCompletion` 的平均值，直接作为 `Tmid`，因为这个值本身已经是列内“半路”时间。`incumbentMakespan` 则从 `data.configure.bestSolution` 的各机器序列重算完整 PWLF timing，取最大末完工时间作为 incumbent makespan，再取 `(left+makespan)/2`。ALNS 平均/中位数口径暂时没有实现，只作为后续可选统计方向。

为便于后续判断，每轮 pricing summary 现在输出 `midpointStrategy/ref/ms` 和列策略的 `lastMin/Avg/Max`、`halfMin/Avg/Max`。测试中 column 策略每轮只评价最多 200 条列，策略本身耗时一般为毫秒到几十毫秒级，相比 node7/node9 旧的百秒级 exact pricing 不是大头。

### 41.55 2026-06-06 012 上几种 Tmid 策略的结果

用 `tmp-wet030_from040_012_2m`、half-domain、`completionBound=allCycles`、pricingOnly subtree、node progress summary 打开作为主要对照。旧 `default` 在 `maxNodes=10` 下可以收敛，但总耗时 `237.742s`，exact pricing `200.519s/27 calls`。慢点仍是 node7 和 node9：node7 `nodeTime=92.267s, exact=89.664s`，最后一轮约 `fw/bw=332142/2`；node9 `nodeTime=103.874s, exact=100.072s`，最后一轮约 `fw/bw=219589/8`。此时 `pricingHorizon=3231`，`Tmid=1620.5`，左半域过大，forward 直接接 sink 的证明成本很高。

`completionBound` 策略是当前最好的结果：`maxNodes=10` 实际在 8 个节点内收敛，`solve=29.573s`，exact pricing `5.195s/8 calls`，heuristic `10.568s`，valid=true。对应旧慢点处，node7 变成 `nodeTime=2.059s, exact=0.170s`，`Tmid=512.5`，参考时间 `1015.0`。这说明 completion bound argmin 给出的参考时间能有效把半域左边界压回真实有希望的时间区间，消除了旧 node7/node9 的 forward label 爆炸。

`columnLastAvg` 也比较稳定：`maxNodes=10` 收敛，`solve=39.431s`，exact pricing `4.885s/10 calls`，heuristic `17.115s`，valid=true。node7 为 `nodeTime=7.057s, exact=0.299s, Tmid=440.3625`，其中列统计约为 `count=200, lastAvg=870.725, halfAvg≈380`；node9 为 `nodeTime=2.806s, exact=0.186s, Tmid=444.74`。它的 exact pricing 甚至略低于 completionBound，但搜索节点和 heuristic/RMIH 路径更多，所以总时间稍慢。当前判断是：如果不想让 Tmid 强依赖 completion bound 语义，`columnLastAvg` 是较稳的备选。

`incumbentMakespan` 有帮助但不如前两者。`maxNodes=7` 时已经达到 `incumbent=bound=13360`，总耗时 `41.410s`，exact `7.471s/9 calls`，node7 `Tmid=572.5`、参考 makespan `1135.0`、`exact=0.504s`。它能把 `Tmid` 从 1620.5 明显拉低，但 incumbent makespan 仍偏保守，因此 label 数和 heuristic 调用仍多于 completionBound/columnLastAvg。

`columnHalfAvg` 不建议作为默认。`maxNodes=7` 下为 `NODE_LIMIT`，`incumbent=13360, bound=13341.75`，总耗时 `49.752s`，exact `10.828s/7 calls`。node7 `Tmid=373.055`，最后一轮 `fw/bw=22100/10`，比 default 的几十万 forward label 小很多，但比 completionBound/columnLastAvg 仍重。原因是直接用列内 halfCompletion 平均值会把 `Tmid` 推得更靠左，能抑制 forward，但容易让 backward/bound 侧和树路径变重，不如 `columnLastAvg` 稳。

由此当前结论为：012 上最推荐的策略是 `completionBound`，其次是 `columnLastAvg`；`incumbentMakespan` 可作为低依赖的保守改善；`columnHalfAvg` 只作为诊断，不宜默认。它们都没有使用简化 timing。后续若要推广，需要继续在 `010/011/013/014/015` 等 30 任务样例上复核，因为 Tmid 改变会影响列生成顺序和分支树，不应只凭 012 单例定默认。

### 41.56 2026-06-06 sink endpoint 分支调整

分支部分这次只改了 endpoint arc branch 的候选范围：`ArcBrancher.findBestEndpointArc()` 现在只考虑 `0 -> job`，不再考虑 `job -> sink`。内部 `job -> job` arc branch、source endpoint branch、required/forbidden adjacency branch 和 tariff segment branch 都没有删。

这样改的原因是 sink 是虚拟终点，`job -> sink` 在 TWET 列里通常只表示该 job 是机器序列最后一个任务，不对应真实任务之间的顺序关系。把 `job -> sink` 作为硬分支写入后续节点，会把“某个 job 必须/禁止作为尾任务”混进 arc/subtree/pricingOnly 的禁弧语义里，容易干扰后续 pricing 图和 completion-bound/subtree 分析。相对地，`0 -> job` 表示机器首任务，仍有明确 endpoint 语义，所以保留。

因此当前 branch 改动不是“删掉所有和虚拟点有关的逻辑”，而是先删除最容易造成语义混乱的 `job -> sink` endpoint 分支候选。pricing 中 `last -> sink` 仍然用于构造完整列，已有分支状态里若存在 sink 相关状态也不会在这次修改中强行回滚；这次只是防止后续继续生成新的 `job -> sink` endpoint branch。

### 41.57 2026-06-06 midpointRatio、实现范围和正确性复核

继续复核当前 `Tmid` 策略实现。`midpointRatio` 指的是测试参数 `twet.bpc.fullDomainCompare.midpointRatio` / `twet.bpc.asymmetricCompare.midpointRatio` / `twet.bpc.timeCompare.midpointRatio` 写入的 `config.bidirectionalRootLocalHorizonMidpointRatio`。如果它被显式设置成 `(0,1)` 内的有限值，`GCNGBBStyleBidirectional.computeCurrentMidpoint()` 会直接使用 `Tmid=pricingHorizon*ratio`，并把策略记为 `ratio`，此时不再使用 `midpointStrategy`。这个优先级是为了保留前面 `ratio=0.30/0.35` 这种受控实验入口；正常比较 `default/completionBound/columnLastAvg` 时不要设置该参数，它默认是 `NaN`，不会产生影响。

当前动态策略只接入 `GCNGBBStyleBidirectional`，也就是现在 012 pricingOnly 测试使用的 NG 半域 pricing 路径。`GCBBStyleBidirectionalFullDomain`、`GCBBAsymmetricBidirectional`、`GCBBStyleBidirectionalFullDomainNodeJoin` 和旧 `GCBidirectional` 仍是原 midpoint 逻辑。因此当前结论只覆盖这条主测路径，不能说所有历史双向实现都已经统一。

构造顺序也复核过：每轮 pricing 先用默认 midpoint 建基础 half-domain cache；若策略为 `completionBound`，再在这个基础上构造 completion bound；随后按策略重算最终 `Tmid`，并重新调用 `ensureBaseHalfPenaltyCache()`，保证最终 half-domain 函数和最终 `Tmid` 一致。也就是说 completion bound 策略需要一次“默认 Tmid 的临时 cache”，但最终用于标号扩展的是重建后的 cache，不会拿旧半域继续跑。

列策略现在使用完整 `TWETColumnEvaluator/Solution` 的 PWLF 口径，不再做 no-wait 简化。`evaluateTiming()` 先用 `calCost(0)` 计算原列成本并刷新 forward prefix functions，再从最后一个 prefix 函数取整列最优末完工时间，并从后往前按后继最优时间上界反推每个任务的最优完工时间。为了避免重复刷新同一批函数，这次新增 `Solution.computeBestCompletionTimesFromCurrentForwardFunctions()`，只复用刚由 `calCost(0)` 更新过的 forward functions；若函数数量和当前机器序列不一致，会直接抛错。这是性能小改，不改变列 timing 的数学语义。

### 41.58 2026-06-06 Tmid 策略多样例对比

本次继续用 direct Java 口径补测，避免前面 `Start-Process` wrapper 杀不掉真实 Java 子进程导致的无效超时记录。主要配置为 half-domain、`completionBound=allCycles`、pricingOnly subtree、node progress summary 打开。

在 `tmp-wet030_from040_012_2m` 上，旧 `default` 虽然能在 `maxNodes=10` 内收敛，但 `solve=237.742s`，exact pricing `200.519s/27 calls`，慢点主要是旧 node7/node9 的 forward label 爆炸。`completionBound` 收敛为 `solve=29.573s`，只处理 8 个节点，exact pricing 降到 `5.195s/8 calls`；`columnLastAvg` 收敛为 `39.431s`，exact pricing `4.885s/10 calls`；`columnHalfAvg` 在 7 节点口径下仍为 `NODE_LIMIT`，`solve=49.752s`；`incumbentMakespan` 7 节点口径下 bound 已到 incumbent，但 `solve=41.410s`。因此 012 上最强的是 `completionBound`，`columnLastAvg` 也能显著解决旧慢点。

`010` 的 `maxNodes=10` 结果说明策略不能只按单例定默认。`default` 为 `FINISHED, incumbent=bound=16237, nodes=6, cols=13184, solve=37.176s, exact=10.625s/15`；`completionBound` 为 `FINISHED, incumbent=bound=16258, nodes=5, cols=7408, solve=23.678s, exact=5.882s/8`；`columnLastAvg` 为 `FINISHED, incumbent=bound=16237, nodes=5, cols=7712, solve=26.098s, exact=4.249s/7`。这里 `completionBound` 最快但树路径/列生成路径得到的闭合值比 default 差，`columnLastAvg` 同时保持 default 的 16237 结果并明显更快，所以更稳。

其它 30 任务样例先按 `maxNodes=2` 做早期节点对比，主要看策略是否快速恶化。结果为：`011` 上 `columnLastAvg` 两节点时 incumbent 从 `14258` 改到 `13935`，但 `solve=18.828s`，略慢于 `completionBound=16.130s`；`013` 上三者 incumbent 都是 `14433`，`completionBound` 最快 `10.057s`，`columnLastAvg` 的 bound 较高为 `14294`；`014` 根节点即闭合，三者都为 `10288`，时间差在 `9.743s-10.171s`；`015` 根节点即闭合，三者都为 `13394`，`columnLastAvg` 最快 `7.389s`。这些结果说明 `completionBound` 对 012/013 的 exact pricing 压缩最强，但 `columnLastAvg` 在 010/011/015 上更稳定，至少没有像 010 的 `completionBound` 那样改变到较差闭合值。

当前建议是保留策略开关，不把 `completionBound` 直接改成全局默认。若目标是解决 012 这类明显 forward-dominant 的节点，优先用 `completionBound`；若希望跨算例更稳，可以先用 `columnLastAvg` 作为候选默认继续扩大测试。`columnHalfAvg` 暂时只适合诊断，不建议默认；`incumbentMakespan` 改动依赖少，但改善幅度不如前两者。后续要做全局默认决策，需要继续用更多样例和更长 node/time limit 比较最终 incumbent、bound、节点数和 exact pricing 时间，而不是只看单个节点的 label 数。

### 41.59 2026-06-06 Tmid 策略稳定性判断

把已有测试按“稳定性”重新归类后，当前最稳的候选不是 `completionBound`，而是 `columnLastAvg`。它在 012 上把 `solve` 从 `237.742s` 降到 `39.431s`，在 010 上保持 default 的闭合值 `16237` 并把时间从 `37.176s` 降到 `26.098s`，在 011 两节点口径下还把 incumbent 从 `14258` 改到 `13935`。它的缺点是 013 两节点口径比 `completionBound` 慢，且列 timing 每轮要评价最多 200 条兼容列；但目前看这个开销通常是毫秒到几十毫秒级，不是主要瓶颈。因此如果要选一个跨算例更稳的候选默认，当前更倾向 `columnLastAvg`，但还需要更多 full-run/限时结果确认。

`completionBound` 是最强的针对性加速策略，但稳定性不如 `columnLastAvg`。它在 012 上效果最明显，exact pricing 从 `200.519s` 降到 `5.195s`，总时间降到 `29.573s`；013 两节点也最快。但 010 上它虽然最快，闭合值却变成 `16258`，而 default 和 `columnLastAvg` 都是 `16237`。这说明它会明显改变列生成和分支树路径，不能只因为某个节点 pricing 快就设成全局默认。当前更适合在识别到 forward-dominant、default `Tmid` 明显偏大的节点或算例上使用。

`incumbentMakespan` 属于保守改善。它利用已有 ALNS/incumbent 解的最大机器完工时间，不依赖 completion bound 和列池统计，语义简单、开销很小。在 012 上 `maxNodes=10` 为 `72.343s`，比 default 好很多，但明显差于 `completionBound` 和 `columnLastAvg`。它的问题是参考值偏保守，而且依赖 incumbent 质量；如果 ALNS 解不够紧或节点结构已经和 incumbent 差很多，`Tmid` 仍可能偏右。当前适合作为低风险备选，不适合作为主推策略。

`columnHalfAvg` 当前不建议继续主推。它直接把列内中间任务完工时间平均值作为 `Tmid`，会把分界点推得很靠左。012 上确实压低了 forward label，但 `maxNodes=7` 仍未闭合，`solve=49.752s`，node7 仍有 `fw kept=22100` 和较重的 paperGraph 查询。直觉上它过度修正了 default 的“太靠右”问题，容易把 backward/join/bound 侧压力拉高，树路径也不稳定。后续只保留为诊断项。

显式 `midpointRatio` 只作为实验旋钮。012 上 `ratio=0.30` 曾得到 `35.367s` 的好结果，说明调小 `Tmid` 的方向是对的；但 `0.35` 又变成 `181.654s`，`0.25/0.40/0.45` 的中断记录也说明比例值很敏感。固定比例不看节点 dual、列池和 bound 信息，跨算例风险最大，因此不应作为默认策略，只用于快速定位“是不是 Tmid 太右”。

旧的 no-window/fraction 实验也不适合继续主推。它在 012 上能改善，但 010 的 `f0.50` 结果为 `72.371s`，明显慢于当前 `columnLastAvg` 和 default；011 两节点也比当前 default 慢。它改变的是 horizon/窗口口径，副作用比只调 `Tmid` 更大，目前没有表现出更好的稳定性。

综合当前证据，策略排序暂定为：跨算例候选默认优先 `columnLastAvg`；解决 012 类 forward-dominant 慢节点优先 `completionBound`；低依赖保守备选为 `incumbentMakespan`；`columnHalfAvg`、显式 ratio 和 no-window fraction 只保留诊断用途。最终默认仍不能现在拍死，需要在更多 30 任务样例上用相同限时或 node limit 继续比较 incumbent、bound、exact pricing 时间和节点数。

### 41.60 2026-06-06 关于“稳定”的口径修正

用户指出“稳定”不应理解为和 default 跑出完全一样的目标值，这个修正是对的。对于动态 `Tmid`，更合理的稳定性标准应是：第一，计算出的 reference 是否接近当前节点真实有效的列完成时间尺度，而不是被松的 `pricingHorizon` 拉到过右；第二，左右两侧 label 是否避免严重单侧爆炸，尤其是不要让 forward 在左半域直接接 sink 时枚举几十万 label；第三，reference 是否能随节点 dual、列池、禁弧状态变化，而不是所有节点使用一个固定值。

按这个口径看，`columnLastAvg` 的优势不是“保持 default 目标值”，而是它直接从当前节点 restricted columns 的完整 PWLF timing 中估计有效 makespan，并且每个节点都会重新算。012 非根节点中 `pricingHorizon=3231`，但 `columnLastAvg` 的列末完工平均值长期在 `870-884` 左右，差了约 `2350`；010 非根节点中 `pricingHorizon=4342`，列末完工平均值约 `1251-1273`，差了约 `3070`；011/013 node2 也分别是 `3560 vs 973`、`3585 vs 938`。这说明 default 的 `(left+H)/2` 确实太靠右，而列策略能按当前列池把 `Tmid` 拉回有效时间区间。

`completionBound` 的 reference 不是“真实 makespan”，而是 completion-bound 函数中有希望状态的 argmin time。它同样能发现 `pricingHorizon` 太松：012 非根节点 reference 多在 `978-1035`，比 `3231` 小约 `2200`；010 非根节点 reference 约 `1383-1444`，比 `4342` 小约 `2900`。它对 012 的慢节点最有效，但本质上是从当前松弛 bound 估计时间尺度，未必等同于真实列末完工时间。

`incumbentMakespan` 的主要问题不是结果是否与 default 一样，而是 reference 不随节点变化。012 中它一直用 incumbent makespan `1135`，非根节点 `Tmid=572.5`，确实比 default 的 `1620.5` 好，但它不会随 node7/node9 的 dual、列池、pricingOnly 禁弧变化调整。因此它是保守改善，不够灵活。

`columnHalfAvg` 当前做法是：从 selected columns 的完整 PWLF timing 中取每条列中间位置任务的完工时间，求平均后直接作为 `Tmid`，不是用列末 makespan。012 中它的 reference 约 `373-403`，而同一批列的末完工平均值约 `872-883`。因此它不是估计真实 makespan，而是在估计“列内半程时间”。这个值往往太靠左，能压 forward，但可能把 crossing/join 和 backward 侧推得过紧，所以目前只适合作为诊断。

前面提到的 no-window/fraction 不是当前正式策略，而是早先用于验证“无窗口情况下固定比例压缩 horizon/Tmid 是否有效”的临时实验口径。它改变的是 horizon/窗口口径，不只是换 `Tmid` reference；由于 010/011 表现不好，当前不作为主线继续推进。

### 41.61 2026-06-06 completionBound 与 columnLastAvg 的进一步比较

继续按用户要求重新比较 `completionBound` 和 `columnLastAvg`。二者差别其实没有“一个稳一个不稳”那么大。更准确地说，二者都在非根节点发现了同一个事实：当前 `pricingHorizon` 明显大于实际有希望列的时间尺度，因此 default 的 `Tmid≈H/2` 太靠右。区别在于 reference 的来源不同。

`columnLastAvg` 的 reference 是当前 restricted columns 中 reduced cost 较好的兼容列的完整 PWLF 最优末完工时间平均值，可以近似理解为“当前列池看到的真实列 makespan 尺度”。`completionBound` 的 reference 是 completion-bound 函数中有希望状态的 argmin time，是一个 elementary 松弛下的时间尺度估计，不是列末 makespan。由于 completion bound 松弛了基本性约束，它给出的 reference 通常比列末平均值略大一些，但数量级很接近。

从当前日志看，根节点上 `pricingHorizon` 本来就不太松，因为 root 能利用 dual profitable window 收紧。010 root 中 `H≈1303`，`columnLastAvg≈1258`，差约 `45`；012 root 中 `H≈915`，`columnLastAvg≈872`，差约 `40`；011/013/014/015 root 也基本是差几十。因此根节点 default 的 `H/2` 不一定差，这也是为什么 root 上三种策略通常差异不大。

真正的问题出现在非根节点。012 的非根节点 `pricingHorizon=3231`，而 `columnLastAvg` 长期在 `870-884`，差约 `2350`；`completionBound` reference 在 `978-1035`，差约 `2200`。010 的非根节点 `pricingHorizon=4342`，`columnLastAvg≈1251-1273`，差约 `3070`；`completionBound≈1383-1444`，差约 `2900`。011 node2 是 `3560` 对 `973/1095`，013 node2 是 `3585` 对 `938/1062`。因此两种策略都把 `Tmid/H` 从 default 的约 `0.50` 拉到 `0.13-0.17`。

这种缩小对结果影响很直接。012 node7 上，default 最后一类慢 pricing 的 `Tmid=1620.5`，forward label 约 `33.2` 万，pricing 调用约 `89.7s`；`columnLastAvg` 的 `Tmid≈440`，forward label 约 `391`，调用约 `0.299s`；`completionBound` 的 `Tmid≈512`，forward label 约 `293`，调用约 `0.170s`。012 node9 上，default forward label 约 `22` 万，调用约 `100s`；`columnLastAvg` 在对应后续节点只有数百 forward label，`0.186s` 量级，`completionBound` 由于树更早收敛没有进入完全同一个 node9 轨迹。010 node3 也类似：default `Tmid=2185.5`，forward label 最大约 `56528`，该节点 pricing 合计约 `5.73s`；`columnLastAvg` `Tmid≈651`，forward label `14087`，约 `1.09s`；`completionBound` `Tmid≈706`，forward label `31686`，约 `2.02s`。

不过需要注意，当前 half-domain 实现并没有让非根节点真正变成左右 label 数量均衡。很多非根节点中 `bw kept=0`，说明算法仍主要依赖 forward 左半域直接接 sink 证明完整列。动态 `Tmid` 的实际作用是把左半域从过大的 `H/2` 压回更合理的有效时间区间，从而避免 forward 单侧爆炸；它不是严格意义上的 meet-in-the-middle 均衡器。若后续目标是让 forward/backward label 数量也接近，需要改更深的 crossing/join 规则，而不只是换 reference。

因此当前二者的比较结论为：`completionBound` 和 `columnLastAvg` 都能正确识别非根节点 horizon 过松。`completionBound` 的 reference 稍大、更偏松弛 bound 语义，012 上压慢点最强；`columnLastAvg` 的 reference 更贴近当前列池真实 makespan，解释性更强，也会随节点列池变化。二者目前都值得继续测试，不能简单说一个明显支配另一个。

### 41.62 2026-06-06 动态 Tmid 策略 600 秒尝试

按用户要求继续尝试 `completionBound` 和 `columnLastAvg` 在更多 30 任务样例上的完整求解表现。本轮配置为 half-domain、`completionBound=allCycles`、pricingOnly subtree、`nodeProgressSummary=true`、每组 600 秒。中途按用户要求停止了最后一组 `013-columnLastAvg`，因此该项没有落盘 stdout，不纳入结论。

`tmp-wet030_from040_009_2m` 在根节点闭合，`completionBound` 为 `solve=8.482s, pricing=28, exact=2.045s/3, cols=5520, incumbent=bound=14065`；`columnLastAvg` 为 `solve=11.543s, pricing=32, exact=4.440s/6, cols=5476, incumbent=bound=14065`。这个样例上二者都正确闭合，`completionBound` 更快，主要少做了 3 次 exact pricing。

`tmp-wet030_from040_011_2m` 两个策略都在 600 秒超时，且 stdout 都只落到了 node1、node2 摘要，说明后续卡在 node3 处理过程中。`completionBound` node1 用 `19.813s`，node2 用 `8.120s`，到 node2 后 `inc=14258, bound=13525.866667`；`columnLastAvg` node1 用 `31.078s`，node2 用 `13.461s`，到 node2 后已把 incumbent 改到 `13935`，`bound=13525.411765`。因此 011 上 `columnLastAvg` 的上界路径更好，但不能解决后续节点长时间不返回的问题；`completionBound` 也没有把 011 变成可在 10 分钟内闭合的样例。

`tmp-wet030_from040_013_2m` 的 `completionBound` 在 600 秒超时，但日志足够说明慢点。它处理到 node8，总时间 `599.021s`，`inc=14433, bound=14381.500000, gap=0.3568%`。真正的大头是 node3：node3 用 `537.475s`，其中 pricing `535.026s/9`，exact pricing 单次 `529.573s`，对应 `fw kept/dominated=228465/294290, bw kept/dominated=4/0`，completion bound 剪枝统计为 `fwPruned=1712595, bwPruned=93`。这说明即使用 `completionBound` 更新 Tmid，013 仍可能在某些分支节点退化成严重 forward 单侧爆炸。后续 node4-node8 每个节点都只有 5-9 秒量级，证明瓶颈非常集中。

结合前面的 012 和 010 结果，当前结论需要更谨慎：动态 Tmid 确实能修复一批由 `pricingHorizon` 太松引起的 forward 爆炸，012 是最典型正例；但 011/013 说明，`completionBound` 和 `columnLastAvg` 都还不是“所有 30 任务都稳”的方案。013 node3 的数据尤其说明，当前 half-domain 结构仍允许 forward 侧直接承担主要证明工作，即便 Tmid 已经从 default 拉低，也可能因为该节点 dual/分支结构使中长 forward 前缀大量保留。也就是说，动态 Tmid 是必要改善，但不是根治；后续若继续处理 011/013，需要进一步看卡住 node 的 dual、required adjacency、allowed graph 和 forward direct-sink 规则，而不是只继续调 reference。

和固定 `midpointRatio` 比较时，`ratio=0.30` 在 012 上有 `solve=35.367s` 的强结果，接近 `completionBound=29.573s`、优于 `columnLastAvg=39.431s`；但同一算例 `ratio=0.35` 已恶化到 `181.654s`，说明固定比例非常敏感。动态策略的优势在于 reference 随节点变化，能解释为什么非根节点要从 `H/2` 拉到大约 `0.13H-0.17H`；固定比例只适合作为诊断，不适合作为默认。当前更合理的继续路线是保留 `completionBound` 和 `columnLastAvg` 两个策略开关，并用更多 hard-node 诊断决定是否需要调整 half-domain 的 forward/backward 连接机制。

### 41.63 2026-06-06 停止前对 011/013 hard node 的复核

按用户要求停止继续跑长实验后，对已有日志重新做了一次证据复核。当前没有找到 `tmp-wet030_from040_011_2m` 和 `tmp-wet030_from040_013_2m` 的 full-tree `FINISHED` 记录；过去所谓“能求解动”的证据主要是 root 或 `maxNodes=2` 能返回，并不表示完整分支树能闭合。011 在动态 `Tmid` 的 600 秒测试里，`completionBound` 和 `columnLastAvg` 都只写到 node1/node2 摘要，后续卡在 node3 处理过程中；这和之前 completion-bound changed 假阳性修复后的“root 能返回”不是同一个结论。

013 的 `completionBound` 600 秒超时日志显示，瓶颈高度集中在 node3。为了确认它是不是没有用到动态 `Tmid`，本次给 `GCNGBBStyleBidirectional` 的 stage heartbeat 增加了 `pricingHorizon/tMid/midpointStrategy/midpointRef` 输出，开关仍由 `twet.bpc.diagnosticStageHeartbeat` 控制，默认关闭。复跑 180 秒诊断后可以确认：node3 已经使用 `completionBound` 策略，`pricingHorizon=3585.0`，`midpointRef=1102.0`，`tMid=563.5`，不是默认的 `H/2=1792.5`。也就是说，这个节点的问题不是动态 `Tmid` 没生效，而是在已经把左半域压到约 `0.157H` 后，forward 侧仍然爆炸。

该 hard node 的结构为 depth=1，`pricingOnlyArc=501`，`adjReq=1`，repair 信息为 `(3,5)` required adjacency 右支。180 秒内 exact pricing 一直停在 forward 阶段，最后 heartbeat 约为 `fwQueue=49188, fwKept=215008, fwDom=201947, fCand=3745642, fBuilt=1771683, fBoundSurvivors=416954, cbFPruned=1354729`，backward 仍未开始有效扩展。结合 600 秒完整日志中的单次 exact pricing `529.573s`、`fw kept/dominated=228465/294290, bw kept/dominated=4/0`，当前可以判断：completion bound 的确大量剪枝，但该节点仍有几十万 forward survivor，需要继续证明没有负 reduced-cost 列。

为了判断固定更小 `Tmid` 是否可能缓解，又临时跑了 013 的 `midpointRatio=0.10/0.15/0.20`、`maxNodes=4`。三组都很快返回，node3/node4 的 exact pricing 只有毫秒级到 0.3 秒级：`ratio=0.10` 的 node3 为 `Tmid=358.5, fw=885/150, exact=0.242s`；`ratio=0.15` 为 `Tmid=537.75, fw=2330/267, exact=0.275s`；`ratio=0.20` 为 `Tmid=717.0, fw=3328/182, exact=0.280s`。但这个对比不能解释成“同一个 hard node 被 ratio 修好了”，因为 ratio 运行的 node3 是另一条分支路径：depth=2，`adjReq=0, adjForbid=2`，restricted columns 约 `4208`；而 completionBound hard node 是 depth=1，`adjReq=1`，restricted columns 约 `5501`。因此它只能说明 Tmid 与分支路径都会强烈改变 label 规模，不能单独证明 fixed ratio 是更稳策略。

当前更客观的结论是：动态 `Tmid` 能修复 012 那类 `pricingHorizon` 太松导致的 forward-dominant 爆炸，但 011/013 还存在另一类 hard node，尤其是 required adjacency 右支下的 forward survivor 规模问题。013 node3 的数据说明，`completionBound` reference 已经比 `pricingHorizon` 小很多，却仍无法让 forward/backward 变均衡。继续优化时不应只调 reference，而要进一步看该节点的 required adjacency dual、允许图核心结构、forward direct-sink 规则以及是否需要对 hard node 做更具体的 pricing 快照/复现工具。

本轮停止前的可落地结论为：保留 `completionBound` 和 `columnLastAvg` 策略开关；`completionBound` 不能直接当成解决所有 30 任务 hard node 的默认方案；013 的 ratio 结果暂时只作为诊断证据，不作为默认策略依据。后续若继续做，应优先保存 013 completionBound node3 的 pricing 输入快照，并在同一个节点上复放不同 `Tmid`，否则不同分支路径之间的结果不适合直接比较。

### 41.64 2026-06-06 非对称双向与 Tmid 二次校准分析

重新查非对称双向日志后，结论仍是：非对称路线差的核心不是“more/fewer 选边策略本身差”，而是真正动态 `HB/HF` 放宽后让当前 final-join 框架承受了更多原本会被静态 `Tmid` 截住的 label。固定 `dynamicHB=dynamicHF=Tmid` 的诊断版在 wet020/wet021 上和静态 full-domain 规模接近；例如 wet021 固定 Tmid 时 asymmetricMore/asymmetricFewer exact 约 `0.598s/0.747s`。临时全宽动态 HB/HF 后，wet021 asymmetricMore/asymmetricFewer exact 变成约 `14.848s/13.030s`，asymmetricMore 日志里 forward kept 约 `11k`、backward kept 只有 `2-4`，仍然是单侧膨胀。因此它没有解决“左右均衡”，反而把边界放宽后的状态空间暴露出来。后续不应把这个旧非对称分支作为主优化方向，只能当诊断参照。

关于用户指出的 Tmid1/Tmid2 现象，当前解释为：`Tmid2` 变小后 forward label 显著减少，不一定说明 completion bound 在 `Tmid1` 下应该能把这些 label 全剪掉。半域算法中 Tmid 是硬扩展边界，label 超过 Tmid 后就不再生成；completion bound 是在已经生成 child label 后判断“松弛补全后是否仍可能形成负 reduced-cost 列”。这两个机制不等价。若 `Tmid1` 较大，`(Tmid2,Tmid1]` 这段时间内会生成大量 forward child；这些 child 即使在真实 elementary pricing 中大概率没用，只要 all-cycles completion bound 给出的松弛补全仍可能为负，就不能被剪。也就是说，Tmid 边界能直接阻止状态生成，而 completion bound 只能用一个偏乐观的下界剪枝，不能承担“让左右任务量均衡”的职责。

013 hard node 的数据正好支持这个判断。`completionBound` 已经把 `Tmid` 从默认 `1792.5` 拉到 `563.5`，但 forward 仍在 180 秒内达到 `fwKept=215008`，同时 `cbFPruned=1354729`。这说明 completion bound 并非没发挥作用，而是已经剪掉了大量状态后，剩下的 forward survivor 仍然太多。ratio 0.10/0.15/0.20 的快速结果也说明 Tmid 再往左可以明显减少 forward 状态，但那些运行落在另一条 depth=2 分支，不能证明同一 hard node 已被修复。

因此当前要解决的问题不是“怎么重新定义一个更小 U”，而是“在已有 U 或 reference 的基础上，如何找一个更均衡的 split point”。只用 makespan/last completion 平均值容易被少数尾部任务拉右；这解释了为什么 `columnLastAvg` 和 `completionBound` 虽然比 `pricingHorizon` 小很多，仍可能偏右。`columnHalfAvg` 曾尝试用列内中间任务完成时间，但它可能又过左，导致树路径和后向/拼接侧不稳定。

可行的后续方案有三类。第一类是基于列内 timing 分布直接估计 split，而不是用列末 makespan。当前 `TWETColumnEvaluator` 已能反推每条列中各任务的最优完成时间，可以对 reduced-cost 好的候选列收集所有任务完成时间，按列权重或简单平均得到“任务量中位时间/累计任务半数时间”。它比 `columnLastAvg` 更能反映大多数任务集中在哪里，也比 `columnHalfAvg` 单取中间位置更稳。实现成本较低，因为列 timing 已经在 column 策略中算过。

第二类是基于 completion-bound argmin 的分布，而不是取最大 argmin。当前 `completionBound` 策略用所有有希望函数的 argmin time 最大值做 reference，天然偏保守，可能被少数晚时间状态拉右。若改为统计负 bound 状态的 argmin 分布，并取一个偏右但非最大的位置，可以更接近“多数有希望状态”的时间尺度。不过这会改变之前“不用分位数”的约束，需作为新实验策略单独命名，不能直接覆盖现有 `completionBound`。

第三类是显式 probe/bisection。做法是在正式 exact pricing 前，对若干候选 Tmid 做预算很小的 dry-run：用同一套初始化、completion bound 和 dominance，但设置 pop/label/time 上限，只统计 `fwKept/bwKept/fwQueue/bwQueue/cbPruned`，不生成列、不改主问题。若 forward 明显大于 backward，就把 Tmid 往左移；若 backward 明显变大，则往右回。这个方法最贴近用户说的“试探左右 label 是否均衡”，但需要严格限额，否则会把一次 pricing 变成多次 pricing。比较现实的第一版不是完整二分，而是只测 2-3 个候选点，例如 `0.35*reference`、`0.50*reference`、`0.65*reference`，每个最多几千 pop 或几十毫秒，然后按 imbalance score 选一个。若 probe 命中 013 这种 hard node，它可能用很小代价提前发现 `563.5` 仍偏右。

当前建议的优先级是：先做列内 timing 分布策略，因为它改动最小、可解释性强；再做预算 probe 作为 hard-node 诊断或 fallback；completion-bound 分布策略放在第三位，因为它会改变已有 `completionBound=max argmin` 的语义。无论哪种，都应保留当前 `completionBound` 和 `columnLastAvg` 作为基线，不能直接替换默认策略。

### 41.65 2026-06-06 completion bound 半域语义与列内任务量 Tmid

继续复核代码后确认，当前 `GCNGBBStyleBidirectional` 在准备 pricing 时先用默认 `Tmid` 建一次基础 half-domain cache，这是为了让根节点动态窗口、backward 窗口等已有预计算流程能沿用同一套对象；但 `completionBound` 构造不使用最终 half-domain penalty，而是单独通过 `buildCompletionBoundPenalty()` 构造 `[0, pricingHorizon]` 上的完整 penalty。若 midpoint 策略为 `completionBound`，流程是先用默认 `Tmid` 建临时 cache，再建 completion bound，随后根据 bound 的 argmin time 重算最终 `Tmid`，最后重建 half-domain cache。也就是说，completion bound 只受 `pricingHorizon`、有效 hard/profitable window、禁弧和 reduced penalty 影响，不应该被最终左右半域裁剪直接限制。

用户关心的“任务量意义上的中间时间”可以这样刻画：不要只看每条列的最后完工时间，也不要只取每条列中间位置任务的完工时间，而是对当前节点中 reduced cost 较好的候选列，先用完整 PWLF timing 反推出列内每个任务的最优完成时间，再把这些完成时间汇总成一个经验分布。然后取这个分布的中心位置，例如按任务出现次数统计的中位完成时间，或按列权重/列质量加权后的中位完成时间。它表达的是“当前好列中的任务完成时间有一半落在这个时间左侧”，比 `columnLastAvg` 更不容易被少数尾部任务拖右；又比 `columnHalfAvg` 更稳，因为后者每条列只取一个中间位置，忽略了整条列内任务完成时间的分布。

如果实现一个新策略，比较合理的第一版是命名为 `columnTaskMedian` 或 `columnTaskCenter`。候选列选择仍沿用当前 column 策略的逻辑：restricted columns 按 reduced cost 排序，取前 `bidirectionalMidpointColumnLimit` 个兼容列；每条列调用 `TWETColumnEvaluator.evaluateTiming(sequence)` 得到完整 `completions`；把所有 completion time 放入一个数组或小型分桶。因为 30 任务、候选列限制一般不大，直接排序这些 completion time 就够了。最终 `reference` 可以取任务量中位数，`Tmid` 直接取这个 reference，或者更保守地取 `(left + reference) / 2`。从当前现象看，若目的是抵消 `columnLastAvg` 偏右，直接取任务量中位数更符合动机；但需要作为单独策略测试，不能覆盖现有 `columnLastAvg`。

预算 probe 的思路和 MIP 里的 probing/diving 类似：临时进入一个可回滚的探索过程，改变局部选择或变量边界，收集质量信号，必要时把发现的可行解提交回主模型。SCIP 文档中 probing mode 的要点是可以临时修改变量界、加约束、做传播甚至列生成，随后回滚；Gurobi 的 heuristic callback/MIP start 思路则是外部启发式可以提供候选解，但主求解器仍负责验证和接受。映射到这里，probe 不应修改 RMP，也不应把临时 label 全部当正式 pricing 结果。它只在几个候选 `Tmid` 上做严格限额的 dry-run，统计 forward/backward label 压力；若 dry-run 已经发现负 reduced-cost 列，可以把这条列作为普通候选列加入池，但不能因为 probe 没找到就断言无负列。

imbalance score 可以用两个层次。第一层只看最终保留/生成规模，例如 `score = abs(log((fwKept + 1) / (bwKept + 1)))`，同时加一个总量惩罚 `alpha * log(fwKept + bwKept + 2)`，避免两边一样多但都很大的候选。第二层加入排队压力和 completion-bound 效果，例如用 `fwPressure = fwBuilt - cbFPruned`、`bwPressure = bwBuilt - cbBPruned` 或者 `queuePeak` 替代 kept。更稳的第一版可以只用 `kept + queuePeak`，因为这些统计最直接反映后续内存和 dominance 压力。候选 `Tmid` 的选择不需要完整二分，先围绕 column/reference 测三点即可，例如 `0.65*reference`、`0.85*reference`、`1.05*reference`，每点限制几千 pop 或几十毫秒。若某点触发预算上限，score 直接加罚。

当前不建议继续推进 completion-bound argmin 分布策略。原因不是它完全没价值，而是用户当前判断是对的：bound 函数数量有限，并且它是 all-cycles/two-cycle 松弛后的时间信号，和真实好列的任务完成时间分布仍有偏差。后续主线应优先基于当前节点较好列的 timing 分布做统计，再考虑 budget probe 作为 hard-node fallback。

### 41.66 2026-06-09 formal subtree 变慢与双向不对称的关系

重新对照 subtree on/off 的旧日志后，当前判断是：双向不对称确实会放大某些节点的 pricing 压力，但 formal subtree 变慢不能只解释成“双向不稳定”。更准确地说，formal subtree 先改变了子节点的受限列池和 RMP dual，随后当前 half-domain pricing 的不对称结构把这种变化表现成 forwardSink 方向的大量负列枚举。

典型证据来自 `tmp-wet030_001_2m` 的 node2。subtree off 时，node2 第一轮 exact pricing 为 `addedColumns=45`，`fw kept/dominated=1442/42`，`bw kept/dominated=0/0`，`forwardSink negative=45`，restricted columns 约 `4801`，`machineDual=-9198`。formal subtree on 时，同一层 node2 变成 `addedColumns=69952`，`fw kept/dominated=121210/77343`，`bw kept/dominated=0/0`，`forwardSink negative=69952`，restricted columns 约 `2003`，`machineDual=-8916.95`。两边 backward 都几乎没有参与，因此慢点不是“方向从均衡变成不均衡”，而是 formal subtree 改变列池/dual 后，forward direct-sink 枚举规模从几十条负列跳到近七万条。

这也解释了为什么同样禁弧数量并不必然变慢。pricingOnly subtree 在相近子节点上只把禁弧写入 pricing 图，不改变正式 child RMP 的列过滤和分支约束语义；旧记录里它反而把 node2 第一轮负列从 `45` 降到 `16`。formal subtree skip child filter 的诊断也没有复现最坏爆炸。由此可见，禁弧本身不是充分原因；真正危险的是 formal 禁弧进入 child restricted pool/dual 之后，产生了更容易出现大量负 reduced-cost forward 完整列的对偶环境。
...
### 41.75 2026-06-16 completion-bound delta propagation 实验实现

按“只新增、默认不改变现有流程”的原则，本轮实现了一个默认关闭的 all-cycles completion-bound delta propagation 实验开关：`twet.bpc.completionBoundDeltaPropagation=true`。旧的 `buildAllCycles()` 保持不变；two-cycle bound 也不接入该实验路径，避免把不同因素混在一起。

具体改动分两层。第一层是在 `PiecewiseLinearFunction.mergeMinimum()` 外增加 `mergeMinimumWithChangeHull()`，在 lower-envelope 合并时记录 candidate 真正把当前函数改小或扩展定义域的时间区间。多个离散改善区间第一版先合并成一个 hull，即只记录 `[changedStart, changedEnd]`。这样不会改变原有 `mergeMinimum()` 的 boolean 语义，也不会影响默认调用方。

第二层是在 `CompletionBoundCalculator` 中新增 `buildAllCyclesDelta()`。状态仍保存完整的 `forwardF/backwardB` 函数，用于最终 completion bound 和剪枝；队列中额外携带本轮改小的区间 hull。forward 传播时，为了覆盖 prefix-min 后局部改善可能向右侧扩散的影响，实际传播 `cropToInterval(forwardF[state], changedStart, pricingHorizon)`；backward 传播时，为了覆盖 suffix-min 后局部改善可能向左侧扩散的影响，传播 `cropToInterval(backwardB[state], 0, changedEnd)`。同一 state 多次入队时，先把 pending 区间合并成一个 hull，减少重复队列项。

这个实现是保守的，但不是最终理想形态。它不会因为只传播原始小区间而漏掉 prefix/suffix normalize 的影响；但 hull 可能非常宽，尤其 backward 侧一旦 `changedEnd` 偏右，就接近传播整条函数。同时每次传播都要从完整函数裁剪子函数，若函数段数已经很大，`cropToInterval()` 本身也会成为额外成本。因此它适合作为正确性和收益方向的第一版验证，不应直接替代默认 bound。

初步测试结果支持这个判断。30 任务三角化 `tmp-wet030_from040_010_2m` 的 root-only smoke 中，默认路径为 `solve=12.542s, exact=4.243s, calls=3, obj=bound=14318, valid=true`；打开 delta 后为 `solve=12.417s, exact=3.567s, calls=3, obj=bound=14318, valid=true`，说明数值口径至少在该小例上对齐，且有小幅收益。

但在目标慢点 `wet040_001_2m` 的 zero-setup root-only 上，delta 路径反而更慢。默认旧路径此前记录约为 `solve=188.930s, exact=161.717s, calls=10, valid=true`；打开 delta 后为 `solve=213.237s, exact=172.704s, calls=10, valid=true`。日志显示 delta 版本第一轮 bound merge 次数从约 `63796` 降到 `54226`，但 build 时间从约 `15.111s` 升到约 `19.162s`，说明减少的 merge 次数被更宽的区间裁剪、重复子函数构造和仍然很大的 PWLF 段数抵消了。

因此当前结论是：delta propagation 的方向仍然合理，但“单 hull + 从完整函数反复 crop”的第一版不够有效，不能默认开启。后续若继续优化，应改成更细的多区间 delta 或直接传播 delta 子函数，避免每次从完整 envelope 重新裁剪；同时需要记录 changed interval 覆盖率和 crop 后段数，判断真实瓶颈是 hull 过宽还是裁剪成本过高。

### 41.76 2026-06-16 mergeMinimum no-change 快路径与精确段压缩

根据前一节实验结论，本轮没有继续加复杂的多区间 delta，而是先处理更直接的瓶颈：`mergeMinimum()` 本身的无效合并和 normalize 后碎片段。具体做法是，在 `mergeMinimum()` 复制右函数和改写链表之前，先做一次只读扫描。若右函数没有扩展当前定义域，并且在所有正长度公共区间上都不低于当前 lower envelope，则直接返回 `false`。这样可以跳过右函数 copy、交点拆段、链表替换和 prefix/suffix normalize。这个判断只在“确定不会改变 envelope”时跳过，因此不改变 bound 语义。

同时在 `normalizeForward()/normalizeBackward()` 的 prefix/suffix-min 后增加一次精确相邻段压缩。这里只合并 slope/intercept 完全相等、且端点连续的相邻段；不做近似压缩，不抬高函数值，也不删除零长度段。后者是本次复查后特意收紧的地方，因为 PWLF 底层和 single-point label 历史上都保留过单点语义，公共类里不能把零长度段当作无害垃圾段统一清掉。

测试结果明显好于上一版 hull delta。30 任务三角化 `tmp-wet030_from040_010_2m` root-only smoke 中，结果仍为 `obj=bound=14318, valid=true`，安全版耗时为 `solve=9.929s, exact=2.571s`。40 任务 `wet040_001_2m` zero-setup root-only 中，结果仍为 `incumbent=17887, bound=17866.666667, valid=true`，安全版耗时为 `solve=162.423s, exact=124.784s`。对比旧路径约 `solve=188.930s, exact=161.717s` 仍有明显收益；对比前一版 hull delta 的 `solve=213.237s, exact=172.704s` 也更好。中间曾试过删除零长度段的激进压缩，40 zero-setup 可到 `solve=136.439s, exact=101.692s`，但复查后认为该行为可能影响单点域语义，已撤回。

当前应保留这组优化作为默认路径，但口径必须是“no-change 快路径 + 相同连续段合并”，不是删除单点段。多区间 delta 只有在进一步压缩 `cropToInterval()` 和 delta 子函数构造成本后再考虑，否则容易重复上一版 hull 的问题。

同一安全版随后重跑一次 40 任务 zero-setup root-only，结果为 `solve=137.604s, exact=111.548s, calls=10, incumbent=17887, bound=17866.666667, valid=true`。该结果和上一轮安全版 `162.423s/124.784s` 存在运行波动，但仍稳定优于旧默认 `188.930s/161.717s`，也明显优于 hull delta 版本。第一轮 completion-bound build 为 `8.737s`，其中 forward/backward 约 `0.461s/8.266s`；merge 次数仍为 `63796`，说明优化主要不是减少状态/merge 次数，而是让 no-change merge 提前返回，降低每次 merge 的 copy/normalize 成本。

### 41.77 2026-06-19 label-derived bound 同轮更新的重新分析

重新对照旧 VRP `GCNGBB` 后，前面对“forward 刚结束不能立刻更新 bound 去剪 backward”的解释需要修正。旧代码的真实流程就是 `FWExtend -> UpdateFWBound -> BWExtend -> UpdateBWBound -> Join`，因此同一轮内用 forward label 更新 bound 再服务 backward 扩展，本身不是问题。

当前 TWET 出现 011 root 过剪的更可疑差异在函数候选的未知区间语义。旧 VRP 的 `UpdateFWBound()` 先把临时数组填成 `BigNumber`，但只有 `m_lbt_bound[i]` 是有限值时才执行 `m_ft_bound[cid][i] = max(m_ft_bound[cid][i], m_lbt_bound[i])`；未知的 `BigNumber` 格点会被跳过，不会拿去抬高已有 bound。

TWET 中间版本为了把半域 label 函数平移到完整 pricing horizon，曾使用 `fullHorizonWithBigMGaps()` 显式补 `big_M` gap，然后把候选函数拿去和已有 completion bound 做 `pointwiseMax`。在这个语义下，`big_M` 不再表示“未知、不要更新”，而会变成一个极强下界参与 max，可能把候选函数真实支持区间以外的部分也抬到 `big_M`。这和旧 VRP 的“只用有限 label bound 更新”不一致，也能解释为什么 011 root 会出现 backward labels 被直接剪空、generated=0 的现象。

当前代码已先退回到更接近旧 VRP 的同 terminal 更新：forward 队列耗尽后，直接用当前轮到达同一个 terminal job 的 forward label no-SRI envelope 更新 `forwardUByJob[j]`；backward 队列耗尽后，直接用同一个 first job 的 backward label no-SRI envelope 更新 `backwardRByJob[j]`。这一步不再做额外一跳扩展，也不再把未知区间补成 `big_M` 后送入 max。`finiteSupportOnly()` 会在更新 `U/R` 前剥掉 `big_M` 段，只让有限支持区间参与 `pointwiseMax`；支持区间之外保留原 completion bound。

验证方面，focused `javac` 已通过。用 010/011 root 的 no-probe、ng-DSSR、label-bound-update smoke 直接长跑时，30/60 秒内没有快速返回；这至少没有复现之前 011 root 快速 generated=0 的错剪现象，但该 smoke 口径不适合作为快速性能结论，后续需要用正式批处理配置重新比较 label-bound-update on/off 的节点数、exact calls、bound 和 generated 列数。

### 41.78 2026-06-19 旧 GCNGBB label-derived bound 的实际作用边界

继续复查旧 VRP `GCNGBB` 后，需要把 forward/backward 两侧的作用区分开。旧代码每轮顺序是 `FWExtend -> UpdateFWBound -> BWExtend -> UpdateBWBound -> Join`。因此本轮 `FWExtend` 使用的是初始化 bound 或上一轮 DSSR 的 backward 更新；本轮 `UpdateBWBound` 主要服务下一轮 forward，而不是刚刚已经完成的 forward。

对 backward 来说，本轮 `UpdateFWBound` 的确可以立即被 `BWExtend` 使用。这里容易误解的是，`BWExtend` 查询 `m_ft_bound[i][it]` 时，`it` 是留给 forward prefix 的剩余时间预算，不是 backward label 自己的时间。若 `it <= T/2`，说明 backward suffix 已经占用了大半个 horizon；但旧 `BWExtend` 没有对称的 `T/2` 停止条件，只检查完整 horizon 可行性。因此这个长 suffix 仍会继续 prepend 更多点，forward 更新得到的短 prefix bound 能在这里提前剪掉一部分长 suffix 扩展。

这解释了为什么旧 GCNGBB 的更新不是完全没用：它依赖的是 forward 半程、backward 可长扩展的不对称结构。若改成当前 TWET 主线这种正反两侧都按 Tmid 半域停止、再做 arc join 的结构，同 terminal label-derived bound update 和半域停止高度重叠，边际作用会明显变弱。本质区别不是公式，而是旧代码的 backward 搜索没有被 T/2 对称截断。

### 41.79 2026-06-19 对旧 GCNGBB bound 更新收益的进一步收束

进一步对照旧 `GCNGBB` 的 node-join 流程后，需要把“能剪 label”和“能减少扩展”分开。旧 `BWExtend` 中的顺序是：先用 `m_ft_bound[i][it]` 做 time-bound 检查；通过后构造 `lb`；然后调用 `BWIsDominate(lb, lp)`，该函数会把未被占优的 label 放入 `BWTL`；最后只有 `lb.m_time < max_time/2` 时才把它加入 `BWUL` 继续扩展。因此超过半域的反向 label 不再继续扩展，但仍可能进入 `BWTL` 参与 node join。

由此可知，当反向 label 已经跨过半域、并且查到了正向小于一半资源的 bound 时，这个更新主要减少的是 `BWTL/FWTL` 中边界 label 的数量，以及后续 `Join()` 的组合数和 bound 更新时扫描的 label 数。它并不一定减少后续扩展树，因为这类 label 本来就不会继续入队扩展。若实际瓶颈主要在 label 扩展、dominance 或 PWLF 函数操作，而不是 node join，那么这部分收益会比较有限。

对 TWET 当前主线的 arc-join 双半域结构，这个收益窗口更窄。正反两侧的 label frontier 本来就集中在 `Tmid` 左右；若某个 label-derived bound 能剪掉对向 label，通常意味着对向 label 下一步扩展已经接近或跨过半域边界。此时它能节省的更多也是 join 候选，而不是核心扩展。因此该更新对 node-join 可能还有一定压缩作用，对 arc-join 基本不应期待明显收益。当前决定：`ngDssrLabelDerivedCompletionBoundUpdate` 继续作为诊断/实验开关保留，默认不启用；近期不再继续沿这个方向做主线优化。

### 41.80 2026-06-19 撤销 label-derived bound 更新代码

基于 41.79 的结论，当前不再保留 `ngDssrLabelDerivedCompletionBoundUpdate` 的代码实现和配置入口。撤销范围包括：ng-DSSR pricing 中每轮 relaxed labeling 后的 forward/backward label envelope 聚合、U/R completion bound 轮间增强、相关 pointwise-max 辅助函数、增强 bound 与 subtree reusable bound 的区分逻辑，以及 `TWETBPCConfig` 和 `GCBBFullDomainComparisonTest` 中的开关字段/系统属性读取。

撤销后，ng-DSSR 仍只使用标准 `CompletionBoundCalculator` 构造的 base completion bounds；这些 bounds 继续服务 label 剪枝、scalar bound、completion-bound arc fixing 和 pricingOnly subtree 复用。旧 GCNGBB 对 label-derived bound 的分析保留在文档中，作为后续研究 node-join 或非对称双向时的参考，但当前 arc-join 双半域主线不再维护该实验路径。

### 41.81 2026-06-20 有外包时 completion-bound arc fixing 的适用条件

当前 completion-bound arc fixing 有两层口径。第一层是在 pricing 轮内使用 `F_i + (setupCost(i,j)-arcDual(i,j)) + B_j` 判断某条内部 arc 是否不可能出现在负 reduced-cost 内部列中，判定阈值是 0，只服务当前 exact pricing 的扩展剪枝。第二层是 subtree arc elimination，在当前 node 的 price-and-cut 已闭合后，用 `incumbent - node LP bound` 作为 gap；如果任何包含 arc `(i,j)` 的内部列 reduced-cost 下界都不小于该 gap，就把该 arc 继承性地禁用到子节点，或者只写入 pricingOnly 禁弧。

外包变量本身不破坏这个 reduced-cost fixing 逻辑。原因是这里固定的是“内部机器列里是否允许使用某条 arc”，不是固定 job 是否外包。外包变量 `y_j` 和 tariff segment 不包含内部 arc，因此不在被固定对象集合内。只要当前 node LP 已经在含外包 master 上闭合，外包变量和 tariff 变量的 reduced cost 在该 LP 对偶下也应为非负；若某个更优整数解使用了包含 arc `(i,j)` 的内部列，那么它的 objective 至少为当前 LP bound 加上该内部列的 reduced-cost 下界，再加上其它非负 reduced-cost 变量贡献。因此当该下界已经不小于 `incumbent - LP bound` 时，任何使用该 arc 的更优解都不存在。解中其它 job 是否外包不影响这个推导，因为外包只是在“其它变量贡献”里出现。

需要注意三个条件。第一，subtree 口径必须在 node 的 pricing/cut loop 闭合之后使用；如果仍存在未生成的负 reduced-cost 内部列或外包相关 cut 改变了 dual 但没有重新 pricing，则不能用 `UB-LB` 做继承 fixing。当前 `Tree` 中的调用位置是在 `PC.solve()` 之后、分支之前，符合这个时机。第二，completion bound 必须只作为内部列 reduced-cost 的下界使用，不能拿它去固定外包变量，也不能解释为“job 不会被外包”。第三，当前 hard-window preprocessing 会用单 job outsourcing baseline `b_j` 裁剪内部完工时间窗口；这要求 `b_j` 对“把 job 外包出去的可接受替代成本”是安全上界。若某些 tariff 使外包边际成本可能大于 `b_j`，则风险来自 hard window 本身，而不是 arc fixing 公式；这种情况下应放松 hard window，或给窗口裁剪单独使用更保守的外包成本上界。这里不是建议直接把 `data.outsourcingCost[j]` 改成 `big_M`，因为当前 LP 会把 `big_M` 解释为该 job 不允许外包。

因此结论是：有外包时可以使用当前 completion-bound arc fixing，但语义必须限定为“内部列 arc fixing”。它在 pricing 轮内剪内部负列、在节点闭合后做 pricingOnly/formal 子树禁弧都是可以的；不应把它扩展为外包决策 fixing，也不应在 LP 未闭合、cut 后未重新 pricing、或外包 hard-window 上界不安全时使用。

### 41.82 2026-06-28 time-indexed 最短路强化 scalar completion bound 的分析

当前 completion bound 的 scalar 剪枝已经按 job 和离散时间缓存了 `forwardUBeforeByJob` 与 `backwardRAfterByJob`。forward label 判断时使用同 job 的 `backwardRAfterFloor(job, earliestTime)`，backward label 判断时使用 `forwardUBeforeCeil(job, latestTime)`。因此如果要引入 time-indexed 图上的最短路信息，最自然的位置不是改 PWLF completion bound 函数，而是新增一组 time-indexed scalar 下界，并和现有 scalar cache 取更强的下界。

这个思路与此前撤销的 label-derived bound update 不完全相同。此前尝试是用当前 DSSR 轮已经生成的 label envelope 去抬高 `forwardU/backwardR`，在当前 arc-join 双半域结构下主要只能减少边界 join，收益弱且维护复杂。现在讨论的是在同一 LP dual 下，在 time-indexed relaxed graph 上求从 `(job,t)` 到 sink 或从 source 到 `(job,t)` 的最短 reduced cost；该图允许重复任务，是比 elementary/ng 路径更松的 relaxation，因此得到的是合法下界，可以用于 completion-bound scalar 剪枝。

安全边界也比较清楚。time-indexed 最短路只应强化 scalar bound：例如 forward label 到达 job `i`、最早完成时间为 `t` 时，可以查 time-indexed 后缀 `SP_suffix(i,t)`；若 `label.minReducedCost + max(existingScalar, SP_suffix)` 已经不小于 cutoff，则剪掉。反向 label 类似查 prefix `SP_prefix(i,t)`。它不能直接替代 PWLF 函数 bound，也不能作为 elementary 路径结构证明；更不能在 active SRI 或不一致的 dual/window 下混用，否则容易出现 cut state 或路径 identity 口径不一致。

实现上第一版建议只做诊断开关。每次 ng-DSSR exact pricing 开始时，若当前 dual、node 禁弧、pricingOnly 禁弧、horizon 和窗口口径固定，则构造一个 time-indexed forward/backward distance 表，并把已被 time-indexed arc fixing 删除的 `(from,to,t)` 视为不可用。然后导出两类离散 scalar：`prefixMin[j][t]` 表示 source 到 `(j,t)` 的 relaxed 最小 reduced cost，`suffixMin[j][t]` 表示 `(j,t)` 到 sink 的 relaxed 最小 reduced cost。现有 completion bound scalar 查询时，只在同一时间方向上取更大的下界；如果 time-indexed 表没有值，则回退现有 scalar，不改变默认语义。

收益上需要谨慎。time-indexed arc fixing 在 40-2 rank1/cut root 上能把几百万级时空弧删到很少，说明图上距离表可能相当强；但在 ng-DSSR 主线里同步维护这张表会带来额外内存和计算成本，尤其每个 DSSR 轮或每次 dual 改变都需要重新计算。它更适合先做 root/hard node 的观测：统计 scalar 查询次数、现有 scalar 可剪数、time-indexed scalar 额外可剪数、构造耗时和 label 扩展减少量。只有确认额外剪枝能覆盖构造成本，再考虑默认接入。

### 41.83 2026-06-28 time-indexed scalar helper 第一版实现与 smoke 结果

当前已增加一个独立的 `TimeIndexedScalarCompletionBound` helper，默认关闭，只在 ng-DSSR 系列 pricing 显式打开时使用。它不生成列，也不替代现有 PWLF completion bound，而是在非根节点用当前 LP dual、branch 禁弧、pricingOnly 禁弧和当前任务窗口构造允许重复任务的 time-indexed relaxed graph。图上同时计算 source 到各 `(job,t)` 的 prefix 最短路和各 `(job,t)` 到 sink 的 suffix 最短路，再导出按 job/time 查询的 scalar 下界。

在 ng-DSSR exact pricing 内部，forward label 查同 job 的 time-indexed suffix，backward label 查同 job 的 time-indexed prefix，并与现有 scalar completion bound 取更强者。若新的 scalar 让原本不能剪的 label 被剪掉，会记录到 `timeIndexedScalarExtraPruned`。整数时间实例上还会用 time-indexed 图中同时 forward/backward 可达的时间点收紧当前 node 内部的 effective pricing window；小数时间实例只做 scalar bound，不做 hard-window 收紧。小数离散化使用向下取整的处理/设置时间和当前 bucket 内的最低 penalty，作为原连续问题的松弛下界。

此外，在 node 已闭合、准备 subtree arc elimination 和分支之前，若仍是 ng-DSSR 系列 pricing，会用同一类 time-indexed graph 和当前 `incumbent - node LP bound` 做 time-indexed reduced-cost arc fixing，把可继承的时空弧写入当前 node 的 pricingOnly time-indexed 禁弧集合。这个 fixing 只作为后续子节点和枚举/时空图辅助的剪枝信息，不改变内部列定义，也不作用于非 ng-DSSR 主线。

初步 smoke 用 `data/40-2/wet040_001_2m.dat`、ng-DSSR、`maxNodes=2`、强分支关闭、helper 三个开关全部打开。结果 `NODE_LIMIT, valid=true, root=128.697s, total=168.479s, processed nodes=2`。root 因 depth=0 不启用 helper；node 2 中 time-indexed scalar build 约 `90-104ms`，部分 exact 轮次记录到 `improved=1453-1819`、`extraPruned=18-19`，说明内部 scalar 的确能额外剪掉少量 label。node 2 闭合后的 time-indexed arc fixing 规模很大：`candidates=3482139, fixed=3322282, unavailable=98663, process/idle/end=3175929/69602/76751, ms=3809.853`。因此当前判断是：time-indexed 图在非根节点能提供强的时空弧删除信息，但作为 ng-DSSR scalar label 剪枝的直接收益暂时偏小；是否值得默认启用还需要在更难 node 上比较其建图/fixing 耗时与后续节点减速幅度。

### 41.84 time-indexed arc fixing 后的窗口剩余统计

针对 wet040_001_2m 做了一个只到 maxNodes=2 的诊断运行，确认前一版命令误用了 mode=full，实际跑到 full-domain pricing；修正为 mode=normal + ngDssr=true + ngDssrInitialMode=nearestK 后，node 2 可以触发 `timeIndexedScalarArcFixing`。

本次 node 2 的 time-indexed arc fixing 结果为：候选时空弧 3,482,139，删除 3,313,050，其中 process/idle/end 分别为 3,169,108 / 68,097 / 75,845，耗时约 2.42s。删除后所有 40 个 job 仍有可达时间，但 40 个 job 的可达时间 hull 都被压缩。平均原始离散时间点约 2133.0，平均剩余 hull 约 437.1，平均真实可达点约 378.8。收缩最大的样例包括 j29:897-923/old0-2132、j35:755-790/old0-2132、j18:917-979/old0-2132 等，最大 hull 收缩为 2106 个时间点。

因此，scalar bound 本身对 label 剪枝的直接效果较弱，但 time-indexed arc fixing 后提取出的 job 时间窗口强化潜力很强。需要注意的是，本次日志先显示 windowMode=relaxed，原因是实现错误地要求 pricingHorizon 也为整数；实际 horizon 只是局部上界，可以是 2131.8，只要 processing/setup/window 是整数，使用 ceil(horizon) 的 time-indexed 图仍是整数时间精确图。已修正该检测逻辑。

### 41.85 2026-06-29 time-indexed scalar/window helper 复查结论

本次复查确认，`TimeIndexedScalarCompletionBound` 的数学口径整体是安全的：它构造的是允许重复任务的 time-indexed relaxed graph，得到的 prefix/suffix 最短路只作为 reduced-cost 下界使用，并且在 ng-DSSR 中只和已有 scalar completion bound 取更强者。小数时间实例下使用向下取整 duration 和 bucket 内最小 penalty，本质上仍是松弛下界；整数时间实例才允许做窗口收紧。`Tree` 中的 node 闭合后 fixing 调用也只在非根节点、非 time-indexed graph pricing、ng-DSSR 系列 pricing 显式启用时触发，默认配置仍关闭，因此不会改变当前主线默认结果。

当前需要重点注意的是工程实现方式，而不是下界公式本身。node 闭合后的 time-indexed arc fixing 会逐条把 `(from,to,t)` 写入 `Node.timeIndexedPricingOnlyForbiddenArc`。在 40 任务诊断节点上已经固定约 331 万条时空弧，若直接继承到子节点，会带来很大的 `HashSet<Long>` 内存、复制和 contains 成本。后续若目标是利用 fixing 后的硬时间窗强化 ng-DSSR，不应沿用“逐条保存所有时空弧”的方式，而应在本地完成 fixing 和 reachability 后提取每个 job 的可达时间 hull，只把紧缩后的 node 级 effective window 传给后续 pricing；只有 time-indexed graph pricing 实验线需要继续保留逐时空弧禁用信息。

另一个边界是 SRI/cut。当前 helper 没有显式维护 SRI 状态，忽略 SRI 成本时一般只会得到更松的下界，作为 scalar pruning 是偏保守的；但在正式默认启用前，仍建议先限定在 no-SRI 的 ng-DSSR 主线或单独复查 cut dual 符号，避免不同 cut 口径下误把实验 helper 当成完整证书。对于小数数据，目前不建议直接由 relaxed bucket 图反推硬时间窗；若确实要做，需要先把数据按统一比例 scale 到整数时间，否则图上一个 bucket 内的边成本和完成时间语义不够精确。

因此下一步如果实现“time-indexed fixing 后硬窗口强化”，建议最小落地方式为：只在非根节点、整数时间实例、no-SRI 或已确认 cut-safe 的配置下启用；在当前 node 闭合后用本地 bitset/数组完成 time-indexed fixing 和 forward/backward reachability，提取每个 job 的 `[minReachableTime,maxReachableTime]`，再作为子节点继承的 effective pricing window 使用；不要把数百万条时空弧写入 `Node` 作为 ng-DSSR 主线状态。

### 41.86 2026-06-29 compact time-indexed job window 实现

本轮按 41.85 的结论做了实现调整。`TimeIndexedScalarCompletionBound` 在 node 闭合后的 time-indexed arc fixing 中不再把每条 `(from,to,t)` 禁弧写入 `Node`，而是在 helper 内部用本地 `BitSet` 记录当次固定的时空弧，重新计算 forward/backward reachability 后，只把每个 job 的可达时间 hull 写成 node 级 compact window。这样 ng-DSSR 主线继承的是每个 job 的 `[start,end]`，避免复制数百万条时空弧到子节点。原来的逐条 time-indexed 禁弧结构仍保留给 time-indexed graph pricing 实验线使用。

`GCNGBBStyleBidirectionalNgDssr` 的 effective pricing window 预处理也做了对应合并：先取原始 hard window，再按需与 dual profitable window 取交集，最后与 node 继承的 time-indexed window 取交集。这个窗口会继续用于扩展可行性判断、函数定义域和本轮 `pricingHorizon` 的收缩，语义上等价于多了一层可继承硬窗口。`Tree` 在 node 闭合后允许 root 也执行 time-indexed scalar arc fixing，因此 root 初始 pricing 仍只依赖原有 dual window；root 闭合后若图上能提取整数时间窗，则传给子节点使用。

整数判定也同步收紧到原始数据层面：`pricingHorizon` 和 dual/effective window 端点可以是小数，不再阻止整数实例写 compact window；真正决定能否反写硬窗口的是 processing time、setup 和原始 hard window 是否都是整数。小数时间实例仍只使用 relaxed time-indexed scalar/fixing 统计，不把 bucket hull 写成主线硬窗口，避免在未 scale 的连续时间问题中误改函数定义域。

验证使用 `data/40-2/wet040_001_2m.dat`、ng-DSSR、`maxNodes=2`、time-indexed scalar/window/fixing 开关全开。结果 `NODE_LIMIT, valid=true`。该实例被判定为 `windowMode=relaxed`，因此 `nodeWindowTightened=0`，但 node 2 闭合时仍能看到 time-indexed fixing 在 `1,557,324` 个候选中删除 `1,380,215` 条时空弧，40 个 job 的 reachable hull 均明显收缩。由此确认：本次实现没有破坏主流程；小数实例按预期不反写硬窗口；整数实例会走 compact window 继承路径。默认开关仍关闭，因此不影响现有主线配置。

### 41.87 2026-06-29 整数时间判定口径修正

进一步复查后确认，41.86 中“原始 hard window 是否整数”这个判定仍然偏保守。`hardWindowStart/End` 是由 due window、外包 baseline、`CmaxH` 等派生出来的预处理窗口，不是原始时间数据。尤其没有外包成本时，`hardWindowEnd` 往往等于 `CmaxH`，而 `CmaxH` 里包含 `sumP / m`，在两台机器时很容易出现 `.5`。这类小数端点不意味着 label 的实际完成时刻会落在小数点上；只要处理时间和 setup 时间是整数，扩展得到的完成时刻仍在整数桶上。派生窗口端点为小数时，只需在离散图中对左端取 `ceil`、右端取 `floor`。

因此 `TimeIndexedScalarCompletionBound` 的 `exactIntegerTime` 判定改为只检查原始离散时间数据：每个 job 的处理时间、setup 时间矩阵，以及原始 due window 左右端点 `d_e/d_l`。不再检查 `pricingHorizon`、dual/effective window，也不再检查预处理 hard window。若 due window 本身存在小数端点，则最优完工时间可能不局限在整数点，当前仍进入 relaxed 口径，不向 node 写 compact hard window。验证：focused `javac` 通过。该修正的目标是让整数数据不再因为派生窗口小数而错过 time-indexed window 收缩。

### 41.88 2026-06-29 整数判定修正后的硬窗口 smoke

修正后重新用 `data/40-2/wet040_001_2m.dat` 做 `maxNodes=2` smoke，配置开启 ng-DSSR、time-indexed scalar/window/arc-fixing，关闭 strong branching 与 route enumeration。结果为 `NODE_LIMIT, valid=true, solve=157.237s, root=121.331s, pool=11944`。

关键日志说明硬窗口确实生效。root 闭合后的 time-indexed fixing 显示 `windowMode=integer`，在 `3,484,221` 个候选时空弧中删除 `3,322,036` 条，并把 40 个 job 都写入 node 级 compact window：`nodeWindowTightened=40, windows reachable/tightened/unreachable=40/40/0`。平均原始时间点约 `2132.0`，收缩后的 hull 平均约 `384.1`，真实可达点平均约 `345.8`，最大 hull 收缩达到 `2109` 个时间点。

子节点继承也正常。node 2 出队日志中 `timeWindowJobs=40`，说明 root 写入的窗口已通过 `Node.copy()` 传到子节点；node 2 闭合后继续得到 `nodeWindowTightened=39, windowMode=integer`，平均 hull 进一步约为 `358.3`，真实可达点约为 `322.1`。因此当前结论是：整数判定修正后，time-indexed fixing 提取的硬窗口已经能够写入当前 node、传给子 node，并被 ng-DSSR 的 effective window 逻辑读取。需要注意的是，这个增强目前接入的是 ng-DSSR 主线；其它旧 bidirectional pricing 类没有统一读取该 node 级窗口，后续若重新启用那些实验线，需要单独接入或明确关闭该开关。

### 41.89 2026-06-29 completion bound 与 time-indexed 硬时间窗的口径确认

本次复查确认，ng-DSSR 主线里的 PWLF completion bound 会使用当前 effective pricing window。具体顺序是：先在 `precomputeEffectivePricingWindows()` 中合并原始 `hardWindowStart/End`、根节点可用的 dual profitable window，以及 node 继承下来的 time-indexed compact window；随后 `buildTimeIndexedScalarBoundAndTightenWindows()` 可以继续在本轮内收紧 `effectiveJobHStart/End`；最后 `precomputeCompletionBoundPricingWindows()` 用这组 effective window 构造 completion-bound 的 full-domain job penalty。因此，只要 `completionBoundRelaxation` 没有关闭，基础硬时间窗、root dual window 和子节点继承的 time-indexed 加强窗口都会进入 completion-bound 函数定义域，窗口外通过 `setDomain(hStart,hEnd,true)` 表现为不可用/大成本，再按现有 completion-bound 传播逻辑计算。

需要区分的是 node 末尾新算出的窗口和本 node 本轮已构造的 bound。time-indexed arc fixing / compact window 通常在 node 闭合后执行，它的主要作用是写入当前 node，供后续子节点继承；它不会在同一个 node 末尾强制重建刚刚用过的 PWLF completion bound。这样做是合理的，因为 node 已经完成 pricing closure，重新构造 bound 只会增加当前 node 尾部开销。若子节点开始 pricing，则这些继承窗口会在开头进入 `effectiveJobHStart/End`，从而同步影响 label 扩展、函数定义域、`pricingHorizon` 以及新构造的 completion bound。

因此当前语义可以概括为：completion bound 本身考虑 effective hard window；time-indexed 硬窗加强是跨 node 传递后生效，而不是回头重算当前 node 的 bound。若日志里显示 `completionBound mode=OFF`，则 PWLF completion bound 没有启用，此时硬窗口仍会影响 label 扩展和 time-indexed scalar/window helper，但不代表主 PWLF bound 也在工作。

### 41.90 2026-06-29 time-indexed window 的触发频率与启发式 pricing 口径

这里需要区分两类 time-indexed 信息。第一类是持久化的 compact window，它在 node 闭合后、分支前由 `Tree.applyTimeIndexedScalarCompletionArcFixing()` 触发，使用当前闭合 LP 的 dual、incumbent gap、branch/pricingOnly 禁弧做 time-indexed arc fixing，然后把每个 job 的可达时间 hull 写入当前 node，供子节点继承。这不是每轮列生成迭代都写一次；root 初始 pricing 也无法使用 root 自己尚未闭合时才会产生的 compact window，但 root 闭合后可以把窗口传给子节点。

第二类是 ng-DSSR exact pricing 内部临时构造的 `timeIndexedScalarBound`。它在 exact pricing 初始化阶段基于当前 effective window 和当前 dual 构造，只用于本次 pricing 调用中的 scalar 剪枝和局部 `effectiveJobHStart/End` 收紧；该收紧不直接写回 node，也不等价于 node 末尾的持久 compact window。由于 ng-DSSR 内部会复用这些 window 预处理结果，同一次 exact pricing/DSSR 内不会每轮都无条件重建。

当前 `HeuristicPricingEngine` 仍按 `data.penaltyFunction` 和 `data.CmaxH` 评价序列，只天然包含数据层已有的基础 hard window；它没有读取 node 继承的 time-indexed compact window，也没有套 root dual window/effective window。因此 time-indexed window 对 exact ng-DSSR label 扩展和 completion bound 有效，但对当前机器序列启发式 pricing 的搜索/评价没有直接剪枝作用。若后续希望启发式也受益，需要单独把 node effective window 接入启发式的 profile 构造。

### 41.91 2026-06-29 启发式 pricing 未使用 effective window 的影响

进一步检查 `HeuristicPricingEngine` 后确认，启发式 profile 构造使用的是 `data.penaltyFunction[job]` 和 `data.CmaxH`：首任务只把定义域设到 `[p_j+s_0j, CmaxH]`，后续拼接直接 `shiftX(...).add(data.penaltyFunction[job])`。因此它不会读取 ng-DSSR 当前收缩后的 `pricingHorizon`，也不会读取 `effectiveJobHStart/End`。这意味着 node 继承的 time-indexed compact window、root dual window、以及 exact pricing 内部临时收紧的窗口，都不会减少启发式的候选 move、profile 函数域或 tabu 搜索空间。

这解释了一个可能现象：time-indexed hard window 能明显减少 exact labeling 和 completion-bound 的有效定义域，但启发式 pricing 仍可能在旧的全局 `CmaxH` 和基础窗口上搜索，生成一些 exact pricing 已经不再需要考虑的序列，造成额外时间消耗或列池波动。当前没有把它接入启发式，主要是历史实现顺序导致的：启发式 pricing 早期是按旧 VRP `GCTabu` 的加速层实现，强调用真实列成本快速找负列，而 effective window 和 time-indexed compact window 是后续在 ng-DSSR exact pricing 中加入的剪枝信息，两者尚未统一。

如果后续要强化启发式，需要注意不能直接用 effective-window 后的 BigM 函数作为返回列成本。RMP 中的 `TWETColumn.cost` 必须是真实 TWET objective cost；effective window 只能用于候选过滤、搜索剪枝或快速判定“不值得尝试”，最终进入列池的序列仍应按原始 `data.penaltyFunction` 或 evaluator 口径得到真实成本。root dual window 尤其是 reduced-cost 相关的动态窗口，更不能直接改变列 objective cost。

### 41.92 2026-06-29 time-indexed fixing、分支约束和列成本口径

当前时序是先在已闭合的当前 node 上做 time-indexed scalar arc fixing，再分支生成子节点。该 fixing 使用当前 node 的 LP dual、当前 node 已有的 branch/pricingOnly 禁弧和 `incumbent - nodeBound` gap；得到的 compact job window 写入当前 node，随后通过 `Node.copy()` 继承到子节点。子节点新增的 branch arc 不会回头改变父节点已经提取出的窗口，而是在子节点开始 pricing、重新建 ng-DSSR 或 time-indexed helper 图时作为新的 `node.isArcForbidden()` 条件进入。因此逻辑是：父节点 fixing 先给出对子树都安全的窗口上界，子节点再叠加自己的分支禁弧继续收紧可行集合。

这里的安全性来自 reduced-cost fixing 口径，而不是简单的图可达性。父节点闭合时，若某条时空弧或由此导出的时间区域已被证明不可能出现在优于 incumbent 的内部列中，则对子节点这个更小的可行域仍然安全。相反，root dual profitable window 不会写入 node，也不会继承，因为它只是 root 当前 dual 下的临时 reduced-cost 筛选。

关于列成本，当前 `TWETColumn` 是按 job sequence 存储的全局列对象，不保存具体完成时间路径，也不保存“在哪个 node 的 window 下优化”的上下文。因此全局 Pool 中同一 sequence 的成本仍应保持真实 TWET objective 的最低已知口径；不能把某个 node 的 effective window 限制下得到的偏高成本作为全局列成本长期保存。即使 time-indexed compact window 对该子树是安全剪枝信息，它也更适合作为候选过滤和搜索范围控制；一旦某个 sequence 要进入全局 Pool/RMP，仍应使用真实 objective cost 或至少保证不会把同一 sequence 的较低真实成本挡在 Pool 外。

此前“同一 sequence 保留更低成本版本”的修正也服务于这个不变量。time-indexed graph、半域 join 或不同完成时间恢复同一 job sequence 时，可能先得到一个成本较高的版本，后续又得到较低版本；由于当前 Pool 以 `SequenceSignature` 去重，如果不允许较低成本版本更新 active/global pool，就会让 RMP 长期使用偏高的列成本，尤其影响 time-indexed 对照器和带半域限制的候选恢复。

### 41.93 2026-06-29 是否继承完整 time-indexed arc 可用集

代码里仍保留了 `Node.timeIndexedPricingOnlyForbiddenArc`，这是给 time-indexed graph pricing 实验线使用的逐时空弧禁用集合。ng-DSSR 主线的 `TimeIndexedScalarCompletionBound.applyArcFixing()` 当前没有把本次固定的数百万条 `(from,to,t)` 写入这个集合，而是在 helper 内部用本地 `BitSet` 记录、重算 forward/backward reachability 后，只把每个 job 的可达时间 hull 写成 compact window。这样做的原因是此前诊断中已经看到 40 任务节点上时空弧数量可达数百万，若逐条复制到子节点，会带来明显内存、复制和 `contains` 成本。

“保存仍可用的时空弧”理论上比保存被禁用弧可能更稀疏，尤其在 fixing 后剩余弧很少的节点上可能划算。但它需要新增一套相反语义的数据结构：默认是全图可用还是只允许白名单、如何和普通 branch/pricingOnly 禁弧叠加、如何在子节点增量删除、如何避免对每次查询多一次白名单判断。这适合后续单独作为实验优化，而不应混进当前 compact window 方案。当前主线更稳的方案仍是：ng-DSSR 只继承 job 级 `[minT,maxT]` 窗口；完整时空弧禁用/可用集只留给 time-indexed graph pricing 实验线。

### 41.94 2026-06-29 启发式 pricing 接入 node 继承硬时间窗

本轮把 `HeuristicPricingEngine` 的 profile 构造改为可读取当前 node 可用的窗口信息。启发式搜索内部现在使用的窗口为：基础 hard window、root/no-cut 下本轮 dual profitable window、以及由 time-indexed reduced-cost arc fixing 继承下来的 node compact window 三者的交集。root dual profitable window 只作为本次启发式搜索窗口使用，不写入 node，不继承到子树，也不进入全局列成本；最终返回列前仍会用真实目标重算成本。因此它可以用来减少 tabu move 中的无效函数拼接和候选搜索空间，但不会污染 `Pool` 中的 `TWETColumn.cost`。

返回列时仍然按原始 TWET objective 重新计算一次 sequence 成本，并用当前真实 dual 重新判定 reduced cost。这样做是为了保持全局 `Pool` 的不变量：`TWETColumn.cost` 必须是 sequence 的真实列成本，而不能带有某个 node 的局部 hard window、dual window 或 effective window 口径。理论上，time-indexed compact window 是对子树安全的硬剪枝信息，窗口外完工时间不应出现在优于 incumbent 的解中；但由于当前列池只按 `SequenceSignature` 管理列，不保存“该 sequence 在哪个窗口下优化”的上下文，保守做法仍是搜索阶段用窗口剪枝、入池阶段刷新真实成本。另一种“只使用非 dual window 的硬窗且不刷新成本”的做法暂不采用。

同一 sequence 多成本的问题仍按当前全局策略处理。time-indexed graph pricing 可能从不同完成时间恢复出同一 job sequence；当前本轮候选和全局 `Pool` 都按 `SequenceSignature` 合并，并保留已知成本更低的版本。启发式 pricing 本轮改动后也保持同一口径：本地候选进入返回池前已经刷新为真实成本，因此不会把窗口限制下的偏高成本写入全局池。
### 41.95 2026-06-29 中间迭代 time-indexed 窗口收缩与 SRI-aware arc fixing

本次把 time-indexed helper 的“中间迭代窗口收缩”改成一个显式实验开关，而不是默认每轮都做。开关 `timeIndexedCompletionBoundInRoundArcFixing` 默认为 false；打开后，ng-DSSR exact pricing 初始化时先在当前 time-indexed 图上按 0 reduced-cost cutoff 做一次本地时空弧 fixing，然后只在本轮内用 fixing 后的可达性进一步收缩 `effectiveJobHStart/End`。这个窗口不会写回 node，也不会继承到子节点；继承仍只发生在 node 闭合后的 `timeIndexedCompletionBoundArcFixing` 路径。这样做的目的只是测试“本轮先删弧再缩窗”能否减少当前 pricing 的后续扩展，避免把一个可能收益有限的操作混进默认流程。

active SRI cut 下的处理也补了明确口径。默认情况下，只要 LP 中存在 active subset-row pricing cut，time-indexed helper 不再自动使用 no-SRI 图；除非显式打开 `timeIndexedCompletionBoundAllowNoSriWithActiveCuts`。如果打开 `timeIndexedCompletionBoundSriAwareArcFixing`，则中间迭代窗口收缩和 node 闭合后的时空弧 fixing 都使用带 SRI residual state 的 time-indexed labeling：正向 label、反向 label 分别携带当前 cut 的 residual count/memory 状态；测试某条时空弧 `(i,j,t)` 时，用所有 forward state、该弧的 SRI 状态转移和所有 backward state 的最小拼接 reduced cost 判断是否可以删弧。节点闭合时使用 `incumbent - node LP bound` 作为 cutoff；中间迭代只用 0 cutoff。这样避免了 active SRI 下把 no-SRI fixing 当成完整 SRI-aware 证明的风险。

当前实现仍保持保守边界：SRI-aware helper 只在开关打开时使用，默认不影响主线；它复用 time-indexed 图的 `(job,time)` bucket，但 bucket 内可能保留多个 SRI residual state label，因此比 no-SRI 最短路更贵。该功能主要用于对照验证，不应默认开启到大规模算例。验证方式为 focused `javac`，覆盖 `TimeIndexedScalarCompletionBound`、ng-DSSR 接入口和相关 LP/Tree 类，编译通过；`git diff --check` 通过。

### 41.96 2026-06-30 active SRI 下 time-indexed helper 的当前默认口径
