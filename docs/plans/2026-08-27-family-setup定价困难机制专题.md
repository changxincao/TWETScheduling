...
| set01 | time-indexed+SRI | 第4轮cut后定价未闭合 | 300.070 | 54255.640（第3轮闭合） | 0.9644% | 1 | 28584 / 28554 | 414 |
...
| set02 | time-indexed+SRI | 第6轮cut后定价未闭合 | 300.062 | 41826.702（第5轮闭合） | 0.1034% | 1 | 33481 / 33444 | 597 |
...
| set03 | time-indexed+SRI | 第6轮cut后定价未闭合 | 300.119 | 39051.377（第5轮闭合） | 0.3639% | 1 | 30205 / 30146 | 619 |

三例ng-DSSR平均`50.306s`并全部在根节点证明最优；两组time-indexed平均都达到约300秒。无cut版本的根CG只需`1.69--1.97s`，但留下`4.05%--6.95%`的弱根gap，随后在大树中仍保留`2.55%--4.78%`gap。SRI版本在300秒内没有完成整个根cut loop，但已经完成了多次“加cut后重新定价闭合”。每次完整闭合都会得到一个新的安全下界，且有效cuts使这些界单调不弱于此前闭合界；因此应报告超时前最后一次完整闭合，而不是固定退回第一次无cut闭合。三例最后认证界依次为`54255.640/41826.702/39051.377`，对应gap仅`0.9644%/0.1034%/0.3639%`。尚不可使用的只是最后一批cuts加入后、当前尚未闭合的RMP目标。
...
### 32.26 probe现有控制流与最小修改边界

进一步对照当前实现后，第32.25节关于“没有必要时跳过probe”和“从已测候选中选最好点”的表述需要收紧。当前每个候选先重建该`Tmid`的half-domain和双向搜索状态，再各执行最多5000次forward/backward pop；最终选中候选的队列、dominance结构和labels会直接交给正式labeling续跑。因此首个候选若被接受，其10000次pop不是重复计算。adaptive-direct看起来省去probe，但正式labeling仍会从同一`Tmid`重新完成这些扩展。真正确定的冗余只有切换到另一候选后被整体丢弃的旧状态。

当前流程为：第一轮从default midpoint开始probe；后续轮先读取上一完整DSSR轮的forward/backward正式耗时。耗时比不超过2时沿用上一`Tmid`，超过2时按失衡程度从重侧存活label的split-time分位数产生adaptive seed。随后无论seed是否移动，都在新ng-memory下执行probe。每个候选各采样5000次pop；两侧都耗尽时直接接受，其他情形只用两段elapsed的比值判断是否不超过1.5。若不接受，则按实测重侧沿有效区间移动10%；方向反转后进入二分，直到区间缩到5%。候选数量没有上限，最终直接使用停止时的最后一个候选，只有该候选状态被保留。

单侧耗尽的严格处理应使用`acceptableRatio=1.5`，而不是只判断未耗尽侧是否超过已耗尽侧。设已耗尽侧完整耗时为`E`。未耗尽侧继续扩展，直到其队列也耗尽，或者累计耗时达到`1.5E`且队列仍未空。前者得到两侧完整时间，再按真实比值决定是否接受；后者已经证明该候选不满足1.5阈值，可以立即停止并确定移动方向。若累计耗时只达到`E`，只能证明未耗尽侧最终不轻于另一侧，尚不能证明候选不合格。该定向加深只发生在恰有一侧耗尽时，不改变两侧均未耗尽的等pop抽样，也不需要完整跑完重侧。

“上一轮平衡或adaptive位移小就跳过probe”暂不应作为第一项修改。DSSR更新后ng-memory已经变化，上一轮平衡不能保证下一轮仍平衡；同时首个候选状态可复用，跳过它没有明确的扩展节省。更稳妥的最小方案是始终在adaptive seed上测试一个当前memory候选：若双侧耗尽、可靠比较后满足1.5，或两侧均未耗尽的等pop样本满足现有阈值，就直接保留该状态进入正式labeling。只有当前候选明确不平衡时，才允许测试第二个候选。

候选数可以先限制为最多两个，但在当前架构下不能低成本实现“测完以后回到最好候选”。测试第二个`Tmid`会重建全部搜索状态并销毁第一个候选；若想重新选择第一个，只能保存整套可变队列/dominance图或重新运行第一个候选，两者都会抵消限流收益。因此最小实现应是：第一个候选只有在可靠判为不合格后才丢弃；沿实测方向移动较小一步，例如有效宽度5%，运行第二个候选后直接以第二个状态继续，不再walk或bracket。较小步长用于限制反向过冲，而不是禁止反向修正。
...
### 32.29 单侧耗尽频率、250-pop续探与反转细分

单侧耗尽续探的检查批量按最新决定改为250 pop，不使用512。追加扩展不重建候选：每批开始和结束各读取一次`nanoTime`，把本批耗时累加到初始side elapsed，随后检查队列、`R E`阈值和全局时限。按250检查时，每追加5000 pop最多20次时间检查，时钟开销相对PWLF扩展和dominance操作可以忽略，同时最多只会越过阈值249次pop。首轮仍用`R=1.5`，同一次exact的DSSR后续轮用`R=4.0`。

现有n40 `probe+minimum`完整family/random六组日志只对每轮保存候选数和总pop，并未保存每轮所有候选的`ex`明细，因此可以统计“某轮至少有一个候选提前耗尽”的频率，不能还原全部候选的精确个数。family三例分别为`49/62=79.0%`、`180/264=68.2%`和`76/419=18.1%`；合计`305/745=40.9%`。37次exact首轮全部出现提前耗尽；DSSR后续轮为`268/708=37.9%`。这些family轮最终均非rank0，因此总pop低于`候选数×10000`意味着至少一个候选恰有一侧耗尽，而不是双侧同时完成。

random三例的`16/22/33`个probe轮最终全部以rank0双侧耗尽结束，没有family式“单侧不完整却按elapsed比接受”的最终状态。每次exact最后一轮保留的候选明细中，family有21个、random有13个单侧耗尽候选；其当前样本比最小分别为4.655和5.809，均已超过拟定后续阈值4，所以这些样本不需要追加就会直接判为不合格。历史TIME诊断中确实出现过`5000未耗尽/1186已耗尽、elapsed比1.173`的反例，说明模糊区并非理论空集，但在当前六组完整根日志的最终轮样本中不常见。结论是：单侧情形在family轮级很常见，但真正需要250-pop续探的是其中当前elapsed尚未达到本轮`R E`的子集；必须在新代码中增加first/later分开的精确计数后才能量化实际追加成本。

现有方向反转不读取2倍阈值。流程是候选A超过当前接受比1.5后移动10%；候选B也超过1.5且实测重侧与A相反，才认定反转并进入bracket。`bidirectionalMidpointProbeDssrImbalanceThreshold=2.0`只控制上一完整轮何时使用分位数生成adaptive seed，与probe候选反转无关。历史family后续轮中有`223/708=31.5%`进入bracket，set01/set02分别约69.0%/66.7%，set03仅约3.8%；random后续轮没有进入bracket。这说明反转是family当前probe频繁增加候选的主要入口，也不宜完全删除，因为它确实表示固定步长跨过了一个重侧切换区间。
...
### 32.31 单侧续探与后续DSSR宽松probe实测

本轮所说的“固定dual/ng-set快照复核”只是诊断方法，不进入求解流程：固定同一组master dual、branch/cut上下文和ng-memory，只改变候选`Tmid`，比较各候选续跑后的完整forward、backward和join工作量。这样可以把中点影响与后续加列、dual变化分开。当前没有做全面`Tmid`网格；已有逐轮完整耗时和根节点配对结果已经给出一致方向，仅在后续结果冲突时再抽少量快照复核。

实现首先修正单侧耗尽比较。首轮和DSSR后续轮仍各自使用`R=1.5`和`R=4.0`：初始各侧最多5000 pop后，若恰有一侧耗尽且另一侧尚未达到`R`倍完整侧耗时，就沿用同一候选状态每250 pop继续未耗尽侧；追加侧若先耗尽，直接复用双侧完整状态；若队列未空但累计耗时达到阈值，则可靠确定重侧并移动中点。候选摘要只保存结局、追加pop和追加毫秒，`solve()`末尾按首轮/后续轮聚合，不增加七个平铺计数器。该逻辑只修正浅探判定和复用已有状态，不改变label扩展集合、dominance、join、DSSR更新或certificate。

随后比较三种策略。`S`为只加单侧修正、后续轮仍沿用`1.5/10%`及原walk/bracket；`R`在此基础上把后续轮改为`4.0/5%`，其余walk/bracket不变；`B`再限制为A、B及反转时一个中点C。所有运行固定`TIME`队列、`minimumNewPairsSegment`、candidate1000、warm-start关闭、同一seed、CPLEX单线程和`maxNodes=1`。

| n30 family | S总时/exact | R总时/exact | B总时/exact |
| --- | ---: | ---: | ---: |
| set01 | 29.980 / 23.520 | 32.620 / 26.590 | 32.820 / 26.500 |
| set02复跑 | 44.304 / 38.416 | 36.043 / 30.230 | 34.439 / 28.447 |
| set03 | 48.162 / 43.066 | 40.123 / 35.262 | 37.302 / 32.280 |
| 合计 | 122.446 / 105.002 | 108.786 / 92.082 | 104.561 / 87.227 |

`R`相对`S`的总时和exact分别下降约`11.2%/12.3%`。三个`R`运行共有198个DSSR后续完整轮，其中99轮完整F/B比不超过1.5、165轮不超过2、全部不超过4，仅23轮测试多个中点；`S`的197轮中有31轮最终仍超过4，59轮测试多个中点，32轮进入bracket。这说明主要收益不是5%步长本身，而是上一完整轮已经给出adaptive seed后，浅样本不再因1.5阈值过度覆盖它。set02和set03的`R`后续轮共发生152次单侧候选，其中79次追加到双侧完成、73次达到比例预算，约20.3万次追加pop只耗`1.881s`，占两例probe时间`52.106s`的约3.6%；单侧修正频繁但开销有限。

三个matched n30 random实例中三种策略均未出现单侧续探，exact总计只有秒级。`S/R`总时间合计为`22.034/22.100s`，变化约`+0.3%`；这些运行因`maxNodes=1`均为`NODE_LIMIT`根节点诊断，不能解释为完整求解时间，但足以说明新参数没有给当前random根定价增加可见成本。

在困难`n040-set02 family,m=2`上，又以相同输入、seed、单线程和1200秒预算并发配对复跑`S/R`。两者均完成根节点并得到相同的`LB=UB=82351`：

| 策略 | 根总时/s | exact/s | exact调用 | DSSR轮 | probe/s | 后续完整轮比值大于4 | 多候选后续轮 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| S | 812.981 | 796.761 | 11 | 254 | 253.242 | 207/243 | 207 |
| R | 748.260 | 731.275 | 16 | 384 | 188.718 | 5/368 | 6 |

`R`使根总时、exact和probe分别下降`7.96%/8.22%/25.48%`。旧规则在第一次exact中多次把约`497--545`的adaptive seed推到`689--755`，完整F/B比最高约16.6；`R`通常直接保留adaptive seed，完整比多在`1.0--1.2`附近。与此同时`R`早期每次返回的elementary列较少，exact调用和DSSR总轮数反而增加，抵消了大部分单轮收益，所以最终不是成倍加速。该结果也说明评价`Tmid`必须同时看完整F+B、返回列和CG调用轨迹，不能只看浅probe ratio。

`B`在n30单次合计时间较低，但set02重复运行的相对排序波动明显，而且set02/set03仍分别留下10/8个完整比大于4的轮次：达到候选预算后只能使用已销毁A之后的B或C，并不能证明该点优于A。它没有n40证据，也引入另一套停止分支，因此不进入正式主线，实验开关和代码已删除。正式profile升级为`2026-08-30-v4`：首轮继续`R=1.5/step=10%/原walk-bracket`，后续DSSR轮使用`R=4/step=5%/原walk-bracket`，两者都启用250-pop单侧续探；`minimum/1000`、warm-start关闭、adaptive阈值2和ng-memory更新均不变。默认profile smoke在n30 set01根节点得到`LB=UB=54784`，验证有效配置和两类单侧续探路径均已执行。

### 32.32 与adaptive-direct基线补充比较及结论修正

第32.31节只比较了单侧修正后的严格probe `S`、宽松probe `R`和候选预算`B`，不能据此把`R=4`称为当前全局最佳。必须同时纳入第32.14和32.21节的adaptive-direct：同一次exact第一轮仍probe，后续DSSR轮保留上一完整轮的adaptive反馈和half-domain重建，但不再执行浅层probe。

`B`的A/B/C语义也需澄清。A是adaptive seed；A的浅层比超过4后移动5%得到B；B若反向则测试A/B中点C，随后因预算停止。由于切换中点会销毁前一候选状态，预算结束后只能沿用最后的B或C，除非重新计算A。所谓“完整F/B比仍大于4”不是正确性问题，也不能单独证明B/C更慢：exact labeling仍会完整结束并给出相同certificate；它只说明停止是因为候选预算，而不是已经证明最后一点平衡或总工作量更小。n30合计中B反而最快，因此当前证据只能判为不充分、未验证到n40，不能判为无效。

当前三方案的有效比较为：

| 口径 | 严格后续probe | 后续`R=4/step=5%` | adaptive-direct |
| --- | ---: | ---: | ---: |
| n30 family三例平均总时/s | 40.815 | 36.262 | **29.473** |
| n30 family三例平均exact/s | 35.001 | 30.694 | **25.037** |
| n40 set02 family总时/s | 812.981 | 748.260 | **419.701** |
| n40 set02 family exact/s | 796.761 | 731.275 | **405.388** |
| n40 set02 family exact调用 | 11 | 16 | **8** |

n40 set02四组使用相同实例、seed fingerprint、`TIME`队列、minimum/1000、warm-start关闭、单线程和根节点口径，三组都得到`LB=UB=82351`。严格probe的新旧复跑为`812.981/807.781s`，说明两批墙钟量级一致；因此`R=4`虽比严格probe快约8%，仍明显慢于adaptive-direct。n30上direct在set01/set02更快，set03略慢，三例平均仍优于`R=4`。历史n40三例family中direct也全部优于严格probe，平均`327.305s`对`525.994s`；random则为`23.474s`对`21.729s`，只有约1.7秒差异且exact本来仅2--7秒，不能形成反对direct的强证据。

因此阈值4的正确定位是：若决定保留DSSR后续浅probe，它是目前比1.5更合理的保护阈值，用于避免浅样本过度覆盖完整轮adaptive seed；它不是理论阈值，也没有经过3/4/5等网格证明唯一最优。与adaptive-direct相比，现有family证据仍明确支持完全取消后续浅probe。`2026-08-30-v4`只能称为“保留后续probe时的改进配置”，不能称为当前最快统一配置；在决定正式默认前，应优先做当前代码下的matched `R=4`与adaptive-direct复核，而不是继续细调4附近的数值。

### 32.33 暂保留阈值4及probe相对direct变慢的机制诊断

按最新决定，正式profile暂时继续使用后续`R=4/step=5%`，不切换adaptive-direct，也不恢复A/B/C候选预算。A/B/C只是一条已测试的中点搜索预算：A为完整轮反馈产生的adaptive seed；A的浅层比超过4后移动5%得到B；若B的重侧反转，再测A/B中点C，随后停止。它已在n30 family三例、set02重复样本和n30 random三例上测试，未测试n40；该策略不改变DSSR或exact正确性，目前只判为证据不足。

进一步拆解相同n40 set02 family、相同seed和minimum/1000下的adaptive-direct与`R=4`日志。direct用8次exact、187轮DSSR返回23条exact列，exact累计`405.388s`；`R=4`用16次exact、384轮DSSR返回37条exact列，累计`731.275s`。两者单次exact平均反而约为`50.67/45.70s`，因此总时间差的直接来源不是`R=4`使每次labeling普遍更慢，而是它所在的列生成轨迹需要两倍exact闭合次数。最终exact列集合中有18条序列共同出现；direct另有5条，`R=4`另有19条。更多列没有缩短闭合，说明列的加入批次、组合及其引起的RMP dual轨迹比列数本身更重要，不能直接称新增列质量差，但可以确认轨迹发生了实质分叉。

分叉有三层原因。第一，probe优化的是浅层两侧耗时比，不是完整`F+B+join`总工作量；一个不平衡的`100+10`仍可能优于平衡的`70+70`。第二，family memory增长后的后代数量和dominance退化是深层非线性的，前5000 pop的比例不稳定预测完整长尾。第三，adaptive本身也使用墙钟耗时。两个set02运行第一轮都选`Tmid=643.05`，但完整比因计时/执行路径差异为`3.874/4.196`，下一轮adaptive seed随即成为`495/485`；`R=4`随后只是接受485，并没有再次移动。十个时间单位的差异已足以在大量近似同族替代列中改变返回列，随后RMP dual、启发式列和下一次exact调用不断放大该差异。

当前`R=4`只有6个n40后续轮测试多个候选，因此“切换中点后丢弃旧状态”的纯计算浪费已不是主要差距。probe统计中的`188.718s`也不能全部视为冗余，因为首候选的扩展状态会由正式labeling复用。更可信的主因是：少量早期中点/返回列差异改变CG轨迹，最终多出8次昂贵exact调用；浅probe目标与完整总工作量不一致则是这种差异可能朝坏方向发展的底层原因。历史direct运行早于本次单侧续探修正，尚不是当前代码的完全隔离A/B，因此本节只解释已有差异，不据此推翻暂用阈值4的决定。

### 32.34 当前代码复跑与同快照反事实诊断

本轮先重新确认正式主线边界。当前生效的算法修改只有两类：第一，首轮和后续轮的probe候选若恰有一侧提前耗尽，未耗尽侧沿同一状态每250 pop继续，直到也耗尽或累计耗时达到本轮阈值；第二，首轮仍用`R=1.5/step=10%`，后续DSSR轮改用`R=4/step=5%`。原有walk、方向反转和bracket流程仍在，候选数没有上限；没有启用A/B/C预算、跳过probe、adaptive位移上限、allSegments或same-node warm-start，DSSR仍为`minimumNewPairsSegment + candidate1000`。除全局时限已到时立即退出初始化的保护和诊断字段外，正常无时限路径中的labeling、dominance、join、witness更新、返回列和certificate流程均未改。

为避免继续使用早于单侧修正的历史direct结果，当前代码在3个`n=30,m=2` family实例上各做3次direct与R4单线程根节点复跑，交替运行顺序。direct指首轮仍probe、后续轮直接使用完整轮反馈产生的adaptive seed；R4指后续轮继续执行`R=4/step=5%`浅探。全部18条run均完整闭合并得到各实例相同的`LB=UB`。

| 实例 | direct总时均值/范围 | R4总时均值/范围 | direct/R4 exact均值 | R4实际位移/后续轮 |
| --- | ---: | ---: | ---: | ---: |
| set01 | 20.48 / 16.38--26.55 | 19.30 / 15.52--25.02 | 16.23 / 15.24 | 0 / 198 |
| set02 | 23.19 / 22.00--24.82 | 18.32 / 13.26--28.30 | 18.77 / 13.99 | 17 / 108 |
| set03 | 27.11 / 24.91--30.27 | 34.17 / 24.00--42.88 | 23.48 / 30.11 | 28 / 245 |
| 9条合计均值 | 23.59 | 23.93 | 19.49 / 19.78 | 45 / 551 |

R4在551个后续DSSR轮中只改变45次adaptive seed，占`8.2%`，所以“大多数轮应与direct接近”的判断成立。9条样本合计R4总时和exact只比direct高约`1.4%/1.5%`，总体近似持平，但实例内和重复间离散很大：set02的R4平均更快，set03则更慢。同一策略的exact调用数、DSSR轮数和最终列数也会跨重复变化，说明单次根时间不能可靠归因于probe开关。

关键原因是adaptive反馈本身使用毫秒耗时，而不是确定性的label工作量。set01第一次exact的第一轮在direct和R4中都选`Tmid=540.8`，labels完全相同，但两个独立进程观测到的完整F/B比为`8.132/5.435`，下一轮adaptive seed立即变成`314/343`。JIT、GC、CPU调度和缓存造成的计时差异随后改变half-domain、返回列批次、RMP dual和下一次exact，路径不断放大。当前n40-set02复跑也印证了这一点：当前direct为`731.152s / exact 707.246s / 12次exact / 282轮DSSR`，当前R4为`748.259s / exact 731.275s / 16次exact / 384轮DSSR`，总时仅差`2.3%`，不再复现历史`419.701s`对`748.260s`的极端差距。两者仍得到`LB=UB=82351`。历史direct早于单侧修正，不能再作为当前代码的严格配对基线。

为了直接回答“probe移动究竟选对还是选错”，临时加入只由JVM系统属性触发的同快照诊断。每次只在probe确实移动时固定同一master dual、branch/cut上下文和ng-memory，依次完整展开`selected A1 -> direct B -> selected A2`；每次候选前清空影子列和witness，A2状态才交回原join。n30的19个变化快照中，A1/A2的pop、constructed和kept计数19/19完全一致，且两侧队列全部耗尽，说明比较的是同一个确定性labeling问题；A1/A2双跑只用于控制墙钟抖动。该临时代码在取得数据后已删除，没有进入正式主线。

同快照结果需要分成“最终点质量”和“找到该点的总代价”两层。n30的19次位移中，probe最终选点19/19都优于direct seed：完整F+B时间平均只有direct的`38.67%`，constructed labels为`42.28%`，kept labels为`63.82%`，完整pop为`57.0%`。因此当前证据不支持“probe把direct中点选歪了”。但是加入所有被丢弃候选的浅探耗时后，只有1次净胜、1次近似、17次净负，平均/中位总代价为direct完整展开的`1.416/1.405`倍。更强的确定性证据是：19次中，所有probe候选的浅探pop总数在进入最终selected状态的剩余扩展之前就已经全部超过direct完整pop，平均为其`3.033`倍。

净损失随候选数明显上升。n30中测试2个候选的1次净比为`1.14`；3个候选的4次平均`1.10`；4个候选的2次平均`1.15`；5个候选的8次平均`1.49`；6个候选为`1.69`；9个候选的3次平均`1.82`。其机制不是最终点差，而是direct seed虽然失衡，却只需承担一次完整长尾，下一DSSR轮会用完整反馈自行纠正；R4若沿5%步长连续走5--9个点，会把前面候选的队列、dominance和labels全部丢弃，寻找成本超过直接忍受一轮过冲。

n40则给出了另一边界。在set02第8次exact、第18轮的同快照中，direct seed为`886`，R4只移动一个5%步长到`677.65`。direct完整F+B为`4435.746ms`，selected A1/A2均值为`2337.639ms`，丢弃首候选只耗`248.179ms`；合计净比为`0.58295`，selected constructed比例为`0.67412`。这里direct点存在昂贵深层长尾，而一个5000-pop浅前缀相对完整搜索很便宜，一步纠偏能够回本。该诊断run只为取得目标快照，在A/B/A完整结束并落盘后人为中止，不能作为根节点性能样本。

这也解释了当前n40总量。R4平均每个DSSR轮exact耗时约`1.904s`，direct约`2.508s`，R4单轮低约`24.1%`；但R4所在CG路径有384轮，direct只有282轮，多`36.2%`，最终exact总时反而高`3.4%`。少量中点变化会改变有限返回列和witness顺序，继而改变dual与后续exact调用数；局部更好的Tmid并不保证整棵CG轨迹更短。

当前结论为：单侧耗尽修正应保留，它修正了不可比的完整耗时与浅前缀，9条n30 R4 run中428次后续单侧事件的追加扩展合计约`3.715s`，不是主要瓶颈。`R=4`作为“是否覆盖adaptive seed”的宽松阈值也没有显示系统性错误。真正未解决的是原有无上限walk/bracket：一两候选在存在深层长尾时可净获益，连续多候选则大多亏损。现阶段不据此恢复已撤回的A/B/C实现，也不改正式配置；若继续修改，应单独验证“后续轮最多保留首候选加一次5%修正”的两候选版本，并同时观察同快照净工作量和完整CG调用轨迹。仅凭根总时选择direct或R4会把计时驱动的路径波动误当成稳定算法收益。

### 32.35 历史direct异常快、5% walk长尾与adaptive限幅复核

历史adaptive-direct的`419.701s`不能解释为当前单侧修正以前的代码必然更快。本轮在当前机器上临时跳过单侧续探，其他配置、实例、seed fingerprint、`TIME`队列、minimum/1000、warm关闭和单线程口径均与历史direct一致；运行完成后立即撤销诊断开关并恢复正式字节码。三组结果如下。

| 运行 | 根总时/s | exact/s | exact调用 | DSSR轮 | exact列 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 历史旧单侧语义direct | 419.701 | 405.388 | 8 | 187 | 23 |
| 当前机器旧单侧语义复现 | 659.876 | 630.544 | 7 | 173 | 31 |
| 当前正式单侧修正direct | 731.152 | 707.246 | 12 | 282 | 39 |

当前机器的旧语义复现比历史少一次exact、少14轮DSSR，算法工作量没有增加，但exact仍慢`55.5%`、根总时慢`57.2%`。更直接的固定工作量证据来自第一次exact第一轮：两次均取`Tmid=643.05`，保留label数完全相同，均为forward/backward `21870/4661`；历史side elapsed为`744.0/192.1ms`，当前复现为`1560.5/470.9ms`，同一确定性label状态的F+B耗时约为历史的`2.17`倍。Java版本均为22.0.1、可用处理器均为16，配置差异只有不生效的后续probe字段和时限。因此历史`419.701s`首先是更有利的机器/进程运行状态，不是旧代码稳定减少了算法工作。

机器差异又会被当前算法放大，因为adaptive seed直接读取`System.nanoTime()`得到的完整F/B耗时比。同一个`21870/4661`状态在历史与当前的耗时比为`3.874/3.314`，下一轮分位数seed随即成为`495/518`；half-domain、返回列批次、RMP dual和后续exact轨迹由此分叉。当前旧语义复现的前两次exact首轮取643.05，第三、四次又因墙钟变化取434.7；历史前七次则都取643.05。由此，历史419秒是“当时单核吞吐更快、墙钟反馈恰好形成较有利中点和列轨迹”的单次低尾样本。单侧修正在当前样本上又使660秒变为731秒，说明代码路径也有影响，但这约71秒不能当成可重复的纯代码成本，因为两次运行的墙钟反馈已经不是确定性输入。

后续DSSR轮的5%步长没有候选上限。有效时间域被分成20段，从任意seed沿同一方向最多可测试21个walk候选；若相邻两点方向反转，当前2.5% bracket tolerance还可再测试一个中点，理论最多22个候选。9条当前n30 family R4运行的实际分布为：551个后续轮中506轮只测试1个候选，占`91.83%`；45轮发生移动。发生移动时平均测试`4.73`个候选，最大为10个，即连续移动9步、跨越45%有效时间域。全部后续轮共测试719个候选，若硬限制为最多两个，候选数会降到596，少123次，等于全部候选的`17.1%`和当前168个丢弃候选的`73.2%`。n40 set02则只有`6/368`轮移动，且都只走一步；这与同快照中一步纠偏净耗时为direct的58.3%一致。故问题不是5%本身过大或每轮都走，而是n30少数轮的无上限同方向长尾。

本轮还隔离测试了“adaptive seed单轮最多移动有效宽度5%”。它确实把当前direct的最大完整F/B比从`138.71`压到`8.70`，消除了`434.7 -> 889`以及旧语义复现中`451 -> 1312`一类局部过冲；但完整根结果恶化为`841.870s / exact 812.869s / 14次exact / 329轮DSSR / 33条exact列`，相比当前direct的731.152秒慢`15.1%`。局部两侧更平衡再次没有转化为更短的CG轨迹，诊断代码已撤销，不能采用5% adaptive硬限幅。

当前可执行结论因此收敛为三点。第一，正式单侧修正、R4和5% probe步长暂时保持，不以历史419秒回滚代码，也不启用adaptive 5%限幅。第二，若继续减少probe冗余，应只针对“发生移动后的长尾”测试候选预算，而不是改变所有轮次；最多两个候选理论上可删除73.2%的已观测丢弃状态，但此前B版本在n30重复样本中路径波动明显，必须在当前代码下补n40并看完整CG调用数，不能仅凭候选节省写入主线。第三，比参数微调更根本的问题是墙钟反馈不稳定。下一项诊断应同时记录当前线程CPU时间与wall time，在固定dual/ng-memory快照上比较两者的方向、比例和重复方差；若thread CPU time显著更稳定，再单独A/B用它产生adaptive/probe决策，同时继续以wall time评价总性能。不能直接改用label数，因为PWLF段数、dominance和join使单个label的成本并不相同。

### 32.36 十候选长尾的具体来源与adaptive触发阈值

9条n30 family R4运行中唯一一次10候选事件发生在`set03-rep3`第6次exact的DSSR第6轮。前5轮始终使用`Tmid=296.7`；第5轮完整forward/backward耗时为`46.4/147.3ms`，比值`3.172`，backward较重。当前`bidirectionalMidpointProbeDssrImbalanceThreshold=2.0`因此触发adaptive，调整比例为

\[
0.5\left(1-\frac{2}{3.172}\right)=0.1847.
\]

该比例不是“从旧Tmid移动18.47%”，而是取上一轮存活backward label的split-time全局`18.47%`分位数；本例该分位数为`1581`。因此adaptive从`296.7`一次跳到`1581`，位移`1284.3`，约为有效时间域`2847`的`45.1%`。新一轮ng-memory下，浅探又连续判定该seed一侧过重，于是按`142.35=5%×2847`依次测试`1581、1438.65、1296.30、1153.95、1011.60、869.25、726.90、584.55、442.20、299.85`。前9个候选没有方向反转，故不会进入bracket；最后`299.85`双侧队列均耗尽，以rank-0停止。该轮最终完整耗时为`43.6/92.8ms`、比值`2.128`。所以“跨45%”不是5%步长单次很差，而是moderate失衡触发了一次全局分位数大跳，随后固定步长又花9个候选走回旧Tmid附近。

这里暴露的是两个阈值不一致。上一完整轮只要比值超过`2`就生成adaptive seed，但后续probe把不超过`4`视为可接受。本例的`3.172`按后者本来不需要主动调整。对9条n30日志的全部adaptive轮统计，68轮中61轮由`2<previousRatio<4`触发，只有7轮真正超过4；29次adaptive后发生多候选移动的事件中，27次来自`2--4`区间，共测试126个候选、丢弃99个状态。真正超过4的多候选事件只有2次，共测试11个候选、丢弃9个状态。前者已占全部168个后续丢弃候选的`58.9%`，并包含9个和10个候选的全部极端长尾。因此当前优先问题不是adaptive在所有场景都失效，而是`2--4`这一已被R4定义为可接受的区域仍被更激进的阈值2提前移动。

不采用统一5% adaptive硬限幅时，下一项最小A/B应把`bidirectionalMidpointProbeDssrImbalanceThreshold`从2改为4，与`bidirectionalMidpointProbeDssrEarlyStopRatio=4`保持一致。这样上一完整轮比值不超过4时直接以旧Tmid为seed，但仍会在更新后的ng-memory上执行当前probe校验；只有完整比值超过4时才生成分位数seed。现有字段同时进入调整公式，阈值改为4还会对刚超过4的情形自动减弱调整：例如比值4.61时，调整比例由使用阈值2的`28.3%`降为`6.6%`，而极端失衡时仍可逐渐接近50%，不会像硬5%位移上限那样阻断所有大调整。本例若使用阈值4，seed会保留在296.7；当前memory下最终rank-0点为299.85，二者仅差有效域的0.11%，因此很可能只需一个候选，但该反事实仍需正式A/B确认。

若阈值对齐后，`previousRatio>4`的少数轮仍出现长距离回撤，再考虑改变回撤方式，而不是恢复硬限幅：adaptive候选A被当前memory判为反向过冲时，下一候选直接测试上一完整轮Tmid P，而不是从A按5%逐格走回；A和P都在当前memory下完成probe后，若压力方向相反，只增加一个中点候选并停止。这样保留真正严重失衡时的大跨度adaptive能力，同时把本例式回撤从最多十个候选压到两至三个。该方案比阈值对齐侵入更大，当前只作为第二阶段候选，不进入正式主线。正式配置和求解代码本轮均未修改。

### 32.37 adaptive触发阈值4的完整A/B

本轮只把正式ng-DSSR配置中的`bidirectionalMidpointProbeDssrImbalanceThreshold`从2改为4，使上一完整轮触发adaptive调整的阈值与后续DSSR浅探的接受阈值`bidirectionalMidpointProbeDssrEarlyStopRatio=4`一致。首轮probe仍使用1.5，后续probe仍使用5%步长、原有方向反转和bracket逻辑，也仍允许多次移动；单侧耗尽续探、候选上限和adaptive分位数算法均未修改。正式profile版本更新为`2026-08-30-v5`。

为避免墙钟反馈被并发负载污染，n30实验均以CPLEX单线程串行运行。三个family算例各重复3次，两个阈值的9条运行都完整闭合根节点，并在同一算例上得到相同最终界。均值如下：

| 算例 | 阈值2总时间/s | 阈值4总时间/s | 阈值2 exact/s | 阈值4 exact/s | exact调用数 2/4 | DSSR轮数 2/4 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| n30-set01-family | 19.306 | 16.763 | 15.238 | 13.201 | 11.00 / 9.33 | 77.00 / 67.00 |
| n30-set02-family | 18.318 | 35.157 | 13.986 | 30.035 | 5.33 / 11.33 | 41.33 / 84.33 |
| n30-set03-family | 34.171 | 18.635 | 30.107 | 14.882 | 10.33 / 7.00 | 92.00 / 61.00 |
| 三组总均值 | 23.932 | 23.519 | 19.777 | 19.373 | 8.89 / 9.22 | 70.11 / 70.78 |

总均值只改善`1.7%`，exact只改善`2.0%`，属于基本持平；单算例轨迹却分别约为`-13% / +92% / -45%`，说明返回列和后续CG轨迹的波动远大于总均值差异，不能把阈值4表述成稳定加速。它更确定的作用是消除参数自相矛盾造成的probe浪费：adaptive轮数从104降到37，后续移动轮数从45降到37，候选总数从719降到601，被丢弃候选从168降到47，单轮最多候选从10降到3。阈值4下完整forward/backward最大比值为5.082，共14轮超过4；没有出现几十倍或上百倍失衡，但确实允许少量约4至5倍的完整工作量差异。

n40-set02-family使用1200秒预算完整闭合根节点，结果如下：

| 策略 | 总时间/s | exact/s | exact调用 | DSSR轮数 | adaptive轮 | 移动轮 | 丢弃候选 | 最大完整F/B比 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 阈值2 | 748.260 | 731.275 | 16 | 384 | 26 | 6 | 6 | 7.657 |
| 阈值4 | 741.248 | 721.894 | 13 | 303 | 19 | 1 | 1 | 4.499 |
| 后续浅探关闭的direct参考 | 731.155 | 707.246 | 12 | 282 | - | - | - | - |

阈值4相对阈值2的总时间只下降`0.94%`，exact下降`1.28%`，同样接近持平。它把exact调用减少`18.8%`、DSSR轮数减少`21.1%`，但每DSSR轮平均exact耗时从`1.904s`升到`2.382s`，增加`25.1%`。这正好验证了“4:1若真失衡，正式labeling会更重”的担心；本例没有整体恶化，是因为调用数和DSSR轮数的减少抵消了单轮增幅。阈值4仍比当前direct参考慢约`1.4%`，因此不能据此证明后续浅探优于直接使用adaptive Tmid。

匹配的三个n30 random算例没有发生多轮DSSR adaptive，阈值代码实际未触发，因而其运行时间差只能视为机器波动，不能作为阈值4有效的证据。另有一个实验元数据细节：主要A/B运行前只重新编译了profile类，`FormalExperimentRunner`中的静态常量内联仍写出v4；有效配置日志已经明确显示阈值4，所以数值实验有效。随后重编译runner并做smoke，已确认结果元数据写为`2026-08-30-v5`且三个有效参数为`dssrImbalanceThreshold=4.0`、`dssrAcceptableRatio=4.0`、`dssrStepFraction=0.05`。

当前决定是保留阈值4作为正式v5配置，因为它修复了`2--4`区间仍触发adaptive、随后又被probe接受的逻辑不一致，并显著减少无效候选；同时把性能结论限定为“当前样本总体持平”，不宣称稳定变快。后续多次移动流程本轮完全不动。若继续优化，应单独处理`previousRatio>4`后发生的长距离回撤，不能把本轮结果解释成4倍失衡本身没有代价。

### 32.38 On-the-exact静态点与同快照工作曲线复核

本轮按要求只做诊断，不修改正式中点策略。On-the-exact使用的静态点可写为

\[
t^{\mathrm{WA}}=\frac{1}{2|J|}\sum_{j\in J}
\left(t_{j,\min}+t_{j,\max}\right),
\]

即当前有效任务时间窗左右端点的平均值；当前代码中的实验策略名为`windowAverage`。正式profile仍使用`default`，即当前有效时间域中点。本轮选取`n030-set02-family-base-zero-m2`做根节点诊断，其默认点为`911.5`，`windowAverage`为`936.067`。

历史上确实做过一次40-2 root-only对照：default与windowAverage总时间为`19.351/19.078s`，exact为`4.710/4.656s`，最终界均为22490，当时只看到约1%的差异。但重新检查发现，首轮probe路径硬编码以`computeDefaultMidpoint()`为reference；日志虽然写出`midpointStrategy=windowAverage`，实际首轮仍从default点开始。因此该历史结果只能说明两条当时实际接近的路径没有明显差异，不能视为对On-the-exact静态点的有效验证。现已把首轮probe入口改为使用已经计算出的`tMid`和`midpointStrategyUsed`；显式选择`windowAverage`时实验策略才真正生效，而正式`default`下传入值与原先完全相同。

为先隔离静态点本身，关闭midpoint probe，其他根节点配置保持一致，各重复两次。全部运行均完整闭合并得到`LB=UB=37948`、`valid=true`。

| 静态点 | 总时间两次/s | 总时间均值/s | exact两次/s | exact均值/s | exact调用 |
| --- | ---: | ---: | ---: | ---: | ---: |
| default 911.5 | 21.174 / 26.187 | 23.681 | 17.489 / 22.564 | 20.027 | 7 / 7 |
| windowAverage 936.067 | 20.613 / 20.443 | 20.528 | 17.039 / 16.826 | 16.933 | 7 / 7 |

在这一算例的两次重复中，windowAverage的平均总时间和exact时间分别低`13.3%`和`15.4%`。它是正信号，但只有一个实例、两次重复，且两个静态点会返回不同的有限列批次，因此还不能据此切换默认配置。

“工作曲线”不能由某个固定`Tmid`的一次完整轮直接恢复。该轮只计算`Tmid`左侧的forward状态和右侧的backward状态；若把分割点向右移动，缺少新增区间的forward工作，向左移动则缺少新增区间的backward工作。要得到严格同一dual/ng-memory下的曲线，仍需在同一快照上对多个候选点分别完整运行。主线已有仅由JVM属性触发、默认关闭且运行后恢复状态的full-midpoint diagnostic，本轮直接使用该诊断，没有加入新的生产策略。

第一次exact的同一dual/ng-memory快照上测试`750,825,875,911.5,936.067,975,1025,1100`。为识别JIT、缓存和扫描顺序影响，又按正序与逆序各执行一次；表中为每个候选完整forward+backward耗时，单位毫秒。

| Tmid | 正序F+B/ms | 逆序F+B/ms |
| ---: | ---: | ---: |
| 750 | 132.704 | 133.595 |
| 825 | 114.134 | 136.225 |
| 875 | 81.590 | 122.764 |
| default 911.5 | 93.335 | 112.928 |
| windowAverage 936.067 | 83.082 | 95.912 |
| 975 | 96.129 | 103.755 |
| 1025 | 115.116 | 98.564 |
| 1100 | 116.313 | 123.699 |

绝对耗时存在明显顺序波动，不能用单次扫描挑出`875`一类偶然最小点；但windowAverage相对default在两个顺序中都更低，分别减少`11.0%`和`15.1%`，说明它至少落在该快照较稳定的低工作量区域。曲线也再次表明“总工作量最低”不等于“forward/backward耗时相等”：在`936.067`处forward仍远重于backward，单纯追求1:1并不是正确目标。

最后恢复正式probe做两次配对。default总时间为`21.443/13.644s`、exact为`15.274/9.933s`，分别经历8/7次exact；windowAverage总时间为`13.369/13.985s`、exact为`9.183/9.155s`，均为7次exact。windowAverage两次较稳定，而default一次进入了不同的列生成轨迹。日志确认首轮reference分别为`911.5(default)`和`936.067(windowAverage)`；从第二轮开始，两者都会根据完整轮反馈移动到约`480--520`，所以静态点只直接影响每次exact的第一轮及由此返回的首批列。由于probe和adaptive都读取墙钟耗时，小波动会改变离散中点、返回列、dual及后续exact调用数，不能把这四次运行的均值差全部归因于静态公式。

当前结论为：On-the-exact的windowAverage在所测family算例上比default静态点更有希望，同快照曲线给出了比根总时间更直接的支持；但证据尚不足以更换默认。完整工作曲线适合作为离线oracle诊断，不适合在线计算，因为每个候选都要付一次完整双向labeling。正式profile继续使用原`default + probe`流程；仅保留显式实验策略能够正确进入首轮probe的接线修正，后续若扩大验证，应优先做多个family/random实例的静态probe-off对照，而不是把工作曲线本身接入求解器。

### 32.39 首轮静态点加后续固定点的代理复核

第32.38节只验证了On-the-exact的`windowAverage`静态点，尚未测试此前提出的另一项改进：每次exact的第一轮使用静态点，后续DSSR轮根据上一完整轮的时间层工作量选择新的`Tmid`，从而避免反复浅层probe。前一节的8点工作曲线还只覆盖`750--1100`，漏掉了真正较轻的区间，因此“windowAverage位于稳定低工作量区域”这一判断需要修正。静态点A/B结果仍有效，但不能把`936.067`解释为接近工作量最优点。

本轮在同一个`n030-set02-family-base-zero-m2`根节点上，把完整诊断扩展到`400,500,600,700,800,900,936.067,1000,1100,1200`，并分别固定DSSR第1轮和第2轮的dual、ng-memory及其余上下文。每组都按正序和逆序扫描，以识别JIT、缓存和墙钟波动。下表给出完整forward+backward耗时，单位为毫秒：

| `Tmid` | 第1轮正序 | 第1轮逆序 | 第2轮正序 | 第2轮逆序 |
| ---: | ---: | ---: | ---: | ---: |
| 400 | 198.285 | 85.069 | 166.353 | 83.970 |
| 500 | 90.018 | 69.544 | 79.165 | 62.916 |
| 600 | 93.415 | 82.764 | 71.926 | 65.449 |
| 700 | 77.088 | 76.930 | 72.801 | 79.276 |
| 800 | 83.827 | 97.303 | 82.327 | 107.888 |
| 900 | 81.147 | 80.015 | 90.362 | 110.664 |
| 936.067 | 87.234 | 93.922 | 78.270 | 93.560 |
| 1000 | 85.845 | 95.254 | 86.305 | 84.224 |
| 1100 | 91.020 | 118.912 | 82.797 | 94.308 |
| 1200 | 81.196 | 194.643 | 81.529 | 149.148 |

扫描顺序的首个候选受JIT影响明显，不能把某个单次最小值当成精确最优点；但两轮、两个方向共同支持`500--700`是较稳定的低工作量区。更关键的是，DSSR更新ng-memory以后，第2轮的低工作量区没有发生大幅漂移。这给“上一完整轮的工作量分布可用于指导下一轮”提供了正证据，也说明只在`750--1100`扫描得到的旧结论不完整。工作量目标仍应是最小化完整`forward+backward`，而不是强制两侧耗时比接近1。

为了检查低工作量区是否可能带来端到端收益，又关闭浅层probe并进行根节点运行。所有运行都完整闭合，得到相同的`LB=UB=37948`和`valid=true`。其中“首轮WA、后续固定500”只是此前设想的一个代理：每次exact第一轮用`windowAverage=936.067`，从第2轮开始无条件使用离线扫描选出的`500`。它没有根据每一轮的工作曲线重新选点。

| 策略 | 重复次数 | 总时间/s | exact时间/s | exact调用 |
| --- | ---: | ---: | ---: | ---: |
| 每轮固定WA 936.067 | 3 | `20.613/20.443/22.573`，均值`21.210` | `17.039/16.826/18.713`，均值`17.526` | 7 |
| 每轮固定500 | 2 | `11.751/13.136`，均值`12.444` | `8.279/9.020`，均值`8.650` | 7 |
| 每轮固定600 | 1 | `13.920` | `10.031` | 8 |
| 每轮固定700 | 1 | `17.206` | `13.544` | 7 |
| 首轮WA、后续固定500 | 2 | `12.005/11.998`，均值`12.002` | `8.370/8.328`，均值`8.349` | 7 |

该固定点代理相对每轮固定WA的平均总时间下降`43.4%`，exact时间下降`52.4%`。日志确认第一轮确实使用`936.067`，第2轮以后使用`500`；第一次exact共执行10轮DSSR，首轮完整F/B比为`18.669`，切换后第2轮降至`1.180`，后续多数轮在`1--3`。这个结果只说明“首轮后固定在500”在该实例上有效，并说明低工作量区值得继续利用；它不能证明根据上一轮工作曲线自适应选点的方法已经实现或已经取得同样收益。
...
第32.39节中约`12s`的运行不是“根据上一完整轮工作量曲线选点”。它关闭了probe，每次exact第一轮使用`windowAverage=936.067`，后续轮无条件固定`Tmid=500`。该运行还是`n=30,m=2`的root-only诊断，关闭strong branching，根节点在列生成闭合后已经为整数；因此只执行7次exact pricing，总时间约12秒。这个结果只能说明离线选出的固定500在该实例上有效，不能作为轮间曲线策略的证据。

本轮随后临时实现了此前真正提出的在线策略。每次完整DSSR轮把已pop的label按有效时间域分入256个时间桶，并以`1 + extensionSet.cardinality()`作为确定性的扩展工作代理。对候选分割点`s`，使用上一轮观测到的forward桶前缀和backward桶后缀，选择使两侧估计工作量最大值最小的点，作为下一轮`Tmid`。第一轮仍从`windowAverage`开始；第2轮以后直接使用曲线选择，不运行浅层probe，也不使用固定500。

相同`n030-set02-family-base-zero-m2`上的结果如下。所有运行均完整闭合并得到`LB=UB=37948`。

| 策略 | probe | 重复次数 | 总时间/s | exact时间/s | exact调用 |
| --- | --- | ---: | ---: | ---: | ---: |
| 当前`windowAverage + probe` | 开 | 2 | `13.369/13.985`，均值`13.677` | `9.183/9.155`，均值`9.169` | 7 |
| 首轮WA、后续固定500 | 关 | 2 | `12.005/11.998`，均值`12.002` | `8.370/8.328`，均值`8.349` | 7 |
| 上一轮曲线直接选点 | 关 | 1 | `41.106` | `37.277` | 8 |

曲线策略不是小幅波动，而是出现确定性的端点振荡。第一次exact中，第1轮`Tmid=936.067`、完整F/B耗时比为`15.910`；上一轮曲线随后依次选出`112.236、1766.607、35.451、1766.607、35.451...`，对应完整比先变为`625.705、287.003、8855.030`，之后继续在时间域两端以数千至数万倍失衡振荡。其他exact调用重复了同一模式，因此没有必要用更多重复运行确认这是一项稳定退化。
...
上一轮由于误解“测试原始阈值”的含义，在相同n50 default上额外把后续DSSR的feedback触发阈值和probe接受阈值从4同时改为3；首轮比例仍为1.5，后续步长仍为5%。该阈值3运行得到`208.337s / exact 119.167s / 40次exact / 157轮DSSR`，但它不是历史原始配置，不能回答2与4的比较，只保留为无关诊断。诊断runner新增了`midpointProbeDssrEarlyStopRatio`覆盖入口，未提供属性时原值原样保留；正式profile仍为4。
...
### 32.43 原始2/4阈值、扩展起点A/B与墙钟噪声

历史原始配置不是“两个阈值都为2”。其准确含义是：上一完整DSSR轮forward/backward耗时比超过2时生成adaptive seed，但新一轮浅probe仍把4倍以内视为可接受，即`dssrImbalanceThreshold=2 / dssrAcceptableRatio=4`。当前正式配置则为`4/4`。本轮用有效配置日志逐条确认了2/4口径，并保持首轮接受比1.5、后续5%步长、default起点、probe和完整轮反馈均不变。

先扩大default与windowAverage起点比较。六个根节点配对均串行、CPLEX单线程运行；n40闭合或处理完根节点，n50限制一个根节点以隔离pricing。每个配对的最终根bound完全相同且`valid=true`。

| 实例 | default总时/exact/s | windowAverage总时/exact/s | exact调用D/W | windowAverage总时变化 |
| --- | ---: | ---: | ---: | ---: |
| n40-set01 family,m2 | 217.121 / 202.572 | 222.156 / 208.709 | 12 / 11 | +2.32% |
| n40-set02 family,m3 | 11.400 / 3.670 | 12.771 / 5.235 | 5 / 6 | +12.03% |
| n50-set01 family,m3 | 209.926 / 120.748 | 157.955 / 89.534 | 44 / 22 | -24.76% |
| n50-set02 family,m3 | 52.418 / 25.402 | 86.863 / 56.199 | 6 / 11 | +65.71% |
| n40-set02 random,m3 | 6.251 / 0.979 | 6.560 / 0.976 | 7 / 7 | +4.94% |
| n50-set02 random,m3 | 17.323 / 3.803 | 15.328 / 2.749 | 12 / 8 | -11.52% |

windowAverage只在6个点中的2个更快，总时间配对变化的中位数为`+3.63%`，即反而略慢；exact变化中位数为`+1.36%`。六组静态reference的差分别为`22.95/26.30/115.25/34.06/35.55/38.04`，只占各自有效时间域的`1.23%/1.55%/5.05%/1.65%/2.03%/1.75%`。公式本身通常只差约1%--2%的时间域，但它改变了固定步长probe的候选网格：例如n50-set01的default在691.20与805.40之间切换，windowAverage则全部落在692.25；n50-set02又出现相反的长CG轨迹。因此小reference差异可以通过离散probe落点、返回列、RMP dual和后续exact调用数放大，不能用单次端到端优势判断公式优劣。

“是否受迭代影响”需要分两层回答。给定同一节点及相同effective windows，default和windowAverage reference是确定的，本轮所有根日志中每次exact的静态seed均保持不变；同一次exact的第2轮以后也不再直接读取静态公式，而是使用上一完整轮反馈。但每次新的exact调用都会重置`ngDssrReusableTmid`并再次从静态reference开始，所以该起点会在整个CG过程中反复进入每次exact的第1轮。首轮返回列改变dual后，后续DSSR和CG轨迹随之变化。因此公式数值不随同节点迭代变化，其算法后果明显受迭代放大。

正确的2/4与当前4/4比较如下。表中adaptive和discard分别为完整轮反馈实际采用adaptive seed的次数，以及所有probe中除最终选中候选外被丢弃的候选数。

| 实例 | 4/4总时/exact/s | 2/4总时/exact/s | exact调用4/2 | adaptive 4/2 | discard 4/2 |
| --- | ---: | ---: | ---: | ---: | ---: |
| n40-set01 family,m2 | 217.121 / 202.572 | 250.216 / 236.047 | 12 / 15 | 13 / 59 | 48 / 59 |
| n40-set02 family,m3 | 11.400 / 3.670 | 10.872 / 3.874 | 5 / 5 | 0 / 9 | 10 / 11 |
| n50-set01 family,m3 | 209.926 / 120.748 | 214.898 / 142.338 | 44 / 32 | 0 / 39 | 113 / 263 |
| n50-set02 family,m3 | 52.418 / 25.402 | 46.351 / 28.890 | 6 / 10 | 1 / 11 | 32 / 109 |
| n40-set02 random,m3 | 6.251 / 0.979 | 6.575 / 1.011 | 7 / 7 | 0 / 1 | 0 / 0 |
| n50-set02 random,m3 | 17.323 / 3.803 | 14.842 / 2.624 | 12 / 8 | 14 / 9 | 0 / 0 |

2/4同样快3个、慢3个，总时间配对变化中位数为`-1.13%`，可视为持平；但exact时间变化中位数为`+9.64%`。只看4个family点，2/4合计总时比4/4高`6.41%`，exact高`16.67%`；adaptive由14次增到118次，discard由203增到442。尤其n40-set01的2/4共59次adaptive全部来自最终完整比不超过3.12的中等失衡，n50-set01又在完整比始终低于3.49时产生39次adaptive和263个丢弃候选。2/4确实更积极，但新增移动没有转化为稳定收益，反而系统性增加family定价的probe工作。结合第32.37节已有n30重复实验，当前仍应保留4/4。

墙钟比例本身确实容易受噪声扰动。n50-set01的两次完全相同default运行中，第一次exact第1轮均为`Tmid=805.4`、保留labels均为`7164/3356`，但forward/backward耗时从`235.5/109.5ms`变为`229.1/93.9ms`，比值从2.150变为2.441，变化13.5%。两次算法工作量接近，根时间仍为209.9与236.1秒。更极端的是n50 random某轮完整比达到116.37，但两侧合计只有14.2ms；此时“大比例”并不代表值得为它改变全局seed。另一方面，时间方向与label数量方向在部分调用中不一致，说明简单改用pop数或kept-label数也不可靠，因为不同label的PWLF、dominance和extension成本不同。

当前没有免费的稳定Tmid优化。上一轮只能观测当前分割点左侧forward和右侧backward，无法推导其他分割点的完整总工作曲线；第32.40节的直接工作曲线迭代已经因缺失区间而失败。重复测试多个完整候选或取中位数可以稳定，但成本本身接近额外labeling。继续微调静态点、阈值或步长，主要是在改变有限列批次和CG长尾，不能保证当前轮更平衡就使全局更快。

若以后只做一个低侵入的后续诊断，优先同时记录wall time与当前Java线程CPU time，不立即改变决策。ng-DSSR labeling在当前线程串行执行，thread CPU time可以排除OS抢占和大部分后台负载；先在固定dual/ng-memory快照和重复运行中比较两种比例的方向一致率与方差，再决定是否替换wall time。它仍不能消除JIT、缓存和每个label成本差异，也不会解决小分母问题。

另一个更保守的候选是“rank-0/materiality gate”：若上一轮probe已经在当前Tmid耗尽两侧队列，说明该轮完整搜索落在probe预算内；即使耗时比很大，也先复用上一Tmid，让新memory下的正常probe重新验证，而不根据一个很小的绝对耗时做全局分位数跳转。对于非rank-0轮，还可要求预期节省超过一次额外probe的CPU成本再移动。该思路只控制启发式seed，不影响定价正确性，但尚未A/B，当前不写入主线。

本轮最终决定不变：正式配置继续使用`default + probe + 完整轮反馈 + 4/4 + 5%步长`。windowAverage和原始2/4均保留为实验选项，不切换默认。就Tmid迭代本身而言，目前没有证据支持继续参数微调；下一步若仍研究，只先补CPU/wall双计时诊断，再决定是否测试rank-0 gate。

### 32.44 3/4阈值口径与时间比较的后续边界

用户所说的“阈值3”是`dssrImbalanceThreshold=3 / dssrAcceptableRatio=4`，即只把上一完整轮触发adaptive seed的门槛放在2/4和4/4之间，后续probe仍保持4倍以内接受。该`3/4`组合目前没有系统测试。第32.42节唯一已有的阈值3运行是`3/3`，同时收紧了adaptive触发和probe接受两个机制，只能作为无关诊断，不能用于推断3/4。当前不补跑、不修改正式配置；若以后为了完整性比较，只应在相同串行配对上比较`2/4、3/4、4/4`，并同时统计exact时间、adaptive次数、丢弃候选和完整CG轨迹，而不能只看总时间。

当前仍使用forward/backward实际elapsed time作为负载指标。原因是算法真正希望平衡计算时间，而pop、constructed、kept labels或队列长度都没有统一成本：不同label触发的PWLF运算、dominance比较和可扩展arc数量差异很大。现有4倍触发/接受阈值本身形成了较宽的deadband，可以吸收一部分短时噪声；因此暂时不改成确定性计数，也不再追求接近1的耗时比。
