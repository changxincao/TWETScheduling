...
新增全局求解时间限制 `solveTimeLimitSeconds`，默认 `0` 表示不限制。该限制按旧 VRP 的思路使用 wall-clock 时间控制，但当前只在节点之间检查，不强行中断正在执行的 LP、pricing、RMIH 或 finite master；因此它是全局搜索层面的粗粒度保护。若时间耗尽且队列未空，最终状态返回 `TIME_LIMIT`。route enumeration 的 finite master 不再使用独立的每次 MIP 时间上限，而是只接收全局剩余时间；没有全局时间限制时不设置 CPLEX `TimeLimit`。
...
有限主问题覆盖约束仍保持当前 RMP 的 set-covering 口径 `cover_j >= 1`。在当前 route enumeration 的内部机器列枚举中，机器列本身是 elementary 序列，`getJobVisitCount(job)` 实际上退化为 0/1；保留 visit count 写法只是为了和通用列对象兼容。若把有限主问题直接改成 `==1`，在列集不足时可能导致本来 `>=` 可证明的节点无法关闭，也会丢掉类似 RMIH 后处理可修复的覆盖重复解。因此当前建议继续用 `>=` 做证明模型；只有在后续专门做“枚举列足够完整且只接受严格排程整数解”的诊断模式时，再考虑单独加 `==` 版本对照。
