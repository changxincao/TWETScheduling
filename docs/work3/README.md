...
当前实现原型采用共同的 \(\rho,\lambda,\alpha,\beta\)，随机加工与机器无关的随机序列相关 setup 时间、确定 setup 金额、任务特定窗口域、免费场景 waiting 和线性 ET 成本。第一阶段只实现并比较 2-index 与 P-W-Abs 两个基础模型及其静态强化；P-W 的 \(y,u,z\) 均显式设为 0–1 变量，随机一致性验证单独放在新工程 `test/` 下。W3-FullHet 保留为较早的一般化方案，不是本轮实现入口。
