...
/**
 * no-cut 双向 pricing 的 full-domain GCBB-style 对照版。
 * <p>
 ...
 * 相对正式 half-domain 基准 {@link GCNGBBStyleBidirectional}，本类保留同样的 GCBB-style
 * final join、forward->sink 收尾和 top-K 候选池流程，但 forward/backward 标签函数都直接定义在
 * [0, pricingHorizon]；join 阶段不再依赖 half-domain 常数延拓。该类主要用于对照“半域裁剪”
 * 与“完整定义域标签”的效果，不作为默认正式入口。
 * <p>
...
public class GCBBStyleBidirectionalFullDomain {
	...
	private double completionBoundCutoff() {
		// 2026-05-31: extension pruning only asks whether a label can still complete
		// to a negative column. Using the current best reduced cost here would turn it
		// into record-only pruning and drop valid negative columns for top-K pricing.
		return REDUCED_COST_TOLERANCE;
	}
