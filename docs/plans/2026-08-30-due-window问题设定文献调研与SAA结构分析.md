...
当前主要结论是：**机器问题完全可以不设 overtime，而且当前所有候选模型都允许场景内等待；需要比较的是等待免费还是付费。** 宽度必须由正宽度成本、固定宽度或最大宽度控制。免费等待时，必须另用正位置成本或真实承诺范围锚定窗口位置，否则有限 SAA 可同步到最迟样本并消掉 ET；付费等待时，只要所有能推动时间线的主动等待均被计费，正等待成本本身就能提供位置斜率。当前主模型应为“正宽度成本 + 期望 ET + 二阶段付费等待”，免费等待版本保留相同窗口成本，作为结构和退化基准；不再把 no-voluntary-idle 作为当前问题的候选模型。
