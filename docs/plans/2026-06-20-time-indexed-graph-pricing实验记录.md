...
## 16. 2026-06-20 补充：immediate-precedence 与当前 arc branching 的关系

原文 immediate-precedence branching 的变量本质上也是“任务 i 是否紧接在任务 j 前面”的聚合弧变量，分支会把对应弧的取值固定到 0 或 1。对 time-indexed graph pricing 来说，这类约束仍然可以通过禁止或奖励/惩罚对应 processing arc 处理，不会破坏 DAG shortest path 结构。因此，若只比较 no-cut graph pricing 与当前函数定价的核心差异，当前 directed arc branching 已经属于同一类分支思想，不必为了“完全同名”再单独实现一套 immediate-precedence 分支。

两者仍有细节差别。当前 `ArcBrancher` 在右支 require `i->j` 后，还会对真实 job i 禁掉其它后继、对真实 job j 禁掉其它前驱；这比单纯固定聚合变量 `z_ij=1` 更强，目的是在 elementary 精确解语义下直接表达“i 的唯一后继是 j，j 的唯一前驱是 i”。左支则就是 forbid `i->j`。端点弧 source/sink 不做这种排他处理，避免误禁其它机器的起点或终点。原文的 immediate-precedence 聚合变量通常不需要显式加这么多排他禁弧，因为主问题/路径结构本身保证每个 job 的前后继关系；在当前 TWET-BPC 里，右支额外禁弧是为了让 pricing 和 RMP 分支行更快收紧。

因此当前对照实验的判断是：核心步骤已经具备，包括 time-indexed DAG pricing、post-CG reduced-cost arc fixing、现有 directed arc branching、访问次数列系数和 RMP 求解。它不等于完整原文 BCP，但足够作为“论文式 time-indexed pseudo-schedule pricing”与当前函数占优定价的工程对照。后续若差距较大，优先观察列返回数量、RMP 列池增长、arc fixing 固定量和分支节点表现，而不是先纠结 immediate-precedence 与当前 arc branching 的命名差异。

## 17. 2026-06-20 40-2 原 setup 当前 time-indexed graph pricing 测试

用当前代码重新测试 `data/40-2/wet040_001_2m.dat`，配置为 `timeIndexedGraphPricing=true`、半域模式、ALNS/RMIH/现有启发式 pricing 开启、无 SRI、关闭无向 adjacency branching，`timeIndexedGraphMaxExactPricingColumns=300`。运行目录为 `test-results/bpc/tmp-timegraph-40-2-current-20260620-163616`。

本次完整收敛，结果为 `FINISHED, incumbent=bound=22580, nodes=29, solve=628.307s, valid=true`。总 pricing 轮数 `1022`，最终 pool/列数为 `55367`。time-indexed exact pricing 本身累计 `20.950s/231 calls`，master LP 为 `15.840s`，现有 heuristic pricing 为 `461.353s/791 calls`。这说明在当前嵌入方式下，DAG exact pricing 不是主要瓶颈，主要时间仍在现有 seed/tabu 风格启发式 pricing 和由其带来的大列池/RMP 循环。

root 节点用时约 `79.044s`，根 LP 下界 `22487.647059`，初始整数上界 `22582`，root gap 约 `0.4178%`。root 中 heuristic pricing 为 `67.962s/81/add12617`，exact graph pricing 为 `3.841s/26/add489`，post-CG time-indexed arc fixing 在 root 处理约 `1,505,028` 个候选并固定 `2,351,881` 条 time-indexed 弧，耗时约 `2.733s`。后续 node 11 找到更好 incumbent `22580`，最终 node 29 将全局 bound 推到 `22580` 后闭合。

当前结论是：在原 setup 40-2 上，time-indexed graph pricing 能给出正确收敛结果，并且 exact DAG 部分很快；但完整求解时间仍明显受当前 TWET-BPC 外围流程影响。若继续沿论文 time-indexed 方向推进，优先应处理 graph-native 候选返回/启发式阶段和列池增长控制，而不是继续优化单次 shortest-path DP。

## 18. 2026-06-20 40-2 原 setup 关闭现有 heuristic pricing 的对照

为验证“上一节总时间主要耗在现有 `HeuristicPricingEngine`”是否意味着可以直接关闭启发式，本次保持同一个 40-2 原 setup 配置，只设置 `enableHeuristicPricing=false`，其它保持 `timeIndexedGraphPricing=true`、ALNS/RMIH 开启、无 SRI、关闭无向 adjacency branching、`timeIndexedGraphMaxExactPricingColumns=300`。运行目录为 `test-results/bpc/tmp-timegraph-40-2-heurOff-20260620-1642b`。

该 run 没有等到 root 闭合，约 `246.5s` 后手动停止。停止时仍在 node 1，最后 heartbeat 为 `restricted=49394, pool=49394, cuts=0`，仍在进入下一轮 `TimeIndexedGraphPricing`。作为对比，上一节启发式开启时 root 约 `79.044s` 后分支，root restricted/pool 约 `13207`，并且完整搜索最终 `628.307s` 收敛。

因此这里的结论不是“启发式耗时高所以应该关闭”，而是当前现有启发式虽然自身耗时大，但它生成的列对 RMP 稳定化更有效；直接关闭后，exact graph pricing 每轮按上限补一批 pseudo-schedule 列，root 需要大量 LP/pricing 轮次，列池比启发式开启时膨胀得更快。后续如果要替换现有启发式，应做 graph-native 的受控候选返回或原文式两阶段 pricing，而不是简单关闭启发式。

## 19. 2026-06-20 40-2 zero setup 关闭现有 heuristic pricing 的对照

继续用同一套 time-indexed graph pricing 口径测试 zero setup，即 `data/40-2/wet040_001_2m.dat` 加 `zeroSetup=true`，只关闭现有 `HeuristicPricingEngine`。其它配置保持 ALNS/RMIH 开启、无 SRI、关闭无向 adjacency branching、`timeIndexedGraphMaxExactPricingColumns=300`。运行目录为 `test-results/bpc/tmp-timegraph-zero40-heurOff-20260620-1710`。

该 run 约 `216.4s` 后仍未闭合 root，手动停止。停止时仍在 node 1，最后 heartbeat 为 `restricted=27985, pool=27985, cuts=0`，仍在进入下一轮 `TimeIndexedGraphPricing`。对比已有启发式开启的 zero setup 完整 run：`FINISHED, incumbent=bound=17881, nodes=17, solve=274.001s, root≈55.179s, pool=36085`；因此关闭启发式后，虽然还没有达到完整 run 的最终 pool，但已经远超过启发式开启时 root 阶段的列规模，且仍未进入分支。

zero setup 上的结论与原 setup 一致：现有启发式看起来耗时高，但直接关闭会让 root 退化成很多轮 exact graph + RMP 重解，补列过程更碎，不能作为提速方案。time-indexed graph 方向如果继续做，关键仍是设计 graph-native 的受控批量列返回/启发式阶段，而不是简单把当前启发式关掉。

## 20. 2026-06-20 no-cut time-indexed graph pricing 的当前结论

结合 40-2 原 setup 和 zero setup 的几轮测试，当前判断是：暂时没有必要继续投入无 cut 的论文式 time-indexed graph pricing。它的单次 DAG pricing 很快，但非基本 pseudo-schedule 太容易产生负 reduced-cost 列；如果不开现有 elementary/repair 风格的启发式 pricing，root 会生成数万条列仍难以闭合。如果打开现有启发式，完整求解能跑动，但主要依赖的是我们已有的 elementary 列生成和修复能力，time-indexed graph exact 本身不是主要贡献。

这个方向还有几个结构性问题。第一，pseudo-schedule 非基本，重复任务会让负列很多，RMP 列池增长快。第二，离散时间图需要 scale，时间离散化本身会带来额外工程复杂度。第三，同一个任务序列可能在不同离散完工时间点形成多个负 reduced-cost 候选；这相当于当前函数定价里同一个 label 的多段函数值都可能为负，但真正需要的是该序列在连续调度意义下的最优成本。当前实现进入 RMP 前会用 `TWETColumnEvaluator` 重算真实 sequence cost，但 pricing 阶段仍可能为同一序列做重复搜索和候选管理。

当前代码已经做了精确序列级别的去重。`TimeIndexedGraphPricingEngine` 在恢复候选列时先用 `SequenceSignature` 检查当前 RMP active columns，active 中已有完全相同 job sequence 的候选会跳过；同一轮 pricing 内部再用 `candidateBySignature` 保存候选，如果同一 sequence 从不同 end state 或不同时间点恢复出来，只保留 reduced cost 更小的版本，最后从这个 map 中排序返回前 `timeIndexedGraphMaxExactPricingColumns` 条。因此，“同一完整任务序列、不同时间点”不会在同一轮被重复加入多次。全局 `Pool` 也会按列签名继续去重。

但这个去重只解决完全相同 sequence 的重复，不解决该方法的根本问题。不同重复访问顺序、不同 pseudo-schedule、或者删除重复 job 后投影到同一 elementary 序列的列，仍然会被视为不同列；这也是 time-indexed graph no-cut 版本列池膨胀的主要来源之一。后续若保留这套代码，更适合作为诊断/对照定价器，而不是当前 TWET-BPC 的主线 exact pricing。

### 2026-06-28 time-indexed rank-1 cut pricing 新实验入口

按“新增模块、最小接线、不改 no-cut engine”的口径，新增 `TimeIndexedGraphRank1CutPricingEngine`。该定价器在没有 active subset-row pricing cut 时委托原 `TimeIndexedGraphPricingEngine`，有 active cut 时改用带 residual state 的 time-indexed bucket label：同一 `(lastJob,time)` bucket 内保留多条不同 cut state 的 label，并按论文式补偿规则做 dominance，即若 label A 的 reduced cost 不高于 label B 加上 A residual 更大的那些 cut dual 补偿，则 A 支配 B。列成本仍从 graph reduced cost 反推 objective cost：加回 machine/job/arc dual，同时按 `SubsetRowCutEvaluator` 的列系数加回 active cut dual，保证进入 RMP 的 `TWETColumn.cost` 不包含 cut penalty。

本次只做单向 full-horizon time-expanded label 版本，目的先验证 rank-1 cut state 是否能改善当前 time-indexed 对照，不一次性引入原文完整双向 cut pricing。新增开关为 `useTimeIndexedGraphRank1CutPricing`，测试入口属性为 `twet.bpc.fullDomainCompare.timeIndexedGraphRank1CutPricing`；subset-row cut 另设 `enableSubsetRowCutsForTimeIndexedGraph`，避免影响原 partial-ng SRI 路径。启发式 pricing 的 SRI 上下文也放开到该新模式，否则 active cut 后启发式会按无 cut reduced cost 误判候选列。建议测试时使用 `subsetRowCutMemoryMode=arcMemory` 或 `nodeMemory`，因为 time-indexed pseudo-schedule 允许重复 job，limited-memory 口径和当前列系数更一致。

验证方面，focused `javac` 已通过。40-2 root-only smoke 配置为 `timeIndexedGraphPricing=true`、`timeIndexedGraphRank1CutPricing=true`、`enableSubsetRowCutsForTimeIndexedGraph=true`、`subsetRowCutMemoryMode=arcMemory`、`maxNodes=1`、`solveTimeLimitSeconds=120`。结果目录为 `test-results/bpc/tmp-timegraph-rank1-smoke-20260628`，状态 `NODE_LIMIT, incumbent=22582, bound=22506.966667, gap=0.3323%, valid=true`。日志显示新 engine 已在 active cut 后运行：`cut rounds=16, added cuts=80`，`TimeIndexedGraphRank1CutPricing=12.501s/79 calls`，单次 rank-1 cut pricing 大多约 `0.16-0.24s`，每轮 labels kept 约 `36-41` 万、dominance 删除约 `322-324` 万。随后补了 `twet.bpc.fullDomainCompare.maxCutRounds` 入口，并用 `maxCutRounds=1` 做短 smoke，目录 `test-results/bpc/tmp-timegraph-rank1-smoke-maxcut1-20260628`，结果 `NODE_LIMIT, solve=34.490s, exact=1.790s/25 calls, valid=true`，日志显示只加入一轮 subset-row cut。该 smoke 只证明接口和 reduced-cost 口径可运行，尚不能说明它优于 ng-DSSR；后续要在 60-2 root 上做同配置对照。

### 2026-06-28 对照论文后的正确性核查

重新核对论文第 3-6 节后，当前新增模块只能称为“time-indexed rank-1 cut pricing 的单向实验版”，不能称为完整复现论文 BCP。和论文一致的核心部分是：active cut dual 为负时，路径 reduced cost 等于普通 time-indexed arc reduced cost 加上 `-mu * coeff`；`arcMemory` 口径下 state 按 job-pair memory 扫描，非 memory arc 先清零，随后当前 scope job 仍可作为新段起点计入；同一 `(lastJob,time)` bucket 内用 residual state 做 dominance，条件对应论文中 `c1 <= c2 + sum(mu_l for S1_l > S2_l)` 的充分支配规则。进入 RMP 的列成本也按 reduced cost 反推并加回 machine/job/arc/cut dual，口径和 LP 的 cut 行系数一致。

不一致处也必须明确。第一，论文定价是双向 labeling，并在 `t*` 处拼接 forward/backward labels；当前 rank-1 模块是单向 full-horizon label，没有实现 Algorithm 5 的拼接 state 修正，也没有用 cut-aware full-horizon label pair 做 Algorithm 7 arc fixing。第二，论文 limited-memory 使用 job-pair memory，并在 Algorithm 2 中把 `Mmin` 的反向 pair 和所有正 multiplier job pair 加入 memory；当前 `arcMemory` 只构造保持当前正值列系数所需的 memory arcs，没有额外补这些 pair，因此 cut 仍有效但可能更弱，后续列上的系数也可能比论文策略更小。第三，论文 separation 同时允许一行和三行 rank-1 cuts，每轮上限为 50 个一行 cut 和 75 个三行 cut，并带 tailing-off 与 inactive cut 删除；当前只接三元 subset-row cut，默认每轮数量更小，也没有完全复现 tailing-off 和 inactive cut 管理。第四，论文 column generation 有两阶段 graph-native pricing，第一阶段每个 bucket 只扩展 reduced-cost 最小 label，第二阶段 exact；当前仍使用现有 `HeuristicPricingEngine` 加新 exact engine 的组合，不是论文的 bucket heuristic。第五，`full` SRI 口径仍沿用项目里 distinct-visit 的旧实现；若要贴近论文 pseudo-schedule rank-1 cut，应优先使用 `subsetRowCutMemoryMode=arcMemory`，因为它按出现次数扫描 sequence，更接近论文 Algorithm 1。

因此当前实现的正确性结论是：在 `arcMemory` 模式下，单向定价器本身的 reduced-cost、state 更新、dominance 和 LP 列成本反推没有发现符号错误或明显不安全的删列；但它不是论文完整算法，尤其不能用它的 60-2 表现直接判断论文完整 BCP 是否可信。后续若要严格对标论文，需要至少补齐双向 cut labeling、拼接 state 修正、论文式 memory 扩展、cut separation 数量/停止规则，以及 cut-aware arc fixing。

### 2026-06-30 旧版 time-indexed root 慢化原因复核

补做旧 commit `98522aa7` 与当前版本的 time-indexed root-only 对比。当前版本在 `timeIndexedGraphPricing=true`、关闭旧 `HeuristicPricing`、关闭 strong branching、`maxNodes=1` 下，`wet040_001_2m` root 用时约 `15.000s`，time-indexed pricing 为 `4.933s/211 calls/add44524`，第一轮 pricing `180.830ms`。旧 commit 用同口径外部 90 秒 timeout 测试，第一轮 `TimeIndexedGraphPricing` 就耗时 `31668.380ms`，只返回 `173` 列，随后仍在第二轮 root pricing 中。

静态对比确认主要差异不在 CPLEX 或分支，而在候选恢复和成本管理。旧版对大量 negative end states 恢复 sequence 后调用 `TWETColumnEvaluator.evaluate(sequence)`，并且 active signature 直接跳过、Pool 不更新更低成本；当前版本先用 top-candidate reduced-cost 过滤，再从 graph reduced cost 反推 objective cost，并允许同 sequence / active 列成本改进后触发 RMP 重解。因此旧版“root 很慢”的核心原因是 time-indexed 候选成本恢复路径过重且 active 列成本不能刷新，不是 no-cut time-indexed shortest path 本身需要几十秒一轮。

### 2026-06-30 旧版 time-indexed 慢化证据的校正

需要修正前面对旧版 root 慢化的表述。日志里的 `negativeStates` 表示扫描到的负 reduced-cost end state 数，不等于 `TWETColumnEvaluator.evaluate(sequence)` 的调用次数。旧 commit `98522aa7` 的代码在恢复 sequence 前仍有 `isPotentialTopCandidate(reducedCost)` 过滤，并且 active signature 会直接跳过，因此不能说旧版对十几万 negative end states 全部做了 evaluator 重算。
