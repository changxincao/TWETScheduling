...
2026-06-25 已按上述思路实现第一版。外包枚举每生成一个 skip/include partial label 后，若 `routeEnumerationUseCompletionBound=true`，就用当前 baseline/profit 加剩余 free job 的 suffix lower bound 判断是否仍可能产生 `rc < gap` 的外包列；不可能则不进入下一层枚举。suffix 中每个剩余 job 的边际成本用 `G(S + prefixBefore + b_j) - G(S + prefixBefore)` 计算，`prefixBefore` 取当前枚举顺序下该 job 前方剩余 job 的 baseline 总和。若该边际计算遇到 BigM/定义域边界，则本次剪枝放弃并保留 label，避免误剪。剪掉的外包 partial label 数计入 route enumeration summary 的 `cbPruned`。
