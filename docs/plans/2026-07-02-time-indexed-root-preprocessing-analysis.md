...
2026-07-12 再次复查了该等价性的边界。早期 multi-delta 实现曾尝试 transition-only 后因 `U/R` 对拍不一致而撤回；当时的问题是没有保留完整 candidate 对 job eligibility 的过滤。当前版本在重建前先检查对应 reduced penalty 非 null，并且 normal、ng-DSSR、partial、full-domain、node-join 和 subtree arc fixing 传入的 completion-bound penalty 都覆盖完整 `[0,pricingHorizon]`，硬窗外以 BigM 表示而不是物理裁掉。因此只要 transition 的 `u/r` 有正长度，后续与非 null job penalty 相加不会因定义域不交而消失，省略 job-cost add 和第二次 normalize 不改变是否纳入 `U/R`。用当前源码在 `wet040_001_2m` 上关闭 ALNS/启发式、开启逐函数 compare，60 秒内完成的 33 次构造全部输出 `result=equal`；focused `javac` 和 `git diff --check` 也通过。该优化在当前接入下严格等价，但以后若给 `CompletionBoundCalculator` 传入物理窄域 penalty，必须恢复完整 candidate eligibility 检查，不能直接沿用 transition-only 假设。
