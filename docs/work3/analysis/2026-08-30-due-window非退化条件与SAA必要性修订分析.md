...
2026-08-31 已将本文及前序讨论收束为统一的 Work 3 问题设定报告：[随机 due-window 多机调度的问题设定、非退化机制与文献依据](../current/2026-08-31-work3随机due-window多机调度问题设定与文献依据.md)。本文继续保留逐项推导和历史边界，后续建模以该统一报告的 W3-Core 为准。
...
若首任务开始不固定、首任务之前的 initial idle 又不计价，则该模型首先出现的是绝对位置平移不唯一，而不是 ET 目标失效或“所有 earliness 必为 0”。对某台机器及任意 $K\ge0$，把该机器全部任务和窗口同时平移：
...
同时把首任务前空闲增加 $K$。窗口宽度、各任务 ET、相邻任务之间的 waiting 和目标值全部不变，因此存在任意晚的一族等价最优解，窗口绝对位置无法识别。该现象更准确地称为时间平移对称或非唯一性：目标值仍然有界，场景间“接受 earliness 还是增加 waiting”的相对时间权衡仍然存在，SAA 并没有因此退化成无时间成本的问题。对求解而言，连续子问题可能具有无界的零成本方向和无穷最优面，通常仍可得到有限最优值，但会增加退化、对偶不唯一和数值不稳定风险。

单场景且窗口完全可决策时，还可能把点窗口直接放到实际完工时刻使 ET 为 0；但在多场景 SAA 中，各场景相对完工间隔不同，单纯整体平移不会改变这些相对差异，所以不能据此断言全部 earliness 必为 0。免费 initial idle 只是一个机器级的共同延迟变量：它可以降低一部分任务的 earliness，同时可能增加另一些任务的 tardiness，最终仍由多场景成本权衡决定。
...
则窗口不能随排程无限后移，上述共同平移最多进行到某个端点碰到 $U_j$，无限零成本射线被切断。此时即使 initial idle 不计费，模型通常仍有有限的最优 timing；initial-idle cost 不再是防止位置不识别所必需的条件，只保留其运营成本含义。但有限区间不保证最优解唯一，也不保证一定出现正 earliness：范围很松时仍可能出现一段平坦最优区间，或大量窗口贴近上界。
...
单独不能消除平移对称，因为同时给 $a_j,b_j,C_{j\omega}$ 加上 $K$ 不会改变窗口长度。只有有限的位置区间、位置成本、固定首任务开始或 initial-idle cost 这类绝对时间锚，才能切断共同平移。因此，“位置区间 + 长度限制”同时存在时，平移非唯一问题基本消失；若只有长度限制，则问题仍在。
