...
### 16.1 为什么最终增量版完整树仍可能明显变慢

进一步逐分支核对后，需要纠正“候选语义完全不变”的表述。增量公式与直接枚举在数学上等价，回归也确认候选集合完全相同、边界值在`1e-9`内一致；但两种写法的浮点加法顺序不同。当前实现先按原始`double`的“距最近半整数”排序，再截取top-40并只测试前20个。大量CutSet的边界恰好接近`1.5、2.5、3.5`，其排序差距只有`1e-15`量级，因此微小舍入差会改变进入strong branching的候选。数学候选没有变，实际搜索流程却会在离散截断处发生跳变。

random set03在前三次分支完全一致，第四次已经分叉：旧版从测试池中选择`cutSet(1-38-49)`，score约`179442`；增量版选择`cutSet(20-34)`，score约`36635`。family zero前17次分支一致，第18次CutSet分支中，旧版选择的候选score约`1099086`，增量版选择的候选只有约`408901`。后者左右界提升更弱，随后分别形成`151 -> 162`个节点和不同列池。因此，最终完整树变慢的一部分确实来自浮点并列候选经过top-K截断后改变了分支轨迹，而不是增量候选计算本身耗时增加。

两组总时间还包含不同性质的运行速度差异。random旧/新运行分别为`266.929s/413.636s`，但新运行在任何CutSet分支发生前，根节点就由`46.555s`增至`73.526s`，比例为`1.58`；总时间比例为`1.55`。同时pricing由`186.679s`增至`294.867s`，master LP由`73.970s`增至`111.058s`，几乎解释全部时间差，而且新树反而只有`22`个节点、旧树有`24`个。这一组不能解释为新CutSet树更差，主要是整次JVM运行中各核心组件单次速度共同下降。family旧/新运行的根节点反而略快，但第18次分支后走向不同，节点、列数、pricing和strong-trial RMP均增加；该组才主要体现分支轨迹差异。

因此，“以前好”需要分两种理解。前面supernode、只保留第一层和目标`1.5`的版本确实删除或改写了有用候选，所以算法性变差；最终增量版没有这类集合语义错误，但旧排序本身依赖浮点尾差，旧完整树只是落在了一个更有利的并列候选顺序上，不能据此证明旧构造原则更强。后续若继续启用CutSet，必须先把半整数距离的并列处理改成显式、稳定的数值规则，再重新做成对实验；否则一次完整树时间不具备可复现的算法含义。

### 16.2 三位排序精度

旧版wide、zero、narrow和两个random长算例日志共提取242次分支中的1936个top预览CutSet。将“距最近半整数”四舍五入到三位后，只有6次分支额外合并了原本差异超过`1e-9`的距离；精确半整数档候选由590增至596，最大被合并距离差约`0.000542`。该日志只保存每次前8个预览候选，不能代表完整top-40，但足以表明三位精度能消除`1e-15`尾差，且没有大面积抹平现有候选层次。

实现只把`round(1000*distanceToHalf)`作为CutSet预排序键；原始`z(S)`仍用于分数性判断、日志、`floor/ceil`分支上下界和strong trial。随后仍按集合规模与任务集合打破同档并列。因此该处理改变的是数值不可辨别候选的确定性先后顺序，不改变候选集合或分支约束。
