...
### 2026-07-16 启发式 PWLF 持久只读视图

继续检查 PWLF 底层后，保留了一项新的数据结构优化：可变 PWLF 仍使用 Segment 链表完成 shift、add、normalize 和 profile 重建；启发式 ADD/EXCHANGE 的高频标量查询额外使用不可变的 primitive 数组视图。视图只在 route profile 实际重建时同步刷新，候选枚举期间直接复用，避免此前失败方案中“每个候选先把链表扁平化再扫描”的重复开销。该结构不缓存可变 PWLF 内部状态，也不改变 completion bound、exact pricing 或通用 merge 的函数语义。
...
两组 root bound 归一化后也不完全相同：原尺度为 `2001.137099`，×20组为 `40004.946767/20=2000.247338`。主要原因不是时间放大改变了原始列模型，而是本次没有固定相同 incumbent 和初始列。×20组归一化 incumbent 为 `2149`，弱于原尺度的 `2128`，使归一化 `UB-LB` 从 `126.863` 增至 `148.753`；首次 graph fixing 后保留的时空弧分别为 `106082` 和 `2501204/20=125060` 个等价时间单位，后者约多17.9%。随后两组又分别分离出 `371/315` 条 cut、最终保留 `129/120` 条 active cut，因而最终 bound 属于两个不同的 UB-dependent fixing 和启发式 cut-closure 轨迹，不能要求严格相差20倍。要隔离纯尺度影响，必须固定同一组初始 sequence、使用同比例 incumbent `42560`，并让两组从相同 cut/column 口径开始。
...
### 2026-07-17 已证明最优排程的早到、窗口内与迟到构成

本次只统计同时满足以下条件的历史算例：求解状态为 `FINISHED`，最终 incumbent 与 lower bound 一致，保存了最优 incumbent 的机器序列，并且使用 `TWETColumnEvaluator` 按原始目标函数重算后与已证明最优值完全一致。按任务完成时间相对 due window 分类：小于窗口左端为早到，落在闭区间内为无惩罚，大于窗口右端为迟到。当前可严格恢复并复核的样本共 5 个算例、200 个任务。

| 算例 | 任务数 | 早到 | 窗口内无惩罚 | 迟到 |
| --- | ---: | ---: | ---: | ---: |
| `wet030_from040_010_2m` | 30 | 13（43.33%） | 2（6.67%） | 15（50.00%） |
| `wet040_001_2m` | 40 | 19（47.50%） | 2（5.00%） | 19（47.50%） |
| `wet040_001_3m` | 40 | 19（47.50%） | 4（10.00%） | 17（42.50%） |
| `wet040_001_2m_timeJitterX10` | 40 | 18（45.00%） | 4（10.00%） | 18（45.00%） |
| `wet050_003_3m_setupR50_timeX10_W300` | 50 | 20（40.00%） | 11（22.00%） | 19（38.00%） |
| **合计** | **200** | **89（44.50%）** | **23（11.50%）** | **88（44.00%）** |
...
### 2026-07-18 60-2 当前版本串行复测与 ng-DSSR repair 卡点

本轮使用当前 `target/classes` 和此前对齐的最佳无 SRI 配置，串行复测原始小时间 `wet060_001_2m`。纯 time-indexed 在 `1696.092s` 内完成，目标和下界均为 `36803`，处理 22 个节点，root 用时 `428.876s`，最终 pool 为 `612250`。其中普通图定价为 `328.127s/4079`，repair 图定价为 `154.273s/461`；master LP 的 `after_pricing`、strong trial 和 repair 合计分别为 `585.346s`、`468.419s` 和 `25.767s`。因此该算例上 time-indexed 后半程仍主要受大列池 LP 和 strong trial 影响，但最终在 1800s 前闭合。

同配置的 ng-DSSR 在 `1806.195s` 时间限制时仍为 `incumbent=37112`、`lowerBound=36739.428063`、`gap=1.0039%`，只处理到 2 个节点。其异常长尾不是普通 root pricing，而是 node 2 的一次右支 strong-trial repair：required arc 为 `36->4`，单次 `GCNGBBStyleNgDssrPricing[FindFeasible]` 用时 `730.105s`，执行 79 轮 DSSR，累计观察 `553835425` 条负非基本路径，最终仍未返回基本列。该次 exact 的分解为 forward `8.791s`、backward `260.495s`、join `458.437s`，初始化仅 `1.221s`。

backward 比 forward 大两个数量级的直接原因已经确定。第一轮 midpoint probe 选择 `Tmid=1685.8125` 时，有限 probe 的正反工作量仍接近平衡；但 `bidirectionalMidpointProbeReuseWithinDssr=true` 会在后续 79 轮中原样复用该 Tmid。随着 ng-set 平均大小由 `6.033` 增至 `8.350`，后向状态对 ng-memory 扩张更敏感，最终 forward kept label 仅 `4580`，backward kept label 达 `105903`。该轮有 32 个 job 不适合 forward half，只有 3 个 job 不适合 backward half；backward extension 构造 `1047900` 个候选，completion bound 只剪掉 `15282` 个，仍有 `1032618` 个 survivor。repair dual 中 required arc 的正 dual 还使大量包含该弧但反复访问其它 job 的 pseudo route 保持负 reduced cost，进一步削弱 0-cutoff completion bound。这个问题属于 DSSR 内 Tmid 复用和 repair dual 共同触发的性能病理，不是 dominance 或 reduced-cost 正确性错误。
...
补充核对初始 Tmid 后，需要修正“初始点本身来自窗口分布中心”的理解。默认公式实际是 midpointLeftBound 与 pricingHorizon 的算术中点，其中左端只取所有任务动态有效窗口左端的全局最小值和 source 最早完成时间的较大者；它不是所有任务窗口中点的中位数，也不关心窗口在时间轴上的密度。一个很早的窗口即可决定左端，因此即使大多数任务集中在后半段，min/max 中点仍可能偏左。60-2 repair 中 dynamicMinHStart=753、pricingHorizon=2888，默认中点约为 1820.5；本次第一轮实际又复用了 node 历史 bestExact 参考 1822.5，并由有限 probe 依次试探 1822.5、1549.125 和 1685.8125，最后选择 1685.8125。它并不是默认公式直接给出的结果。有限 probe 在 1685.8125 处看到 queue ratio 约 1.16，但完整第 79 轮的 label ratio 已变为 23.12，说明主要错误是用浅层、早期 ng-set 下的平衡结果代表后续全部 DSSR 轮。
...
修正后的严格时间乘10组最终在 `2084.988s` 内证明
`obj=bound=308840`，处理408个节点，time-indexed exact pricing累计
`995.323s/8446` 次，最终pool为1155647。root总时间为126.699s，其中正式root
nodeTime为66.067s、pricing为35.435s/98次，root LP bound为303443.875359；
在node 316、总时间1920.614s时找到最优整数解。

随后串行运行不乘10的原尺度对照
`data/setup-variants/50-3/wet050_003_3m_setupR50.dat`，将窗口半宽恢复为W100，
其余保持同一修正后no-cut profile。输出目录为
`test-results/bpc/exp-50-3-R50-W100-cost20-ti-nocut-bestfix-20260726b`。
该组在 `871.103s` 内证明 `obj=bound=30884`，处理390个节点，exact pricing累计
`141.948s/5338` 次，最终pool为542451。root总时间为87.358s，其中正式root
nodeTime为26.946s、pricing为4.186s/120次，root LP bound为30344.387536；
在node 372、总时间866.668s时找到最优整数解。

两组分支树规模接近，408对390个节点，因此时间尺度放大没有在本次实验中明显改变树规模；
主要影响是图定价成本和列池规模。原尺度相对乘10组总时间减少58.22%，即快约2.39倍；
exact pricing总时间减少85.74%，单次平均由约117.84ms降至26.59ms，即快约4.43倍；
最终pool减少约53.06%。root中的pricing更敏感，由35.435s降至4.186s，但root总时间只从
126.699s降至87.358s，因为两组都固定承担约60秒ALNS，并且原尺度root的master LP占比更高。

该对照仍不能视为严格固定起点的纯时间尺度消融。乘10组的初始上界为311740，距最终最优值约
0.94%；原尺度组初始上界为31726，距最终最优值约2.73%。原尺度组因此在大部分搜索期间维持更差
...
原计划待该单进程运行结束后补充。随后按用户要求人工停止；停止时到 node 12、queue 11、
pool 56470，未形成最终下界证书，因此该目录只保留单进程轨迹，不作为完整求解结果。
