...
259. 2026-07-19 有效更新数的跨规模、窗口和 setup root 复测
为检查第258节是否只适用于 50-3 W300，本轮把候选 reservoir 固定为当前默认 100，继续比较 K10/15/20/25。新增场景覆盖 40、50、60 个任务，原始窄窗口、W100、W300，原始 setup、setupR50 和 setupR75；仍关闭 ALNS、RMIH、strong branching、time-indexed root preprocessing 和 SRI，保证比较集中在 root DSSR 更新。所有同场景组的 bound、valid 和列池一致或仅有一条列的差异。逐 run 汇总位于 `test-results/bpc/ab-ng-effective-update-matrix-20260719.csv`。

40-2 原窗口的 K10/15/20/25 exact 为 `5.031/4.681/4.896/4.952s`，certificate 都只有3轮、实际只更新4条路径，说明 K10 已未触顶。40-2 W100 为 `3.688/3.753/3.828/3.572s`，certificate 为 `5/4/4/4` 轮；K15 已达到轮数平台，后续差异不到0.3秒。40-3 W300 原时间几乎不需要 exact，四组均约 `0.06s`；同目录的 timeX10 变体约 `1.40--1.50s`，同样只有2轮和2条有效更新。这些轻场景不支持继续加大 K，也没有显示 K20 的负面结构影响。

50-3 setupR50 W100 的 K10/15/20/25 exact 为 `5.882/5.328/5.533/5.444s`，certificate 为 `5/4/4/4` 轮；K15 后已经进入平台。setupR75 W100 为 `6.483/7.456/6.138/6.066s`，K15 的 exact 调用数偶然增加到20次，其余为16--17次；K20/K25 较快，但 K25 相对 K20 只快0.072秒，不能视为稳定收益。
...
### 266. 60-2 repair 左右支、候选有效率与 Tmid 复核

对 `test-results/bpc/20260719-60-2-ng-best-k20-cand1000-3600-v1` 中 36 次 ng-DSSR `FindFeasible` 按 `repairType` 重新拆分。`type=3` 是左支 forbidden arc，`type=4` 是右支 required arc。右支共 33 次、累计 637.017s、平均 19.304s 和 28.42 轮 DSSR，占 repair exact 总时间 83.6%；左支只有 3 次，但累计 125.279s、平均 41.760s 和 48.33 轮。由此不能简单说“右支单次一定更难”：总瓶颈确实主要来自右支，因为 36 次中 33 次是右支；但在已经触发 repair 的条件下，本次少量左支样本反而更重。结构原因是左支只增加 `sum(lambda using i->j)=0`，父解通常仍有替代列支撑，因而大多数左支根本不进入 repair；右支增加 `sum(lambda using i->j)=1`，同时把 `i->k(k!=j)` 和 `h->j(h!=i)` 记为 branch-implied 竞争弧，父 basis 更经常缺少足够的 required-arc 支撑。
