...
对固定的 $b_j$，取 $a_j=b_j$ 可使宽度成本为 0；得到点窗口后，增大 $b_j$ 只会减少 tardiness，又没有位置成本，因此取允许的最大值 $b_j=U_j$。最终就是

$$
[a_j^*,b_j^*]=[U_j,U_j].
$$

较快场景免费等待到 $U_j$，超过 $U_j$ 的场景承担 tardiness。这里 due-window 位置并没有经过内部权衡，而是被推到外生上界，模型实质上更接近一个给定 due date 为 $U_j$ 的随机调度问题。
