...
public class GCBBAsymmetricBidirectional {
	...
	private enum DynamicSideSelection {
		MORE_LABELS_FIRST, FEWER_LABELS_FIRST
	}
	...
	private DynamicSideSelection dynamicSideSelection;

	// 2026-05-22: 双向 midpoint，只对当前 pricing 轮有效。
	private double tMid;
	// 2026-05-28: Tilk-style 动态 half-way 边界，只控制扩展停止，不进入 label 状态。
	private double dynamicHB;
	private double dynamicHF;
	...
	private long dynamicForwardPops;
	private long dynamicBackwardPops;
	private long forwardBoundaryClears;
	private long backwardBoundaryClears;
	...
	public ArrayList<TWETColumn> solve(LP lp) {
		...
		// 2026-05-28: dynamic half-way 外层交错处理两侧队列，最终仍统一做 crossing-arc join。
	...
	private boolean shouldProcessForward() {
		if (FWUL.isEmpty()) {
			return false;
		}
		if (BWUL.isEmpty()) {
		...
		if (dynamicSideSelection == DynamicSideSelection.FEWER_LABELS_FIRST) {
			return FWUL.size() <= BWUL.size();
		}
		return FWUL.size() >= BWUL.size();
	}
	...
	private DynamicSideSelection parseDynamicSideSelection(String value) {
		if (value == null) {
			return DynamicSideSelection.MORE_LABELS_FIRST;
		}
		String normalized = value.trim().toLowerCase();
		if ("fewer".equals(normalized) || "fewerlabels".equals(normalized)
				|| "fewer_labels".equals(normalized) || "less".equals(normalized)
				|| "lesslabels".equals(normalized) || "less_labels".equals(normalized)) {
			return DynamicSideSelection.FEWER_LABELS_FIRST;
		}
		return DynamicSideSelection.MORE_LABELS_FIRST;
	}
	...
	private void initialize(LP lp) {
		...
		queueOrdering = LabelQueueOrdering.TIME;
		...
		dynamicSideSelection = parseDynamicSideSelection(config.asymmetricBidirectionalSideSelection);
	...
	private void dynamicForwardExtend(LP lp) {
		while (!FWUL.isEmpty()) {
			...
			if (Utility.compareGt(earliestForwardCompletion(label), dynamicHF)) {
				forwardBoundaryClears++;
				forwardBoundaryClearedLabels += FWUL.size();
				FWUL.clear();
				return;
			}
			label = FWUL.poll();
			...
			PackedBitSet currentReachable = currentForwardBoundaryCandidates(label);
	...
	private void dynamicBackwardExtend(LP lp) {
		while (!BWUL.isEmpty()) {
			BackwardLabel label = BWUL.peek();
			if (Utility.compareLt(latestBackwardCompletion(label), dynamicHB)) {
				backwardBoundaryClears++;
				backwardBoundaryClearedLabels += BWUL.size();
				BWUL.clear();
				return;
			}
			label = BWUL.poll();
			...
			Node node = lp.getNode();
			PackedBitSet currentReachable = currentBackwardBoundaryCandidates(label);
	 ...
	 * 漏掉可行列，正确性上可以接受。visited 等静态合法性依赖 reachableSet
	...
	private PackedBitSet currentForwardBoundaryCandidates(ForwardLabel label) {
		...
		for (int nextJob = label.extensionSet.nextSetBit(1); nextJob > 0 && nextJob <= data.n;
				nextJob = label.extensionSet.nextSetBit(nextJob + 1)) {
	 ...
	 * visited 依赖 reachableSet 构造不变量，当前直连 forbidden arc 则留在
	 * canExtendBackward 中即时检查。
	 */
	private PackedBitSet currentBackwardBoundaryCandidates(BackwardLabel label) {
		PackedBitSet currentReachable = new PackedBitSet(data.n + 2);
		for (int prevJob = label.extensionSet.nextSetBit(1); prevJob > 0 && prevJob <= data.n;
				prevJob = label.extensionSet.nextSetBit(prevJob + 1)) {
			if (isDirectBackwardExtensionTimeFeasible(label.jid, label.isSinkRoot, label.frontier, prevJob)) {
	...
	private BackwardLabel extendBackward(BackwardLabel label, int prevJob, LP lp) {
		...
		if (label.isSinkRoot) {
			...
			if (Utility.compareLt(rhoPrime, Math.max(dynamicHB, successorHStart))) {
				return null;
			}
	...
	private void resetStatistics() {
		...
		dynamicForwardPops = 0;
		dynamicBackwardPops = 0;
		forwardBoundaryClears = 0;
		backwardBoundaryClears = 0;
		forwardBoundaryClearedLabels = 0;
		backwardBoundaryClearedLabels = 0;
	}
	...
	private String statisticsSummary() {
		return "labels fw kept/dominated=" + forwardLabelsKept + "/" + forwardLabelsDominated
				...
				+ ", dynamic pops fw/bw=" + dynamicForwardPops + "/" + dynamicBackwardPops
				+ ", boundary clears fw/bw labels=" + forwardBoundaryClearedLabels + "/"
				+ backwardBoundaryClearedLabels + ", clearEvents=" + forwardBoundaryClears + "/"
				+ backwardBoundaryClears
	...
	private boolean isDirectForwardExtensionTimeFeasible(PiecewiseLinearFunction frontier, int prevJob, int nextJob,
			boolean requireDynamicBoundary) {
		...
		return !Utility.compareGt(earliestCompletion, hEnd)
				&& (!requireDynamicBoundary || !Utility.compareGt(earliestCompletion, dynamicHF));
	}
	...
	private boolean isDirectBackwardExtensionTimeFeasible(int firstJob, boolean isSinkRoot,
			PiecewiseLinearFunction frontier, int prevJob, boolean requireDynamicBoundary) {
		...
		double lower = requireDynamicBoundary ? Math.max(dynamicHB, hStart) : hStart;
	...
	/**
	 * 2026-05-28: dynamic half-way 版本没有固定 Tmid 半域，不能用初始化边界永久排除任务；
	 * 这里仅保留 penalty 非空检查，真正的动态剪枝交给 direct feasibility 和队首边界处理。
	 */
