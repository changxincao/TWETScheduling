...
令 $F$ 为这些场景完工时刻的加权经验分布。当两个端点都处于内部且 $a<b$ 时，最优性条件为
...
Cavaliere et al. (2026) 的模型 (1a)–(1m) 同样包含 tour cost、窗口宽度、overtime 和 ET。它明确设置 $w_{0\omega}=0$，时间传播约束 (1e) 允许等待，约束 (1g) 用最后一个客户离开时刻加返程时间计算超过 $T$ 的 overtime。该文只要求窗口宽度非负，所以点窗口可行。
