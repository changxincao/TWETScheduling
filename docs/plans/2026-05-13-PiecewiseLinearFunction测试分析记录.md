...
这次把分段线性函数右端定义域检查和 `mergeMinimum` 的输入契约检查重新梳理了一遍。这里要区分两个值：`tail.end` 是当前 segment 链表真实覆盖到的最右端，`domainEnd` 是函数对象保存的元数据。正常情况下二者应该一致；但 `setDomain/trimToDomain` 这类操作可能把链表裁到 `end<T`，随后又把 `domainEnd` 恢复成原来的 `T`。因此检查 `tail.end == domainEnd` 不是重复检查，而是在定位“实际函数已经被裁短、但元数据还像是完整 `[a,T]`”的情况。

`mergeMinimum` 和 `mergeMinimum2` 现在不再只做普通右端一致性检查，而是接到 `Utility.debugCheckPWLFMergeContract(...)`。该检查由 `Configure.debugPWLFDomainCheck` 控制，默认关闭，不改变当前求解逻辑。打开后会检查四件事：两个输入是否有正长度公共区间、右端是否一致、左函数是否退化成单点、右函数是否退化成单点。发现异常时会向 `Utility.debugMap` 累加对应 key，并在控制台输出操作名、左右函数区间、公共区间、异常类型以及两个函数对象的 `System.identityHashCode`。这不能直接恢复调用路径，但足够判断是哪类操作、哪一对函数出了问题，后续如果需要更强追踪，可以在 label 或 move 层再传入业务 id。

当前结论不变：在 pricing 的 forward label 设定下，普通函数操作应尽量保持 `[a,T]` 的右端统一；退化到 `[T,T]` 的 label 不进入普通 merge/dominance，而是在 pricing 层直接尝试收尾到终点。调试检查只是为了验证后续实现有没有破坏这个约束，不是为了把 `PiecewiseLinearFunction` 改成任意定义域都能处理的通用函数库。
