...
为了单独估计该改进的上限，又关闭浅层probe并进行端到端根节点运行。所有运行都完整闭合，得到相同的`LB=UB=37948`和`valid=true`。其中“首轮WA、后续oracle 500”最接近此前提出的流程：每次exact第一轮用`windowAverage=936.067`，从第2轮开始直接使用离线完整曲线选出的`500`。
...
| 首轮WA、后续oracle 500 | 2 | `12.005/11.998`，均值`12.002` | `8.370/8.328`，均值`8.349` | 7 |

oracle代理相对每轮固定WA的平均总时间下降`43.4%`，exact时间下降`52.4%`。日志确认第一轮确实使用`936.067`，第2轮以后使用`500`；第一次exact共执行10轮DSSR，首轮完整F/B比为`18.669`，切换后第2轮降至`1.180`，后续多数轮在`1--3`。因此此前提出的改进并非只换On-the-exact默认值：它在这个实例上存在明显的潜在收益，而且收益主要来自DSSR轮间选点。

但这仍是oracle上限，不是已经完成的在线算法。固定`Tmid`的一次完整labeling只观察左侧forward和右侧backward，不能免费得到整个时间域上两侧的完整工作曲线；本轮的`500`是付出多次完整labeling后事后选出的。若要落地主线，需要额外构造可在线获得的确定性代理，例如按时间层累计extension、constructed或kept工作量，再用极少量局部候选校准。当前浅probe本身也经常把后续点移动到`480--520`，新方法必须证明其优势来自减少被丢弃候选和降低墙钟噪声，而不只是重新发现同一区间。

本轮只增加默认关闭的诊断入口：可指定完整曲线对应的DSSR轮，并可在第2轮以后强制使用oracle点。未设置JVM属性时不进入这些分支，正式profile、默认中点、probe、DSSR更新、返回列和certificate流程均不变。当前证据只来自一个n30 family实例，不能据此更换正式配置；它支持继续研究低成本的轮间工作量预测，但不支持把oracle写成正式策略。
