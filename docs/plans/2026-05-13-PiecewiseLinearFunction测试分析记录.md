...
随后补了一组更大的方向化随机测试。测试规模仍用 `RANDOM_CASES=500`：一组随机检查 `normalize(Direction)`，分别构造 forward 的左侧不可达 `big_M` 和 backward 的右侧不可达 `big_M`；一组随机检查 `mergeMinimum(Direction)`，forward 按 prefix 下包络闭包，backward 按 suffix 下包络闭包；一组随机检查 `updateDominatedIntervals(Direction)`，分别比较被支配区间打成 `big_M` 后再做 prefix/suffix 闭包的结果。为了让测试能覆盖 discontinuity，`prefixMinRef/suffixMinRef` 也从“只看断点右侧值”改成直接扫描每个 segment 的左右端函数值，避免 vertical gap 下的左/右极限被测试 oracle 漏掉。

这组测试的结果是：`normalize(Direction)` 随机 500 组通过，`mergeMinimum(Direction)` 随机 500 组通过；`updateDominatedIntervals(Direction)` 中 backward partial dominance 有 6/500 个失败。失败的典型原因是 dominated 区间边界附近的极限值处理：forward partial dominance 后做 prefix-min 时主要依赖左侧极限，之前已经围绕这个方向做过较多修正；backward partial dominance 后做 suffix-min 时会对称依赖右侧极限，但当前 `updateDominatedIntervals(..., BACKWARD)` 只是把原 forward 的区间替换逻辑接到 suffix normalize，尚未专门处理 backward 边界极限。因此当前更准确的结论是：backward `normalize` 和有效输入下的 `mergeMinimum` 已经通过随机测试；backward 的 partial dominance 还不能认为完全正确，后续如果 BPC pricing 真要启用 backward partial dominance，需要单独修这块，或者先禁用 backward partial dominance，只做完整 dominance/merge。
