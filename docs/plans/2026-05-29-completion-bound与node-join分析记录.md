...
### 41.54 2026-06-06 无动态窗口节点的 Tmid/Horizon 实验

继续分析 012 中后续节点 forward label 远多于 backward label 的原因。当前直接证据支持：根节点因为可以用 job dual 构造动态 profitable window，`pricingHorizon` 会被局部窗口收紧，`Tmid=(L+U)/2` 基本合理；但非根节点一旦不能启用动态窗口，`U` 退回 `data.CmaxH`，该值来自全局无等待启发式和安全 slack，可能明显大于实际最优完工区域。此时 `Tmid` 被推得太靠右，forward 可以在很大的左半域内直接接 sink，backward 后缀又容易被 completion bound 剪掉，于是出现 forward-dominant 的证明成本。

为避免大改 pricing 结构，本次只增加一个默认关闭的实验开关：`bidirectionalNoWindowHorizonFactor`。当节点没有动态 profitable window 时，`pricingHorizon` 从原来的 `data.CmaxH` 改为 `min(data.CmaxH, data.CmaxH * factor)`，并保留 `earliestSourceCompletion` 作为下界；动态窗口节点仍走原来的 `min(data.CmaxH, localHorizon)`，默认 `NaN` 时完全保持旧逻辑。这个改动同时接入了 `GCNGBBStyleBidirectional`、full-domain、node-join 和 asymmetric 几个双向实现，比较程序可通过 `twet.bpc.fullDomainCompare.noWindowHorizonFactor` 等参数打开。

012 的 direct Java、half-domain、`completionBound=allCycles`、pricingOnly subtree、`maxNodes=10` 测试结果为：`factor=0.60` 能收敛，`solve=83.845s`，`nodes=8`，`pricing=107`，exact `24.986s/23`；`factor=0.50` 能收敛，`solve=40.945s`，`nodes=8`，`pricing=114`，exact `10.296s/22`，是本轮当前代码下最好的完整结果。继续压低并不稳定：`factor=0.45` 在 node2 已出现 `bw=292/344`、新增列 12906 且 RMIH infeasible，后续未稳定返回；`factor=0.40` 在 node4 出现 backward 爆炸，`bw=4025/46181`、completion-bound eval 约 97702，后续同样未稳定返回。因此这个问题不是简单“越小越好”：太大时 forward 证明重，太小时 backward 和后续节点会变重。

其它样例上，010 用 `factor=0.50` 也能完整收敛，`solve=72.371s`，`nodes=6`，`pricing=85`，exact `26.763s/18`。其中 node3 仍有 `fw kept/dominated=32792/4633`、`bw=114/595`，说明 horizon cap 不能消除所有 forward-heavy 节点，只是减少由过大 `CmaxH` 引起的无谓左半域。011 做了 `maxNodes=2` 烟测，`NODE_LIMIT` 正常返回，`solve=22.262s`，exact `4.507s/6`，没有复现 completion-bound 初始化卡死。

关于用户提出的三种思路，当前判断如下。第一，按分支禁弧重新计算 node 级 `CmaxH` 是合理方向，但若仍使用“全局 due 分布 + 无等待安全上界”的公式，禁弧只会改变部分 route duration，未必能把 horizon 拉到真实最优完成区域；若要精确变紧，本身又接近一个受禁弧约束的调度上界问题。第二，用 completion bound 的最小点估计 `Tmid` 在直觉上有价值，但当前 completion bound 构造发生在 `pricingHorizon/Tmid` 之后，若反过来使用它，需要先用临时 Tmid 建一遍 bound，再重建半域缓存和 bound，代码侵入较大；同时 completion bound 是非 elementary 松弛，它的 argmin 时间不一定对应 elementary pricing 的最优完成时间。第三，用 ALNS/初始解 makespan 作为全局 horizon 候选比较稳，可以先作为诊断字段记录，再考虑 `min(data.CmaxH * factor, seedHorizon * margin)` 之类组合；但“每条机器调度最优函数最后一个点的最优时间”需要和 `TWETColumnEvaluator` 的 PWL 语义对齐，不能只拿序列长度替代。

当前结论为：最小风险的实现是保留默认关闭的 no-window horizon factor，用它只处理没有动态窗口的节点；暂不把 `0.50` 设为默认值。就当前 010/011/012 的证据，`0.50` 是比 `0.60` 更强、比 `0.45/0.40` 更稳的候选值，值得后续在 013/014/015 以及更多 30 任务样例上继续跑批量复核。若批量仍稳定，才考虑作为 no-outsourcing/no-window pricing 的默认启发式参数。
