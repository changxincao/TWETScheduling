...
先看一条整数机器序列`depot -> A -> B -> C -> D -> E -> sink`。若`S={B,C}`，只有`A->B`从集合外进入集合内，因此`z(S)=1`。把`D`加入后，`S'={B,C,D}`：原来的`A->B`仍是唯一进入弧，`C->D`由集合外连接变成内部连接，所以`z(S')`仍为1。若改为把不与`S`相邻的`E`加入，则除`A->B`外还新增`D->E`，进入次数由1变成2。若原集合是`S={B,D}`，进入弧为`A->B`和`C->D`，所以`z(S)=2`；加入夹在两段之间的`C`以后，`B,C,D`连成一个片段，只剩`A->B`，进入次数下降为1。
