...
单点方面，底层 `add()` 对零长度 segment 有一定支持，但这里要精确理解：它不要求两侧都刚好在端点相交，也没有任何 `Tmid` 特判。若一侧是 `[x,x]` 单点，另一侧是覆盖 `x` 的函数段，`add()` 会取 `start=max(...)=x`、`end=min(...)=x`，并在覆盖 `x` 的 segment 上用当前 slope/intercept 生成一个 `[x,x]` 的零长度结果。因此“一个 `[0,H]` 函数 + 一个 `Tmid` 单点”只是其中一个例子，底层同样可以处理其他位置的单点，只要另一侧物理 segment 覆盖该点。pricing label 层另有专门处理：`isSinglePointFrontier()` 只识别 `head.start == tail.end == tMid` 的单点，随后进入 `SinglePointStore` 并用 `dominatesSinglePoint(..., tMid, ...)` 和普通 dominance graph 做点支配检查。当前风险不在于 `add()` 完全不能处理 `full-domain + 单点`，而在于 label 层的 single-point 逻辑只参数化到了 `Tmid`。如果后续半域裁剪仍只以 `Tmid` 为边界，这套逻辑是匹配的；如果引入其他裁剪点、局部 horizon 边界或非 `Tmid` 的零长度 frontier，就需要把 single-point 判定和 graph 点查询参数化，否则这些非 `Tmid` 单点会走普通路径或被空域过滤，语义不够稳。
